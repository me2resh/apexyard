...
- **Known gap — lexical `/../` can disagree with the kernel, but only via a symlink.** An earlier draft of this record claimed `<ops>/plaindir/../workspace/A/…` escaped the fork and that `dev` blocked it. Re-measured with real `open()` calls rather than shell `cd`: with a plain directory the kernel and the hook agree, so there is no escape and no divergence. The divergence requires `plaindir` to be a **symlink** — which makes it the same gap as the one below, not a second one — and `dev` did not reliably block that case either; it allowed whenever the ops ticket happened to be a valid migration ticket. Corrected in the third review round.
...
- `.claude/hooks/tests/test_require_migration_ticket.sh` — cases 23–38. Cases 31–34 pin `.cwd` trust; **cases 36–38 are what pin normalisation itself** — they use a fixture whose ops and per-project markers give opposite verdicts, so the exit code reveals which marker answered. Case 34 does not pin normalisation, contrary to an earlier draft: its target lands outside `workspace/` either way, so the ops marker answers identically normalised or not.
- PR me2resh/apexyard#1180 — three rounds of Rex and Hakim review. The first cut, the first redesign, and the `.cwd` fix were each rejected on a defect found by probing rather than by reading; this record was corrected three times for overclaiming. Mutation runs now confirm the suite detects all three defects, which it previously did not.
