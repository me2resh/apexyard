...
- A Bash migration write whose target carries an unexpandable construct is refused, naming the path. **`~/` paths are expanded and gated normally. Relative paths are neither refused nor resolved** — they reach the tier-2 ops marker exactly as on `dev`, which is the known gap below. The refusal message says precisely that, and recommends an absolute path; an earlier draft of this bullet claimed relative paths "are resolved and gated normally — the refusal message says so", which was false on both clauses after round 9 removed the `.cwd` join.
- `~/`, `//`, `/./` and `/../` spellings now resolve to the **correct** project marker instead of silently reaching tier 2. **Relative spellings do not** — that attempt was withdrawn in round 9 (see Decision), so they still reach tier 2 exactly as on `dev`.
...
- **Known gap — the per-target meta-exemption has no coverage.** The file header asserts that `.claude/`, `docs/` and `*.md` targets are skipped per target; deleting that check leaves the suite green. Recorded rather than fixed: the exemption is pre-existing and #1182 rewrites the loop that applies it.
...
- **Known gap — a relative migration target is not resolved (a live Failure 1 instance).** It reaches marker resolution un-normalised, so it falls through to the tier-2 ops marker rather than the project that owns the write — the same wrong-marker signature this change exists to fix, in the one spelling it does not fix. Rounds 3–8 attempted it by joining the harness `.cwd`; round 9 withdrew that as a HIGH fail-open. **Owned by me2resh/apexyard#1182** as an explicit acceptance criterion, because the shared resolver is where it can be done correctly. Recorded here rather than left to a code comment pointing at this document.
