...
- **Pass 2** judges *every* migration-shaped target, not the first. Where the matching targets are governed by more than one marker, the command is **refused** and the operator is asked to split it: gating on any one of them approves the writes the others govern. That is a new refusal shape, and it is strictly closer to `dev`, which blocked the measured case in both argument orders. Two targets in the *same* project still gate normally — the refusal fires on divergent governance, not on writing twice.
