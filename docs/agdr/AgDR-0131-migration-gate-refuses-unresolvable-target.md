...
- **The delta set has three categories, not two.** Resolution deltas (the fix), pass-1 refusals (new, fail-closed, and the only direction where this change blocks what `dev` allowed), and selection deltas — of which there are now none by construction.
