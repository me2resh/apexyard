...
| Reuses existing config layer (`.apexyard/project-config.defaults.json` + `_lib-read-config.sh`) | |
| File-based marker is debuggable (`cat .apexyard/session/active-bootstrap`) | |
...
3. **Reuses existing patterns.** The config layer, the `_lib-` helper convention, the SessionStart-sweep pattern — all already present. No new mechanisms; the marker is a one-line `.apexyard/session/<name>` file like `current-ticket`, the hook chain pattern is the same as `clear-fetch-cache` and friends.

## Scope summary

### 1. Schema — `.apexyard/project-config.defaults.json`

Adds `ticket.bootstrap_skills` listing `setup`, `handover`, `update`, `split-portfolio`. Adopters can extend in `.apexyard/project-config.json`. Same pattern as the rest of the config layer.
...
- `require-active-ticket.sh`:
  ...
  - New bootstrap-marker exemption: reads `.apexyard/session/active-bootstrap`; if its content matches a skill in `ticket.bootstrap_skills`, the gate is skipped.
...
Sweeps stale `.apexyard/session/active-bootstrap` files from interrupted sessions. Wired into `.claude/settings.json` after the existing `onboarding-check`, `check-upstream-drift`, `check-portfolio-config` chain. Logs to stderr only when it actually clears a marker; silent on the no-marker path (the common case).
...
`.apexyard/rules/workflow-gates.md` § "Pre-Build Gate" gets a one-paragraph callout explaining the exemption.
...
- `.apexyard/hooks/_lib-detect-bash-write.sh` — new helper
- `.apexyard/hooks/require-active-ticket.sh` — Bash + bootstrap exemption extension
- `.apexyard/hooks/require-migration-ticket.sh` — parallel Bash extension
- `.apexyard/hooks/clear-bootstrap-marker.sh` — new SessionStart sweep
- `.apexyard/project-config.defaults.json` — `ticket.bootstrap_skills` entry
- `.apexyard/skills/{setup,handover,update,split-portfolio}/SKILL.md` — Step 0 + Cleanup
- `.apexyard/hooks/tests/test_detect_bash_write.sh` — lib unit tests (32 cases)
- `.apexyard/hooks/tests/test_require_active_ticket_bash.sh` — integration tests (12 cases)
- `.apexyard/rules/workflow-gates.md` — § Pre-Build Gate doc note
