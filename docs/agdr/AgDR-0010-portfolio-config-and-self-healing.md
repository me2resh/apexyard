...
The existing config layer (`.apexyard/project-config.{defaults,}.json` + `_lib-read-config.sh` + `config_get_or`, with 6+ hooks already using it) is the right home — it already supports per-fork overrides, ships an `_lib-` shared library pattern, and provides shallow-merge semantics with sensible defaults.
...
Chosen: **`portfolio:` block in `.apexyard/project-config.{defaults,}.json` + helper library + self-healing SessionStart hook + skill audit + new `/split-portfolio` skill, all in one bundled PR.**
...
### 2. Helper — `.apexyard/hooks/_lib-portfolio-paths.sh`
...
### 5. New skill — `.apexyard/skills/split-portfolio/SKILL.md`
...
### 7. Tests — `.apexyard/hooks/tests/test_portfolio_paths.sh`
...
- **Path-resolution callout duplicated across skills.** Future cleanup could extract to a single shared `.apexyard/skills/_shared-conventions.md` with each SKILL.md linking to it. Out of scope here; tracked as future cleanup if it becomes a maintenance burden.
...
- **Extract the path-resolution callout** to a single shared `.apexyard/skills/_shared-conventions.md` file if the duplication becomes a maintenance pain point. Currently 18 copies of the same paragraph.
