...
4. **Performance contract is mechanically enforced.** A smoke test asserts no framework-distinctive tokens (`apexyard.projects.yaml`, `_lib-portfolio-paths.sh`, `/handover`, `portfolio_*`) leak into the extracted output, AND that the same files serve the integrated framework without modification.
