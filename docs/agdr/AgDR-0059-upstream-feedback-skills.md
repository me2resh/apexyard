...
3. **Mandatory leak-scrubbing** — both skills write to a PUBLIC repo, so private registered-project names must never appear. The skills instruct authoring-time scrubbing and rely on `block-private-refs-in-public-repos.sh` as the mechanical backstop, consistent with `.apexyard/rules/leak-protection.md`. The `<!-- private-refs: allow -->` escape is used only on explicit user confirmation.
...
- Two new skills: `.apexyard/skills/report-apexyard-bug/SKILL.md`, `.apexyard/skills/request-apexyard-feature/SKILL.md`.
...
- New: `.apexyard/skills/{report-apexyard-bug,request-apexyard-feature}/SKILL.md`
- Edited: `CLAUDE.md` (skills table + counts), `site/*` (counts)
- Related: `/bug` + `/feature` (the project-scoped siblings), `.apexyard/rules/leak-protection.md` (the scrubbing rule + backstop hook), AgDR-0030 (#268 issue-skill marker), the `/onboard`→`/setup` alias-redirect precedent for how thin intent-named skills coexist.
