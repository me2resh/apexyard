...
| No gate calls the masker | By name only. Pin 2 finds the text `_lib-mask-quoted` or `mask_quoted_metachars` in files under `.claude/hooks/`, `.githooks/`, and `bin/`, and in `.claude/settings.json`, outside test folders. It compares full paths from the repository root. It also pins the list of functions that bash defines when it sources the library. It does not find a call through a dynamic name or `eval`, or a copy of the scanner under a new name. It catches honest edits, not deliberate hiding. | `test_mask_quoted.sh` section 5, pin 2. For `require-active-ticket.sh` only, hook case A also pins the exit code, however the masker is called. |
...
2. **Writes the detector does not recognise.** On this branch, `bash_command_appears_to_write` reports no write for commands such as `touch`, `ln -s`, `mkdir`, `truncate`, `rsync`, or a redirect written as `>&word`. When one of these sits beside a quoted `>`, the note still prints. The note says the detector does not see every kind of write, so its text stays true. The verdict still blocks. Without the quoted `>`, the hook allows such a command today (exit 0).
3. **Two consumers have no behaviour check against masking.** `require-migration-ticket.sh` and `warn-review-marker-write.sh` also read `bash_command_appears_to_write`. Their test suites have no quoted-write case, such as `bash -c 'echo x > src/app.ts'`. So only pin 2 guards them, and pin 2 matches names only. A behaviour case in the migration suite would check the verdict itself. `warn-review-marker-write.sh` always exits 0 (AgDR-0111), so a case there would check the warning text. This PR does not add those cases.
