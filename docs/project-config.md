...
merges exactly as documented above (the override wins, unchanged), but
`_lib-read-config.sh` also prints one advisory `WARN:` line to stderr naming
the key and every dropped entry, the first time that override is read in a
session. This never blocks and never changes the merged value — it only
makes an otherwise-silent drop visible. It applies only to a key that has a
default array **in this file**; `migration_paths`, `migration_label`,
...
built-in default in code), so a drop against one of those produces no
warning — see AgDR-0167.
