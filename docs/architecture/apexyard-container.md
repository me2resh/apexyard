...
**2026-09-17 — Split-portfolio adapter trust (AgDR-0156, me2resh/apexyard#1315).** A project-local adapter must exec the framework hooks, not an unrelated symlink that happens to look valid. `--check` now compares link targets to the framework paths. The Codex regression runs a generated PreToolUse command. After AgDR-0157 that command is `dispatch-bash.sh`, which still blocks `git add -A`.
