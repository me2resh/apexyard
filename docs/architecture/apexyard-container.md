...
**2026-09-17 — Split-portfolio adapter trust (AgDR-0156, me2resh/apexyard#1315).** A project-local adapter must exec the framework hooks, not an unrelated symlink that happens to look valid. `--check` now compares link targets to the framework paths. It exits non-zero even when no workspace-bearing project increments `count`. The Codex regression runs generated `dispatch-bash.sh` with the session pin unset, so the fixture anchor is the one that runs.
