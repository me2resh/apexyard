...
**2026-09-17 — Rex body validation before marker write (AgDR-0161, me2resh/apexyard#1322).** The Output Format was instruction-only. A posted review could omit required headings and still write a SHA marker. Architecture change: `_lib-review-markers.sh` validates the local body file before the write. The merge gate still reads only the SHA. The hooks container stays the gate. The agent file tells Rex to call the helper.
