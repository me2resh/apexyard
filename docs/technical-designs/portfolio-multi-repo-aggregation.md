...
So this threshold is **new**. It is introduced explicitly as `portfolio.stale_days`, default `30`, read by `pa_stale_days` with the same shape as `pa_concurrency`. The library falls back to a literal in-code default when the key is absent, because the file or the key may simply not be there — an adopter with no `.claude/project-config.json`, or a defaults file predating the key. **Not** because a `portfolio` override would drop it: `_lib-read-config.sh:235` merges with `jq -s '.[0] * .[1]'`, and jq's `*` recurses into objects, so an override naming only `portfolio.registry` keeps `stale_days` (measured). It replaces **arrays** wholesale, which is where the framework's "shallow merge" wording came from. That wording is wrong in three places — `_lib-read-config.sh:234`, `.claude/rules/skill-first.md:48`, `.claude/rules/workflow-gates.md:51` — and needs its own ticket. It is not corrected here.
...
| 8 | `/handover` — N-repo intake, the D6 container layout with per-repo clone status and its rerun / cleanup / failure rules including the collision-rename report, per-repo sections, worst-of verdict with the unassessed annotation, contributor union, `repos:` / `primary:` / `workspace:` entry, the D4 prompt rule, and the in-repo write target + path rules (D4 § In-repo writes) — replacing all six hardcoded `$WORKSPACE_DIR/<name>` repo roots with the recorded per-repo path: the write sites at `SKILL.md` 735–739, 1368, 1536 and the read sites at 172, 339, 285 | 4.5 h | PR-A |
...
| Any of the six `$WORKSPACE_DIR/<name>` repo roots survives, so a step targets the container directory | Med | High | The three write sites fail loudly; the three read sites fail **silently**, blending N repos into one harnessability score and invalidating D6's deciding argument. All six are named with line numbers in D4 § In-repo writes and in task 8, and AT-5 asserts both a write root and a scan root resolve to a per-repo clone |
...
Four properties the implementation must assert. **They do not all live at the same layer, and the difference is load-bearing.** The library is a shell library, so the folds, the fan-out, and the exception set are asserted mechanically under the mock `gh` (D5-2) — no network, deterministic. A rendered row is produced by **skill prose**, and `/projects`, `/inbox`, and `/tasks` ship as `SKILL.md` with no executable script, so no shell test can capture one. That half stays a recorded manual diff, exactly as D5-3 frames it. Each row says which layer carries which half.
...
| **AT-5** | Every multi-repo write **and scan** resolves to a per-repo clone, never the container | Three-repo fixture under D6's container layout. **Write path:** with a recorded selection of repo B, assert the resolved repo root is B's container path `<workspace_dir>/<name>/<dir-B>`, that `git -C <resolved> rev-parse --git-dir` succeeds, and that it is **not** `<workspace_dir>/<name>`. **Read path:** assert the steps 2–6 scan root (`SKILL.md` 172, 339) and the default-branch lookup (285) each resolve **per repo** — three distinct roots for three repos, every one a real repo — and that a marker file planted in repo C is **not visible** from repo B's scan root. That last assertion is the one that catches a blended harnessability score. Negative assertion: with no selection recorded, write-path resolution fails closed and emits no path rather than falling back to the container | The branch-and-PR half is not exercised — no throwaway multi-repo product exists to receive a real PR. The PR-B live smoke covers the singular path only |

AT-1 and AT-2 are the byte-identity contract; AT-3 and AT-4 are the two ways the aggregation can be confidently wrong rather than merely absent. AT-5 covers both halves of the path rule: the writes that reach someone else's repository, and the scans whose blending would be invisible. None of the four asserts on model-rendered text — which is why the exception rule moved out of skill prose and into `pa_exceptions` (D3).
