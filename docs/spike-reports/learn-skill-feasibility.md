...
| 5 | **Branch-name rejection cluster** — 9 `validate-branch-name.sh` BLOCKED, all on missing `GH-` prefix or wrong type prefix. Rule already documented in `.apexyard/rules/git-conventions.md`. | 9 hits across 4 sessions. Common shape: `chore/215-…` (missing `GH-`), `feature/…-…` without ticket. | Auto-suggest the corrected branch name in the hook output (parse the active ticket ID and propose `git checkout -b feature/GH-<id>-…`). Currently the hook tells you the format; doesn't compute the right answer. | **MEDIUM** — small UX win, recurring enough to be worth automating. |
...
- Active rules + hooks: `<ops_root>/.apexyard/rules/*.md` + `<ops_root>/.apexyard/hooks/*.sh`
- Suppression ledger: `<ops_root>/.claude/learn/suppressed.yaml`

**Output modes** (the choice mentioned in the spike spec):

1. **Adopter-config-only** (`--config`): proposes additions to `.apexyard/project-config.json` (e.g. tighten an existing hook's threshold, add a path to a deny-list). Lowest blast radius.
