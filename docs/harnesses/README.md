...
> **Not a rebrand.** The primary tagline is still **"for Claude Code."** These pages document the engineering reality — and as of 2026-07-09 that reality includes **two live-proven adapters (opencode and pi)**: a real model turn under each was actually blocked by a delegated bash gate. That meets the rebrand trigger's ≥2-live-proven condition, but the headline flip is a **separate, coordinated decision** and hasn't been made — the tagline stays "for Claude Code" until it is. See [Rebrand trigger](#rebrand-trigger) below.
...
| **Codex** | Generated — `.claude/skills/` → `.agents/skills/`, `.claude/agents/*.md` → `.codex/agents/*.toml` | **Delegated** — generated `.codex/hooks.json` execs the *unmodified* `.claude/hooks/*.sh`; block-on-`exit 2` preserved | `bin/sync-codex-adapter.sh` → `.codex/hooks.json` | ⏳ **Not enforcing in headless** — `codex exec -m gpt-5.5` runs a shell command *without* firing the project `.codex/hooks.json` hook; the interactive `codex` TUI + user-level `~/.codex/hooks.json` path is untested (#840, [`codex.md`](codex.md)) |
...
The honest asterisk, now **per-harness** rather than a blanket "all pending" — and the real lesson is an **architectural split** between how each adapter reaches the gate:

> **Imperative-plugin adapters (opencode, pi) enforce in headless/CLI — both live-proven. Declarative-hooks adapters (Codex, Cursor) do not fire project hooks in their headless/CLI modes and need the IDE/interactive path plus, likely, a user-level config.**
>
> opencode's plugin and pi's dispatcher register on the harness's own tool-call event, so the gate runs *inside* a real model turn regardless of interactive-vs-headless. Codex's `.codex/hooks.json` and Cursor's `.cursor/hooks.json` are static config files the harness has to *load* — and the headless CLIs (`codex exec`, `cursor-agent`) don't load a project-level one. That single design difference is why opencode and pi cleared the live bar first and Codex/Cursor haven't.

- **opencode + pi are live-proven (2026-07-09).** The last hop — the harness's live internal dispatch actually invoking the delegated bash hook mid-turn — has been observed with credentials for both: a real `git add -A` / `git add .` refused by the unmodified gate, verbatim hook output, nothing staged. The one precondition each needs is that the tool call actually reaches the gate (opencode `--auto`, pi headless `-a`/`--approve`); without it the harness resolves the action itself and never calls the hook.
- **Codex is not enforcing in headless.** The generated `.codex/hooks.json` schema is correct and the model mapping (`opus`→`gpt-5.5`) is right, but a definitive probe showed `codex exec -m gpt-5.5` runs a benign shell command *without* firing the project `.codex/hooks.json` hook (empty instrumentation log) — so headless `codex exec` does **not** honor project hooks, the same shape as the `cursor-agent` CLI. Untested and likely required: the interactive `codex` TUI and/or a user-level `~/.codex/hooks.json` (mirroring the Cursor user-level finding). Not proven working, not the delegated block opencode/pi have. Tracked in #840.
- **Cursor blocks, but only via failClosed at user level.** Two hard findings this round: the `cursor-agent` CLI does **not** run `hooks.json` (it uses its own `cli-config` permissions model), and Cursor IDE 3.x loads hooks only from **user-level** `~/.cursor/hooks.json`, not the project `.cursor/hooks.json` the generator emitted. Even loaded at user level, the observed block came from `failClosed:true` on a hook-runner error — the delegated `.claude/hooks/*.sh` did not cleanly execute — so it is **not** equivalent to the opencode/pi delegated-execution proofs. See [`cursor.md`](cursor.md); the generator/install fix is tracked separately.
...
- **[Codex](codex.md)** — ⏳ **Not enforcing in headless**. Generated from `.claude/`; the `.codex/hooks.json` schema is correct and the model mapping (`opus`→`gpt-5.5`) is right, but `codex exec -m gpt-5.5` runs a shell command without firing the project hook — headless `codex exec` doesn't honor project `.codex/hooks.json`. Interactive TUI + user-level `~/.codex/hooks.json` path untested (#840). Full workflow in [`docs/codex-adapter.md`](../codex-adapter.md) (linked, not moved).
