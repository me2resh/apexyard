...
|---------|-------------|-----------|
| `harness-adapters/pi/` | Hand-maintained `DEFAULT_GATES` table (AgDR-0082) | Dispatcher extension, `tool_call` event, `{block, reason}` return |
...
All three converge on the same underlying principle established by AgDR-0086 ("hooks stay bash, not ported"): `.claude/hooks/*.sh` remains the single source of truth for every gate decision, and every harness reaches it through a thin transport layer rather than a reimplementation. This adapter's contribution is closing the "gate table can drift from settings.json" gap the pi adapter still carries — noted as the pattern pi should converge to in a follow-up, not modified as part of this change.
