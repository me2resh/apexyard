...
Read the registry path via `portfolio_registry` and the per-project docs dir via `portfolio_projects_dir` from `.codex/hooks/_lib-portfolio-paths.sh`. Source the helper at the top of any bash block that touches those paths:
