...
# `.agents/skills/update/SKILL.md` § 8a:
#
...
LIB_OPS="$ROOT/.codex/hooks/_lib-ops-root.sh"
LIB_PORT="$ROOT/.codex/hooks/_lib-portfolio-paths.sh"
LIB_CFG="$ROOT/.codex/hooks/_lib-read-config.sh"
DEFAULTS="$ROOT/.codex/project-config.defaults.json"
...
build_pre_v2_split() {
  local sb="$1"
  mkdir -p "$sb/public/.codex/hooks" "$sb/public/workspace/demo"
...
YAML
  ...
  cp "$LIB_OPS"  "$sb/public/.codex/hooks/_lib-ops-root.sh"
  cp "$LIB_PORT" "$sb/public/.codex/hooks/_lib-portfolio-paths.sh"
  cp "$LIB_CFG"  "$sb/public/.codex/hooks/_lib-read-config.sh"
  cp "$DEFAULTS" "$sb/public/.codex/project-config.defaults.json"

  cat > "$sb/public/.codex/project-config.json" <<JSON
{
  "portfolio": {
...
build_single_fork() {
  local sb="$1"
  mkdir -p "$sb/.codex/hooks" "$sb/projects" "$sb/workspace"
...
MD
  ...
  cp "$LIB_OPS"  "$sb/.codex/hooks/_lib-ops-root.sh"
  cp "$LIB_PORT" "$sb/.codex/hooks/_lib-portfolio-paths.sh"
...
detect_v2_needed() {
  ...
  (
    ...
    if ! jq -e '.portfolio.registry' .codex/project-config.json >/dev/null 2>&1; then
      return 1
    fi
    return 0
  )
...
snapshot_state() {
  ...
  (
    cd "$fork" || return 1
    {
      ...
      for path in onboarding.yaml .gitignore .apexyard-fork .codex/project-config.json; do
        if [ -e "$path" ]; then
...
echo '{"_comment": "no portfolio block"}' > "$SB/.codex/project-config.json"
...
ONB_KEY=$(jq -r '.portfolio.onboarding // empty' "$SB/public/.codex/project-config.json")
WS_KEY=$(jq -r '.portfolio.workspace_dir // empty' "$SB/public/.codex/project-config.json")
[ -n "$ONB_KEY" ] && [ -n "$WS_KEY" ] \
...
SIBLING_TEST="$ROOT/.codex/hooks/tests/test_split_portfolio_v2_migration.sh"
