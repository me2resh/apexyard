...
| `--from-version vN.N.N` | Explicit version-anchor override. Use when `.codex/framework-version` is missing (legacy fork pre-v1.4.0) OR you've manually rolled back and the anchor is stale. The chain is built against this value instead of the file. Refuses if the value doesn't match `vMAJOR.MINOR.PATCH`. |
| `--skip-migrations` | Sync the framework files but DO NOT run the per-version migration chain. Prints an advisory warning naming each skipped pair and reminding the operator that the migrations can be replayed later with `bash .codex/migrations/<pair>.sh`. The anchor file IS still advanced to the new release tag, so subsequent runs won't re-offer the same migrations. Use sparingly — the chain is the point. |
...
`/update` edits framework-root files (resolving merge conflicts, updating AGENTS.md imports, etc.) which the `require-active-ticket.sh` PreToolUse hook would otherwise block when the only "ticket" is the upstream-sync work itself. Write a marker so the hook exempts this skill (it's on the default `bootstrap_skills` list in `.codex/project-config.defaults.json`):

```bash
mkdir -p .codex/session && echo "update" > .codex/session/active-bootstrap
```
...
```
CONFLICT in .codex/rules/pr-workflow.md

Upstream changed:  adds "### Both merge shapes are gated (#47)" section
Local changed:     inserted custom header paragraph at the top

Options:
  (1) Keep mine        — git checkout --ours .codex/rules/pr-workflow.md
  (2) Accept upstream  — git checkout --theirs .codex/rules/pr-workflow.md
...
```bash
source "$(git rev-parse --show-toplevel)/.codex/hooks/_lib-detect-deprecated-config.sh"
...
```
ApexYard /update detected N config block(s) in .codex/project-config.json
...
The detector can't tell them apart — choose:
  ...
  [y] yes, remove the listed keys from .codex/project-config.json
...
| Reply | Action |
|-------|--------|
| `y` | Run `remove_deprecated_config_keys` (edits `.codex/project-config.json` in place, no commit), then `git add .codex/project-config.json` to stage the change for the operator's review. Print `Removed N keys. Staged for review — diff with: git diff --staged .codex/project-config.json`. |
...
if portfolio_is_v2; then
  V2_NEEDED=0
elif ! jq -e '.portfolio.registry' .codex/project-config.json >/dev/null 2>&1; then
...
Migrate now? This will:
  ...
  - Add portfolio.{onboarding,workspace_dir} keys to .codex/project-config.json
...
```bash
SIBLING_ROOT=$(dirname "$(jq -r '.portfolio.registry' .codex/project-config.json)")
...
```bash
source "$(git rev-parse --show-toplevel)/.codex/hooks/_lib-migration-chain.sh"
...
```
ApexYard /update: no .codex/framework-version anchor in this fork.
...
Per-version migration chain (3 steps):
  1. v1.0.0 → v1.1.0   (.codex/migrations/v1.0.0-to-v1.1.0.sh)
  2. v1.1.0 → v1.2.0   (.codex/migrations/v1.1.0-to-v1.2.0.sh)
  3. v1.2.0 → v1.3.0   (.codex/migrations/v1.2.0-to-v1.3.0.sh)
...
```
Step N/M — <PAIR>
  Script: .codex/migrations/<PAIR>.sh
  Lines:  <wc -l output>
...
git add .codex/framework-version 2>/dev/null || true
```
...
```
Skipped migrations (replay later with bash .codex/migrations/<pair>.sh):
  - v1.1.0-to-v1.2.0
  - v1.2.0-to-v1.3.0
```
...
| `.codex/project-config.json` missing (no override) | Skip step 8 silently — by definition no deprecated keys to surface. |
...
### Why a file-based version anchor (`.codex/framework-version`)
...
| `framework_version` key in `.codex/project-config.json` | Mixes a *measured* fact (which release am I on?) with *configured* facts (custom paths, feature toggles). Adopters edit project-config by hand; the anchor would drift on copy/paste. |
| **Standalone file `.codex/framework-version` (CHOSEN)** | One line, one job: the version of the framework this fork last synced against. Easy to read, easy to write, robust to history rewrites. /update owns it. |
...
2. **Replay is meaningful** — `bash .codex/migrations/v1.2.0-to-v1.3.0.sh` is a complete, idempotent operation. Replaying a chain step from a monolithic script is much harder.
3. **The chain shape stays in source-control history** — every release that ships either adds a real migration OR a no-op placeholder. A v1.5.0 adopter looking at `.codex/migrations/` sees an exhaustive list of "what changed for adopters at each release".
...
- `.codex/hooks/_lib-migration-chain.sh` — the chain detection + run library.
- `.codex/migrations/README.md` — convention for authoring a new per-version migration script.
- `.codex/rules/pr-workflow.md` — the PR workflow the sync branch will follow.
- `.codex/hooks/block-main-push.sh` — the hook that motivates the sync-branch approach.
