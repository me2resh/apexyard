...
workspace_dir=""
if [ -n "$ops_root" ] && [ -f "$ops_root/.codex/hooks/_lib-portfolio-paths.sh" ] && [ -f "$ops_root/.codex/hooks/_lib-read-config.sh" ]; then
  # shellcheck source=/dev/null
  . "$ops_root/.codex/hooks/_lib-read-config.sh" 2>/dev/null
  # shellcheck source=/dev/null
  . "$ops_root/.codex/hooks/_lib-portfolio-paths.sh" 2>/dev/null
...
if [ -n "$ops_root" ]; then
  ...
  case "$workspace" in
    "(ops)"|"(unknown)"|"")
      ;;
    *)
      marker_paths+=("$ops_root/.codex/session/tickets/$workspace")
      ;;
  esac
  marker_paths+=("$ops_root/.codex/session/current-ticket")
