...
When `--briefing` (or `-b`) is passed, skip every other section and shell out to `.agents/skills/status/briefing.sh`. The full default `/status` output is unchanged for invocations without the flag.
...
| Active ticket | `<ops_root>/.codex/session/tickets/<workspace>` first (only when workspace is a real name), then `<ops_root>/.codex/session/current-ticket`. Format: `#<number> — <title>`. No marker → `(none)` |
...
Implementation: the logic lives in `.agents/skills/status/briefing.sh` so it's testable in isolation and runnable from a plain shell (no Codex dependency). Smoke tests at `.codex/hooks/tests/test_status_briefing.sh`.

When invoked from inside Codex as `/status --briefing`, run the helper:

```bash
bash .agents/skills/status/briefing.sh
```
...
The shim walks up from `$PWD` looking for the apexyard fork root (`onboarding.yaml` + `apexyard.projects.yaml`), then runs the same briefing helper. No PATH-shadowing of the `claude` binary, no recursion into Codex — pure `bash` end-to-end.
