...
> in the issue. Per `.codex/rules/leak-protection.md`, describe the context
...
echo "report-apexyard-bug" > "$ops_root/.codex/session/active-issue-skill"
```
...
**a) What part of the framework?** (which hook / skill / rule / agent / doc — e.g. `detect-role-trigger.sh`, `/handover`, `.codex/rules/leak-protection.md`)
...
3. **Scrub private project names** — this writes to a PUBLIC repo. Describe context generically. See `.codex/rules/leak-protection.md`. Use the `<!-- private-refs: allow -->` body marker ONLY on explicit user confirmation that a name must stay.
