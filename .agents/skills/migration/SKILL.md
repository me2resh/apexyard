...
echo "migration" > "$ops_root/.codex/session/active-issue-skill"
```
...
Labels: `migration` (or whatever the project configures as its migration label — check `.codex/project-config.json` key `migration_label`, default `migration`), plus the priority label (`P0` / `P1` / `P2` / `P3`).
...
4. **Never create the AgDR under `.codex/` or `docs/` unless the migration IS against apexyard itself**. For managed projects, the AgDR lives inside that project's repo (`workspace/<project>/docs/agdr/`), not in the ops fork.
