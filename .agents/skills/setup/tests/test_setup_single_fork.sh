...
#   - .codex/project-config.json is NOT written (single-fork mode
...
build_pre_setup_single_fork() {
  local sb="$1"
  mkdir -p "$sb/.codex/hooks" "$sb/projects"
...
if [ -f "$SB/.codex/project-config.json" ]; then
  # If it exists, it must NOT have a .portfolio key
  if command -v jq >/dev/null 2>&1; then
    has_portfolio=$(jq -r 'has("portfolio")' "$SB/.codex/project-config.json" 2>/dev/null)
  ...
  mark_pass "no .codex/project-config.json written in single-fork mode"
fi
