...
#  10. AGENTS.md /handover skill row mentions harnessability
#  11. docs/multi-project.md /handover skill row mentions harnessability
#  12. Skill count in AGENTS.md is unchanged (this is an extension to an
...
SKILL_MD="$SRC_ROOT/.agents/skills/handover/SKILL.md"
AGDR="$SRC_ROOT/docs/agdr/AgDR-0042-harnessability-scoring-dimensions.md"
CLAUDE_MD="$SRC_ROOT/AGENTS.md"
...
# ---------------------------------------------------------------------------
# Case 10: AGENTS.md /handover skill row mentions harnessability.
# ---------------------------------------------------------------------------
...
if echo "$handover_row_claude" | grep -iqF "harnessability"; then
  mark_pass "10. AGENTS.md /handover skill row mentions harnessability"
else
  mark_fail "10. AGENTS.md /handover row" "no 'harnessability' on the /handover row"
fi
...
# skill, so the AGENTS.md "Available skills (N)" header must stay CONSISTENT
...
# ---------------------------------------------------------------------------
actual_skills=$(find "$SRC_ROOT/.agents/skills" -name SKILL.md 2>/dev/null | wc -l | tr -d ' ')
if grep -qE "### Available skills \($actual_skills\)" "$CLAUDE_MD"; then
  mark_pass "12. AGENTS.md skill-count header matches actual skill count ($actual_skills)"
else
  mark_fail "12. skill count" "AGENTS.md '### Available skills (N)' header != actual SKILL.md count ($actual_skills)"
fi
