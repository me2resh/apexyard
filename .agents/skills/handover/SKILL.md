...
Per-language LSP plugins live in Codex's marketplace. Install once; the skill detects the active language and dispatches automatically.
...
`/handover` may run before any tracker tickets exist for the project being adopted, so the `require-active-ticket.sh` PreToolUse hook would block the registry / `projects/<name>/` writes the skill needs. Write a marker so the hook exempts this skill (it's on the default `bootstrap_skills` list in `.codex/project-config.defaults.json`):

```bash
mkdir -p .codex/session && echo "handover" > .codex/session/active-bootstrap
```
...
- `.codex/session/active-bootstrap` — the marker itself (step 0)
...
- [ ] PR workflow (`.codex/rules/pr-workflow.md`) — every change goes through a PR
...
`/update` reads these two files to know which topology to diff against (see [`.agents/skills/update/SKILL.md`](../update/SKILL.md) § "Topology drift detection").
...
Mirrors `/tickets-batch`'s failure-handling shape (see [`.agents/skills/tickets-batch/SKILL.md`](../tickets-batch/SKILL.md) § "Failure handling").
...
✓ <name> is cloned at $WORKSPACE_DIR/<name>/ and indexed in MCP.
  ...
  Codex LSP plugin installed. Cross-project queries still need grep
