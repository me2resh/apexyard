...
Cache results per-session at `.codex/session/agdr-index.cache.json` keyed by registry mtime. Subsequent invocations within the same session re-read from cache; `--no-cache` forces a refresh. The cache is small (~1 KB per AgDR — id, path, category, project, body-snippet) and rebuilt in well under a second for typical portfolios.
...
Per-session cache at `.codex/session/agdr-index.cache.json`:
...
1. **Read-only** — never modifies any AgDR, never writes to `docs/agdr/` of any project. Cache writes go to `.codex/session/` only.
