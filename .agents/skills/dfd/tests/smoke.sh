...
SB="$TMPROOT/sandbox"
mkdir -p "$SB/.codex/hooks" "$SB/projects" "$SB/workspace/billing-api/src" "$SB/workspace/notifications-svc/src"
...
cp "$HOOKS_DIR/_lib-multi-repo-trace.sh" "$SB/.codex/hooks/"
cp "$HOOKS_DIR/_lib-portfolio-paths.sh"  "$SB/.codex/hooks/"
cp "$HOOKS_DIR/_lib-read-config.sh"      "$SB/.codex/hooks/"
DEFAULTS_SRC="$(cd "$DFD_DIR/../.." && pwd)/project-config.defaults.json"
cp "$DEFAULTS_SRC" "$SB/.codex/project-config.defaults.json" 2>/dev/null || true
...
RESULT=$(cd "$SB" && bash -c '
  ...
  source ./.codex/hooks/_lib-multi-repo-trace.sh
