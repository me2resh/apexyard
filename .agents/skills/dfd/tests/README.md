...
```bash
bash .agents/skills/dfd/tests/smoke.sh
```
...
bash .agents/skills/dfd/discover.sh /path/to/project [scope-hint]

# Classification heuristics (axis 6)
bash .agents/skills/dfd/classify.sh /path/to/project

# Mermaid markdown generator
bash .agents/skills/dfd/generate-mermaid.sh <project-name> [discovery.yaml] [classifications.yaml]

# Threat Dragon JSON serialiser
echo "$MODEL_JSON" | bash .agents/skills/dfd/generate-dragon.sh
```
