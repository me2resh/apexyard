...
echo "task" > "$ops_root/.codex/session/active-issue-skill"
```
...
The title prefix is derived from the content, and must come from the project's configured prefix whitelist (`.codex/project-config.*.json` → `.ticket.prefix_whitelist`, default list at `.codex/project-config.defaults.json`). Shipped defaults map as follows:
