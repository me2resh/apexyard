...
Pairs with `/launch-check` (fans out to this skill at milestone boundaries) and complements the existing `> 80%` coverage gate from `.codex/rules/workflow-gates.md`. **Not** run per-PR — see § "When to use this" for the cadence rationale.
...
Read `mutation.runner` from `.codex/project-config.json` (merge over `.codex/project-config.defaults.json`):
...
`.codex/project-config.defaults.json` ships a `mutation` block:
...
Adopters override per-project in `.codex/project-config.json`:
...
| File | Purpose |
|------|---------|
| `.agents/skills/mutation-test/SKILL.md` | This file — the skill spec |
| `.agents/skills/mutation-test/detect.sh` | Language detection + runner-availability check (shared between full and `--check-only` modes) |
| `.agents/skills/mutation-test/tests/smoke.sh` | Language-detection + graceful-degradation + report-shape tests |
| `.codex/project-config.defaults.json` → `mutation.*` | Default runner map + threshold + timeout |
| `.agents/skills/launch-check/SKILL.md` § "10. Behaviour quality" | The launch-check dimension that dispatches here |
...
- `.agents/skills/launch-check/SKILL.md` — milestone-boundary umbrella that fans out to this skill
- `.agents/skills/pdf/SKILL.md` — the graceful-degrade shape this skill mirrors
- `.agents/skills/process/SKILL.md` — sibling skill with the same exit-3 install-advisory pattern
- `.codex/rules/workflow-gates.md` — the existing > 80% coverage gate that this skill complements (coverage = was-it-executed; mutation = does-it-constrain)
