...
This skill is the **happy path** for parallel execution. The companion rule [`.codex/rules/parallel-work.md`](../../rules/parallel-work.md) defines **when** to *offer* fan-out proactively. Read both before invoking.
...
| Custom | Any subagent in `.codex/agents/` |
...
- Per-project: `<ops_root>/.codex/session/tickets/<project>` if the task targets a managed project's `workspace/<name>/`
- Fallback: `<ops_root>/.codex/session/current-ticket` for ops-fork edits
...
Cannot fan out — these tasks share write targets:
  - Task 2: edits .codex/hooks/pre-push-gate.sh
  - Task 4: edits .codex/hooks/pre-push-gate.sh
