...
inside Codex's environment and writes HTML directly via `Write`; this
...
```bash
python3 .agents/skills/journey/tests/render_smoke.py
```
...
| Input | Output |
|-------|--------|
| `.agents/skills/journey/fixtures/sample-checkout.yaml` | `/tmp/sample-checkout.html` |

Or pass explicit paths:

```bash
python3 .agents/skills/journey/tests/render_smoke.py path/to/journey.yaml /tmp/out.html
```
