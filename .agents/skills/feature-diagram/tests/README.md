...
```bash
bash .agents/skills/feature-diagram/tests/smoke.sh
```
...
- The helper is deliberately conservative: when a row's `Source` column doesn't mention an axis, that subgraph is rendered as `(none)`. Re-runs in Codex may populate axes that pure-grep misses; that's the point of the LLM-driven path.
