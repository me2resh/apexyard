...
description: Record per-PR architecture-review approval for design-artifact PRs. ONLY on explicit per-PR architecture approval.
argument-hint: "<pr-number>"
---

# /approve-architecture — Record Per-PR Design-Review Approval

Writes `.codex/session/reviews/<owner>__<repo>__<pr>-architecture.approved` (repo-qualified path, see AgDR-0060) with the current HEAD SHA so the `require-architecture-review.sh` merge-gate hook will let a design-artifact PR through. Without this marker, the hook blocks merges on any PR that touches a technical design, a migration AgDR, or a feature spec / PRD.
...
Normally Tariq writes the marker himself on an APPROVED verdict (see `.codex/agents/solution-architect.toml`). This skill is the **operator path** to record the same marker — for when a human architect reviewed the design, or when you need to re-record after a rebase.
...
. "$MARKER_HOME/.codex/hooks/_lib-review-markers.sh"
...
mkdir -p "$MARKER_HOME/.codex/session/reviews"
...
- The marker is gitignored (`.codex/session/` is in `.gitignore`). Session state, not code.
