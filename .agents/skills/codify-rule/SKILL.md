...
- **The miss is actually covered by an existing framework rule.** When generating the draft, scan `.codex/rules/*.md` for keyword matches (auth, secrets, AgDR, ticket-vocab). If a likely framework rule already covers this concern, surface it: "this looks similar to `.codex/rules/<file>.md`. Capture as a handbook anyway (more specific), or amend the framework rule?". Default to capturing the handbook.
