...
5. **project-config / template alignment** —
   ...
   `.codex/project-config.defaults.json` matches the sections actually in
...
```bash
bash .agents/skills/investigation/tests/smoke.sh
```
...
- The interactive interview flow (lives inside Codex; not bash-testable).
