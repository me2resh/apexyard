...
echo "investigation" > "$ops_root/.codex/session/active-issue-skill"
```
...
Read `.codex/session/current-ticket` to determine the active project context. Then:
...
Read `.ticket.prefix_whitelist` from `.codex/project-config.*.json`. If `Investigation` (case-insensitive) is not in the list, warn and stop:
...
(The shipped default in `.codex/project-config.defaults.json` includes `Investigation`. This check exists for forks that have customised the whitelist — see apexyard#109.)
