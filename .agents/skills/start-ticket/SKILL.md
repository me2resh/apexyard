...
Writes a session marker so the `require-active-ticket.sh` PreToolUse hook permits Edit/Write on code paths. Without it, the hook blocks edits to anything outside `.codex/`, `docs/`, `projects/*/docs/`, and `*.md`.
...
All markers live in the ops fork (gitignored). No more `.codex/session/` inside each managed-project clone. `tickets/<project>` is a FILE (tier 1) or a DIRECTORY holding `<safe-branch>` markers (tier 0) — the hook's `-f` tests keep the two from colliding.

This is the mechanical enforcement of the Pre-Build Gate in `.codex/rules/workflow-gates.md` — "do not start coding until the ticket exists".
...
Source the tracker library and call `tracker_view`. The library dispatches the right CLI based on `.tracker.kind` in `.codex/project-config.{defaults,}.json` — `gh` (default), `linear`, `jira`, `asana`, `custom`, or `none`. See `.codex/hooks/_lib-tracker.sh` and AgDR-0033.

```bash
source "$(git rev-parse --show-toplevel)/.codex/hooks/_lib-read-config.sh"
source "$(git rev-parse --show-toplevel)/.codex/hooks/_lib-tracker.sh"
...
Match the convention in `.codex/rules/git-conventions.md`.
...
if [ -n "$project" ]; then
  ...
  if [ -n "$wt_branch" ]; then
    ...
    marker="$ops_root/.codex/session/tickets/$project/$safe_branch"
  else
    marker="$ops_root/.codex/session/tickets/$project"
  fi
  mkdir -p "$(dirname "$marker")"
else
  marker="$ops_root/.codex/session/current-ticket"
  mkdir -p "$(dirname "$marker")"
fi
```
...
file, remove it first (`rm "$ops_root/.codex/session/tickets/$project"`) so the
...
- `.codex/session/` (including `.codex/session/tickets/`) is gitignored — markers are per-machine, per-clone of the ops fork.
...
- To clear a specific project's marker: `rm <ops_root>/.codex/session/tickets/<project>`.
- To clear the ops-level fallback: `rm <ops_root>/.codex/session/current-ticket`.
- Exempt paths (`.codex/`, `docs/`, `projects/*/docs/`, any `*.md`) don't need a ticket — the skill is only required before touching source / config / infra.
- **Migration from pre-#41 layout**: if your workflow still has a `.codex/session/current-ticket` inside a managed-project clone (`workspace/<name>/.codex/session/current-ticket`), it's harmless but no longer read by the hook. Delete it or re-run `/start-ticket` to have the new marker written under the ops fork's `.codex/session/tickets/<name>`.
