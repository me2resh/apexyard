...
| **AEO** (Agentic Engine Optimization) | Will a coding agent prefer this doc over its training data when coding against this product? | Codex, Cursor, Aider, Cline, Continue |
...
**G3 detail.** The skill iterates the AI-crawler list at `.codex/registries/ai-crawlers.json` (12 entries spanning training and retrieval scopes). For each crawler it reports:
...
**Reminder**: this is the upstream `skill.md` convention, **distinct from Codex's `SKILL.md`**. See the naming-clash section above.
...
```bash
source "$(git rev-parse --show-toplevel)/.codex/hooks/_lib-audit-history.sh"
...
4. **`skill.md` (the audit's capability-manifest check) is distinct from Codex's `SKILL.md`** — see the naming-clash callout at the top of this file. The check is for the **lowercase `skill.md` at site root**.
5. **AI-crawler list lives in the registry.** `.codex/registries/ai-crawlers.json` is the single source of truth. The G3 check iterates the list. Adopters who want to add an internal crawler edit the JSON.
...
| File | Purpose |
|------|---------|
| `.codex/registries/ai-crawlers.json` | v1 AI-crawler list (12 entries) consumed by the G3 check |
| `templates/audits/geo-audit.md` | Per-run body template (audit family standard, AgDR-0019) |
| `.codex/hooks/_lib-audit-history.sh` | Shared persistence + trend rendering (audit family standard, AgDR-0019) |
| `.agents/skills/seo-audit/SKILL.md` | Google-shaped SEO sibling — independent, complementary |
| `.agents/skills/launch-check/SKILL.md` | Fans out to both this skill and `/seo-audit` at milestone boundaries |
