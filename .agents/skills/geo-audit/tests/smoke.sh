...
#   9. AI-crawler registry exists at .codex/registries/ai-crawlers.json,
#      valid JSON, lists all 11 named crawlers from the ticket
#  10. Audit template exists at templates/audits/geo-audit.md
#  11. AGENTS.md skill count is 53 AND /geo-audit row is present
...
#
# The skill itself runs inside Codex (interactive audit + persistence).
...
ops_root="$script_dir"
while [ "$ops_root" != "/" ] && [ ! -f "$ops_root/AGENTS.md" ]; do
  ops_root=$(dirname "$ops_root")
done
if [ ! -f "$ops_root/AGENTS.md" ]; then
  echo "FAIL: could not locate ops-fork root (AGENTS.md missing)"
  exit 1
fi
...
SKILL="$ops_root/.agents/skills/geo-audit/SKILL.md"
AGDR="$ops_root/docs/agdr/AgDR-0043-geo-audit-skill.md"
REGISTRY="$ops_root/.codex/registries/ai-crawlers.json"
TEMPLATE="$ops_root/templates/audits/geo-audit.md"
SEO_SKILL="$ops_root/.agents/skills/seo-audit/SKILL.md"
LAUNCH_SKILL="$ops_root/.agents/skills/launch-check/SKILL.md"
CLAUDEMD="$ops_root/AGENTS.md"
...
assert_grep_fixed "Naming clash phrase present" \
  "distinct from Codex's \`SKILL.md\`" "$SKILL"
...
assert_grep "Names the registry file"   "\\.codex/registries/ai-crawlers\\.json" "$SKILL"
...
# ---------------------------------------------------------------------------
# 11. AGENTS.md skill count + row
# ---------------------------------------------------------------------------
echo ""
echo "11. AGENTS.md skill count is 53 + /geo-audit row present:"
