#!/usr/bin/env bun
/**
 * block-git-add-all hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/block-git-add-all.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "block-git-add-all.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[block-git-add-all] bash script not found, skipping` }
  }
...
const block_git_add_all: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by block-git-add-all hook:\n${r.stderr}`)
      }
    },
  }
}

export default block_git_add_all
