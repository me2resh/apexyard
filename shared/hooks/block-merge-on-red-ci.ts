#!/usr/bin/env bun
/**
 * block-merge-on-red-ci hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/block-merge-on-red-ci.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "block-merge-on-red-ci.sh")
...
function runBashHook(input: BashInput): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[block-merge-on-red-ci] bash script not found, skipping` }
  }
...
const block_merge_on_red_ci: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook({ tool: input.tool, args: output.args || {} })
      if (!r.ok) {
        throw new Error(`BLOCKED by block-merge-on-red-ci hook:\n${r.stderr}`)
      }
    },
  }
}

export default block_merge_on_red_ci
