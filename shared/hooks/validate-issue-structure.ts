#!/usr/bin/env bun
/**
 * validate-issue-structure hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/validate-issue-structure.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "validate-issue-structure.sh")
...
function runBashHook(input: BashInput): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[validate-issue-structure] bash script not found, skipping` }
  }
...
const validate_issue_structure: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook({ tool: input.tool, args: output.args || {} })
      if (!r.ok) {
        throw new Error(`BLOCKED by validate-issue-structure hook:\n${r.stderr}`)
      }
    },
  }
}

export default validate_issue_structure
