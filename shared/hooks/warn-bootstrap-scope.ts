#!/usr/bin/env bun
/**
 * warn-bootstrap-scope hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/warn-bootstrap-scope.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "warn-bootstrap-scope.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[warn-bootstrap-scope] bash script not found, skipping` }
  }
...
const warn_bootstrap_scope: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by warn-bootstrap-scope hook:\n${r.stderr}`)
      }
    },
  }
}

export default warn_bootstrap_scope
