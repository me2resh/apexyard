#!/usr/bin/env bun
/**
 * require-design-review-for-ui hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/require-design-review-for-ui.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "require-design-review-for-ui.sh")
...
function runBashHook(input: BashInput): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[require-design-review-for-ui] bash script not found, skipping` }
  }
...
const require_design_review_for_ui: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook({ tool: input.tool, args: output.args || {} })
      if (!r.ok) {
        throw new Error(`BLOCKED by require-design-review-for-ui hook:\n${r.stderr}`)
      }
    },
  }
}

export default require_design_review_for_ui
