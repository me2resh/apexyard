#!/usr/bin/env bun
/**
 * auto-code-review hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/auto-code-review.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "auto-code-review.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[auto-code-review] bash script not found, skipping` }
  }
...
const auto_code_review: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by auto-code-review hook:\n${r.stderr}`)
      }
    },
  }
}

export default auto_code_review
