#!/usr/bin/env bun
/**
 * check-upstream-drift hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/check-upstream-drift.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "check-upstream-drift.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[check-upstream-drift] bash script not found, skipping` }
  }
...
const check_upstream_drift: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by check-upstream-drift hook:\n${r.stderr}`)
      }
    },
  }
}

export default check_upstream_drift
