#!/usr/bin/env bun
/**
 * suggest-ticket-template hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/suggest-ticket-template.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "suggest-ticket-template.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[suggest-ticket-template] bash script not found, skipping` }
  }
...
const suggest_ticket_template: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by suggest-ticket-template hook:\n${r.stderr}`)
      }
    },
  }
}

export default suggest_ticket_template
