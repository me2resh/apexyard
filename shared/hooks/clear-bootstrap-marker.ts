#!/usr/bin/env bun
/**
 * clear-bootstrap-marker hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/clear-bootstrap-marker.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "clear-bootstrap-marker.sh")
...
function runBashHook(input: BashInput): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[clear-bootstrap-marker] bash script not found, skipping` }
  }
...
const clear_bootstrap_marker: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook({ tool: input.tool, args: output.args || {} })
      if (!r.ok) {
        throw new Error(`BLOCKED by clear-bootstrap-marker hook:\n${r.stderr}`)
      }
    },
  }
}

export default clear_bootstrap_marker
