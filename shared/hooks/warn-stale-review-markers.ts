#!/usr/bin/env bun
/**
 * warn-stale-review-markers hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/warn-stale-review-markers.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "warn-stale-review-markers.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[warn-stale-review-markers] bash script not found, skipping` }
  }
...
const warn_stale_review_markers: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by warn-stale-review-markers hook:\n${r.stderr}`)
      }
    },
  }
}

export default warn_stale_review_markers
