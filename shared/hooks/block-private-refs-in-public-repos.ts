#!/usr/bin/env bun
/**
 * block-private-refs-in-public-repos hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/block-private-refs-in-public-repos.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "block-private-refs-in-public-repos.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[block-private-refs-in-public-repos] bash script not found, skipping` }
  }
...
const block_private_refs_in_public_repos: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by block-private-refs-in-public-repos hook:\n${r.stderr}`)
      }
    },
  }
}

export default block_private_refs_in_public_repos
