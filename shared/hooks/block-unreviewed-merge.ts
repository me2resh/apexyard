#!/usr/bin/env bun
/**
 * block-unreviewed-merge hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/block-unreviewed-merge.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "block-unreviewed-merge.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[block-unreviewed-merge] bash script not found, skipping` }
  }
...
const block_unreviewed_merge: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by block-unreviewed-merge hook:\n${r.stderr}`)
      }
    },
  }
}

export default block_unreviewed_merge
