#!/usr/bin/env bun
/**
 * validate-commit-format hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/validate-commit-format.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "validate-commit-format.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[validate-commit-format] bash script not found, skipping` }
  }
...
const validate_commit_format: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by validate-commit-format hook:\n${r.stderr}`)
      }
    },
  }
}

export default validate_commit_format
