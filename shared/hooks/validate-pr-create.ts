#!/usr/bin/env bun
/**
 * validate-pr-create hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/validate-pr-create.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "validate-pr-create.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[validate-pr-create] bash script not found, skipping` }
  }
...
const validate_pr_create: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by validate-pr-create hook:\n${r.stderr}`)
      }
    },
  }
}

export default validate_pr_create
