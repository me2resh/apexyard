#!/usr/bin/env bun
/**
 * require-migration-ticket hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/require-migration-ticket.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "require-migration-ticket.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[require-migration-ticket] bash script not found, skipping` }
  }
...
const require_migration_ticket: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by require-migration-ticket hook:\n${r.stderr}`)
      }
    },
  }
}

export default require_migration_ticket
