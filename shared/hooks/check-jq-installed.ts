#!/usr/bin/env bun
/**
 * check-jq-installed hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/check-jq-installed.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "check-jq-installed.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[check-jq-installed] bash script not found, skipping` }
  }
...
const check_jq_installed: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by check-jq-installed hook:\n${r.stderr}`)
      }
    },
  }
}

export default check_jq_installed
