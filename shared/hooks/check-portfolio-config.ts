#!/usr/bin/env bun
/**
 * check-portfolio-config hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/check-portfolio-config.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "check-portfolio-config.sh")
...
function runBashHook(input: BashInput): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[check-portfolio-config] bash script not found, skipping` }
  }
...
const check_portfolio_config: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook({ tool: input.tool, args: output.args || {} })
      if (!r.ok) {
        throw new Error(`BLOCKED by check-portfolio-config hook:\n${r.stderr}`)
      }
    },
  }
}

export default check_portfolio_config
