#!/usr/bin/env bun
/**
 * remind-mcp-tools hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/remind-mcp-tools.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "remind-mcp-tools.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[remind-mcp-tools] bash script not found, skipping` }
  }
...
const remind_mcp_tools: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by remind-mcp-tools hook:\n${r.stderr}`)
      }
    },
  }
}

export default remind_mcp_tools
