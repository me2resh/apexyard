#!/usr/bin/env bun
/**
 * detect-role-trigger hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/detect-role-trigger.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "detect-role-trigger.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[detect-role-trigger] bash script not found, skipping` }
  }
...
const detect_role_trigger: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by detect-role-trigger hook:\n${r.stderr}`)
      }
    },
  }
}

export default detect_role_trigger
