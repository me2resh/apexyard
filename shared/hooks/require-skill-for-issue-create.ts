#!/usr/bin/env bun
/**
 * require-skill-for-issue-create hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/require-skill-for-issue-create.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "require-skill-for-issue-create.sh")
...
function runBashHook(input: BashInput): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[require-skill-for-issue-create] bash script not found, skipping` }
  }
...
const require_skill_for_issue_create: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook({ tool: input.tool, args: output.args || {} })
      if (!r.ok) {
        throw new Error(`BLOCKED by require-skill-for-issue-create hook:\n${r.stderr}`)
      }
    },
  }
}

export default require_skill_for_issue_create
