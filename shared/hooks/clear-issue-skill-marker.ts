#!/usr/bin/env bun
/**
 * clear-issue-skill-marker hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/clear-issue-skill-marker.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "clear-issue-skill-marker.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[clear-issue-skill-marker] bash script not found, skipping` }
  }
...
const clear_issue_skill_marker: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by clear-issue-skill-marker hook:\n${r.stderr}`)
      }
    },
  }
}

export default clear_issue_skill_marker
