#!/usr/bin/env bun
/**
 * pin-ops-root hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/pin-ops-root.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "pin-ops-root.sh")
...
function runBashHook(input: BashInput): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[pin-ops-root] bash script not found, skipping` }
  }
...
const pin_ops_root: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook({ tool: input.tool, args: output.args || {} })
      if (!r.ok) {
        throw new Error(`BLOCKED by pin-ops-root hook:\n${r.stderr}`)
      }
    },
  }
}

export default pin_ops_root
