#!/usr/bin/env bun
/**
 * block-agent-routing-drift hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/block-agent-routing-drift.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "block-agent-routing-drift.sh")
...
function runBashHook(input: BashInput): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[block-agent-routing-drift] bash script not found, skipping` }
  }
...
const block_agent_routing_drift: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook({ tool: input.tool, args: output.args || {} })
      if (!r.ok) {
        throw new Error(`BLOCKED by block-agent-routing-drift hook:\n${r.stderr}`)
      }
    },
  }
}

export default block_agent_routing_drift
