#!/usr/bin/env bun
/**
 * require-active-ticket hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/require-active-ticket.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "require-active-ticket.sh")
...
function runBashHook(input: BashInput): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[require-active-ticket] bash script not found, skipping` }
  }
...
const require_active_ticket: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook({ tool: input.tool, args: output.args || {} })
      if (!r.ok) {
        throw new Error(`BLOCKED by require-active-ticket hook:\n${r.stderr}`)
      }
    },
  }
}

export default require_active_ticket
