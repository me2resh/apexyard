#!/usr/bin/env bun
/**
 * suggest-mcp-reindex-after-clone hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/suggest-mcp-reindex-after-clone.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "suggest-mcp-reindex-after-clone.sh")
...
function runBashHook(input: BashInput): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[suggest-mcp-reindex-after-clone] bash script not found, skipping` }
  }
...
const suggest_mcp_reindex_after_clone: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook({ tool: input.tool, args: output.args || {} })
      if (!r.ok) {
        throw new Error(`BLOCKED by suggest-mcp-reindex-after-clone hook:\n${r.stderr}`)
      }
    },
  }
}

export default suggest_mcp_reindex_after_clone
