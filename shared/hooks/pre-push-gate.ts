#!/usr/bin/env bun
/**
 * pre-push-gate hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/pre-push-gate.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "pre-push-gate.sh")
...
function runBashHook(input: BashInput): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[pre-push-gate] bash script not found, skipping` }
  }
...
const pre_push_gate: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook({ tool: input.tool, args: output.args || {} })
      if (!r.ok) {
        throw new Error(`BLOCKED by pre-push-gate hook:\n${r.stderr}`)
      }
    },
  }
}

export default pre_push_gate
