#!/usr/bin/env bun
/**
 * suggest-mcp-search hook — auto-generated wrapper.
 *
 * Wraps the original bash implementation in .claude/hooks/suggest-mcp-search.sh as an
...
const BASH_SCRIPT = join(REPO_ROOT, ".claude", "hooks", "suggest-mcp-search.sh")

function runBashHook(tool: string, args: Record<string, any>): { ok: boolean; stderr: string } {
  if (!existsSync(BASH_SCRIPT)) {
    return { ok: true, stderr: `[suggest-mcp-search] bash script not found, skipping` }
  }
...
const suggest_mcp_search: Plugin = async () => {
  return {
    "tool.execute.before": async (input, output) => {
      const r = runBashHook(input.tool, output.args || {})
      if (!r.ok) {
        throw new Error(`BLOCKED by suggest-mcp-search hook:\n${r.stderr}`)
      }
    },
  }
}

export default suggest_mcp_search
