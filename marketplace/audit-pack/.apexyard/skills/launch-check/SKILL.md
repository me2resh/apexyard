...
- Read `.apexyard/project-config.json → mutation.threshold` (default 60% from `.apexyard/project-config.defaults.json`)
...
| File | Purpose |
|------|---------|
| `.apexyard/hooks/_lib-audit-history.sh` | Audit-history library shared with /threat-model, /security-review, etc. (4 functions: `audit_resolve_dir`, `audit_run_persist`, `audit_run_list`, `audit_render_trend`) |
