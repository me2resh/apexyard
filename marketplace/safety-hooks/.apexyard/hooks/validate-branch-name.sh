...
# .apexyard/project-config.json (.branch.type_whitelist). Defaults ship at
# .apexyard/project-config.defaults.json. See apexyard#109 for the schema.
...
TRACKER_ID_PATTERN=""
if [ -n "$REPO_ROOT" ] && [ -f "$REPO_ROOT/.apexyard/hooks/_lib-tracker.sh" ]; then
  # shellcheck disable=SC1090,SC1091
  . "$REPO_ROOT/.apexyard/hooks/_lib-tracker.sh"
...
Required shape (.apexyard/rules/git-conventions.md § "Branch Naming"):
  {type}/{TICKET-ID}-{description}

Accepted types: ${TYPES//|/, }
  (configurable: .apexyard/project-config.*.json → .branch.type_whitelist)

Accepted ticket-ID pattern: ${INNER_PATTERN}
  (configurable: .apexyard/project-config.*.json → .tracker.id_pattern)
