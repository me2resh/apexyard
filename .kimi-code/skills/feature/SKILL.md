...
echo "feature" > "$ops_root/.apexyard/session/active-issue-skill"
```
...
5. **Labels auto-applied.** `enhancement` always, plus the priority label. The priority label scheme is read from `.apexyard/project-config.*.json` → `.ticket.label_priority_scheme` (default `P0,P1,P2,P3`); forks that use a different scheme (e.g. `priority-p0`) configure it there.
6. **Title prefix.** `[Feature]` by default. The accepted prefix list is read from `.apexyard/project-config.*.json` → `.ticket.prefix_whitelist`; if a fork has added alternate feature-class prefixes (e.g. `[Enhancement]`), this skill will accept them. See apexyard#109 for the schema.
