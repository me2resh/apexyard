...
if [ -n "$REGISTRY_OVERRIDE" ]; then
  ...
  if [ -n "$toplevel" ] && [ -f "$toplevel/.apexyard/hooks/_lib-portfolio-paths.sh" ]; then
    # shellcheck source=/dev/null
    . "$toplevel/.apexyard/hooks/_lib-read-config.sh"
    # shellcheck source=/dev/null
    . "$toplevel/.apexyard/hooks/_lib-portfolio-paths.sh"
