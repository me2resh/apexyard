...
`/setup` runs BEFORE any portfolio is configured, so no project tickets can exist yet. The `require-active-ticket.sh` PreToolUse hook would otherwise block every Edit / Write / Bash-write the skill needs to make. To stay coherent with the ticket-first rule without forcing adopters to file a placeholder ticket against nothing, the skill writes a one-line marker at `.apexyard/session/active-bootstrap` containing the skill name. The hook reads the marker and exempts skills listed in `ticket.bootstrap_skills` (in `.apexyard/project-config.defaults.json` — `setup` is on the default list).

Run this **before any tool calls that edit files**:

```bash
mkdir -p .apexyard/session && echo "setup" > .apexyard/session/active-bootstrap
```
...
- `.apexyard/project-config.json` has a `portfolio:` block pointing at a sibling repo (config-block mode, recommended; introduced #145), OR
...
6. **Configure path resolution in the fork** (recommended — v2 config-block mode):
   ...
   - Write `.apexyard/project-config.json` with the v2 `portfolio:` block pointing at the sibling repo. Substitute the actual sibling-dir name the operator chose for `apexyard-portfolio` below:
   ...
   - Stage `.gitignore`, `.apexyard/project-config.json`, and `.apexyard-fork` for commit. All three are per-fork, not per-machine.
...
7. **Verify**: source `.apexyard/hooks/_lib-portfolio-paths.sh` and call `portfolio_validate`. Skill MUST refuse to declare success if validate fails — surface the specific failure and ask the operator to fix it before re-running.
