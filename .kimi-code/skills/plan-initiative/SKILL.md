...
Read the registry path via `portfolio_registry`, the per-project docs dir via `portfolio_projects_dir`, and the template via `portfolio_resolve_template initiative.md` — all from `.apexyard/hooks/_lib-portfolio-paths.sh`. Source the helper at the top of any bash block that touches those paths:
...
Defaults match today's single-fork layout (`./apexyard.projects.yaml`, `./projects`). Adopters in split-portfolio mode override the `portfolio.{registry, projects_dir, ideas_backlog}` keys in `.apexyard/project-config.json`. Don't hardcode literal `projects/` paths — the helper resolves whichever mode the adopter is in. See `docs/multi-project.md`.
...
echo "plan-initiative" > "$ops_root/.apexyard/session/active-issue-skill"
```
...
11. **Active-issue-skill marker handling.** Write `plan-initiative` to `<ops_root>/.apexyard/session/active-issue-skill` before the first `gh issue create` in pass 1; remove on EVERY exit path (success, abort, error). Per AgDR-0030.
...
- **Resource allocation / role assignment is out of scope.** The framework's role-trigger machinery (per `.apexyard/rules/role-triggers.md`) handles who-picks-up-what once tickets are filed. `/plan-initiative` stops at the filing step.
...
- [`.apexyard/skills/handover/SKILL.md`](../handover/SKILL.md) § 7.5 — the per-item filing UX this skill mirrors (introduced #376)
- [`templates/initiative.md`](../../../templates/initiative.md) — the master initiative-doc template
- [`.apexyard/skills/write-spec/SKILL.md`](../write-spec/SKILL.md) — the per-feature PRD skill that each milestone naturally flows into after filing
- [`.apexyard/skills/validate-idea/SKILL.md`](../validate-idea/SKILL.md) — the pre-spec gate that sometimes runs BEFORE `/plan-initiative` (when the initiative starts as a raw idea)
