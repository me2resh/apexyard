...
Read `.apexyard/session/current-ticket` to determine which repo we're working in. If absent:
...
Read the priority scheme from `.apexyard/project-config.json` → `.ticket.label_priority_scheme` (fallback `P0,P1,P2,P3`). Present the values:
...
Read the whitelist from `.apexyard/project-config.json` → `.ticket.prefix_whitelist` (fallback `Feature, Bug, Chore, Refactor, Testing, CI, Docs`). Present numbered options:
...
9. **Read schema from project-config.** Use `.ticket.prefix_whitelist`, `.ticket.label_priority_scheme`, and `.ticket.required_sections` from `.apexyard/project-config.json` (with defaults from `.apexyard/project-config.defaults.json`). Don't hard-code the schema in the skill — config drives.
