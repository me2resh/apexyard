...
Writes a session marker so the `require-active-ticket.sh` PreToolUse hook permits Edit/Write on code paths. Without it, the hook blocks edits to anything outside `.apexyard/`, `docs/`, `projects/*/docs/`, and `*.md`.

Marker layout (apexyard#41 + #513):

| Path | When the hook uses it |
|------|----------------------|
| `<ops_root>/.apexyard/session/tickets/<project>/<safe-branch>` | **Tier 0 (#513).** Edit is under `<ops_root>/workspace/<project>/` AND the file's repo is on a git-worktree branch (or `CLAUDE_WORKTREE_BRANCH` is set). Lets parallel agents on the *same* project hold independent tickets. `safe-branch` = branch with `/`→`__`. |
| `<ops_root>/.apexyard/session/tickets/<project>` | **Tier 1.** Edit is under `<ops_root>/workspace/<project>/` AND this per-project marker exists (as a FILE). Single-agent default. |
| `<ops_root>/.apexyard/session/current-ticket` | **Tier 2.** Fallback. Checked if neither above matched. Also the marker for ops-repo framework edits (no `workspace/<name>/` prefix). |

All markers live in the ops fork (gitignored). No more `.apexyard/session/` inside each managed-project clone. `tickets/<project>` is a FILE (tier 1) or a DIRECTORY holding `<safe-branch>` markers (tier 0) — the hook's `-f` tests keep the two from colliding.

This is the mechanical enforcement of the Pre-Build Gate in `.apexyard/rules/workflow-gates.md` — "do not start coding until the ticket exists".
...
Source the tracker library and call `tracker_view`. The library dispatches the right CLI based on `.tracker.kind` in `.apexyard/project-config.{defaults,}.json` — `gh` (default), `linear`, `jira`, `asana`, `custom`, or `none`. See `.apexyard/hooks/_lib-tracker.sh` and AgDR-0033.

```bash
source "$(git rev-parse --show-toplevel)/.apexyard/hooks/_lib-read-config.sh"
source "$(git rev-parse --show-toplevel)/.apexyard/hooks/_lib-tracker.sh"
...
Match the convention in `.apexyard/rules/git-conventions.md`.
...
if [ -n "$project" ]; then
  ...
  if [ -n "$wt_branch" ]; then
    ...
    marker="$ops_root/.apexyard/session/tickets/$project/$safe_branch"
  else
    marker="$ops_root/.apexyard/session/tickets/$project"
  fi
  mkdir -p "$(dirname "$marker")"
else
  marker="$ops_root/.apexyard/session/current-ticket"
  mkdir -p "$(dirname "$marker")"
fi
```
...
file, remove it first (`rm "$ops_root/.apexyard/session/tickets/$project"`) so the
...
- `.apexyard/session/` (including `.apexyard/session/tickets/`) is gitignored — markers are per-machine, per-clone of the ops fork.
...
- To clear a specific project's marker: `rm <ops_root>/.apexyard/session/tickets/<project>`.
- To clear the ops-level fallback: `rm <ops_root>/.apexyard/session/current-ticket`.
- Exempt paths (`.apexyard/`, `docs/`, `projects/*/docs/`, any `*.md`) don't need a ticket — the skill is only required before touching source / config / infra.
- **Migration from pre-#41 layout**: if your workflow still has a `.apexyard/session/current-ticket` inside a managed-project clone (`workspace/<name>/.apexyard/session/current-ticket`), it's harmless but no longer read by the hook. Delete it or re-run `/start-ticket` to have the new marker written under the ops fork's `.apexyard/session/tickets/<name>`.
