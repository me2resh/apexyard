...
> Per `.apexyard/rules/leak-protection.md`, describe the motivating context
...
echo "request-apexyard-feature" > "$ops_root/.apexyard/session/active-issue-skill"
```
...
3. **Scrub private project names** — this writes to a PUBLIC repo. See `.apexyard/rules/leak-protection.md`. Use the `<!-- private-refs: allow -->` marker ONLY on explicit user confirmation.
