...
# `.apexyard/skills/update/SKILL.md` § 8a:
#
...
LIB_OPS="$ROOT/.apexyard/hooks/_lib-ops-root.sh"
LIB_PORT="$ROOT/.apexyard/hooks/_lib-portfolio-paths.sh"
LIB_CFG="$ROOT/.apexyard/hooks/_lib-read-config.sh"
DEFAULTS="$ROOT/.apexyard/project-config.defaults.json"
...
build_pre_v2_split() {
  local sb="$1"
  mkdir -p "$sb/public/.apexyard/hooks" "$sb/public/workspace/demo"
...
YAML
  ...
  cp "$LIB_OPS"  "$sb/public/.apexyard/hooks/_lib-ops-root.sh"
  cp "$LIB_PORT" "$sb/public/.apexyard/hooks/_lib-portfolio-paths.sh"
  cp "$LIB_CFG"  "$sb/public/.apexyard/hooks/_lib-read-config.sh"
  cp "$DEFAULTS" "$sb/public/.apexyard/project-config.defaults.json"

  cat > "$sb/public/.apexyard/project-config.json" <<JSON
{
  "portfolio": {
...
build_v2_split() {
  local sb="$1"
  mkdir -p "$sb/public/.apexyard/hooks"
...
build_single_fork() {
  local sb="$1"
  mkdir -p "$sb/.apexyard/hooks" "$sb/projects" "$sb/workspace"
...
MD
  ...
  cp "$LIB_OPS"  "$sb/.apexyard/hooks/_lib-ops-root.sh"
  cp "$LIB_PORT" "$sb/.apexyard/hooks/_lib-portfolio-paths.sh"
...
detect_v2_needed() {
  ...
  (
    ...
    . .apexyard/hooks/_lib-read-config.sh
    # shellcheck source=/dev/null
    . .apexyard/hooks/_lib-portfolio-paths.sh
    portfolio_clear_cache

    if portfolio_is_v2; then
      return 1
    fi
    if ! jq -e '.portfolio.registry' .apexyard/project-config.json >/dev/null 2>&1; then
      return 1
    fi
    return 0
  )
...
snapshot_state() {
  ...
  (
    cd "$fork" || return 1
    {
      ...
      for path in onboarding.yaml .gitignore .apexyard-fork .apexyard/project-config.json; do
        if [ -e "$path" ]; then
...
echo '{"_comment": "no portfolio block"}' > "$SB/.apexyard/project-config.json"
...
ONB_KEY=$(jq -r '.portfolio.onboarding // empty' "$SB/public/.apexyard/project-config.json")
WS_KEY=$(jq -r '.portfolio.workspace_dir // empty' "$SB/public/.apexyard/project-config.json")
[ -n "$ONB_KEY" ] && [ -n "$WS_KEY" ] \
...
SIBLING_TEST="$ROOT/.apexyard/hooks/tests/test_split_portfolio_v2_migration.sh"
