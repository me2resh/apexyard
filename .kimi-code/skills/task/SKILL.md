...
echo "task" > "$ops_root/.apexyard/session/active-issue-skill"
```
...
The title prefix is derived from the content, and must come from the project's configured prefix whitelist (`.apexyard/project-config.*.json` → `.ticket.prefix_whitelist`, default list at `.apexyard/project-config.defaults.json`). Shipped defaults map as follows:
