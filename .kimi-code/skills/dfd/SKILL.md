...
Read the registry path via `portfolio_registry`, the per-project docs dir via `portfolio_projects_dir`, and the workspace dir via `portfolio_workspace_dir` from `.apexyard/hooks/_lib-portfolio-paths.sh`. Cross-repo discovery uses the shared `_lib-multi-repo-trace.sh` helper. Source both at the top of any bash block:
...
source "$(git rev-parse --show-toplevel)/.apexyard/hooks/_lib-multi-repo-trace.sh"
projects_dir=$(portfolio_projects_dir)
```

Defaults match today's single-fork layout. Adopters in split-portfolio mode override the `portfolio.*` keys in `.apexyard/project-config.json` — the helper resolves whichever mode they're in. See `docs/multi-project.md`.
...
bash .apexyard/skills/dfd/discover.sh "$target_dir" "$scope_hint"   > "$discovery_yaml"
bash .apexyard/skills/dfd/classify.sh "$target_dir"                 > "$classifications_yaml"

bash .apexyard/skills/dfd/generate-mermaid.sh "$PROJECT" "$discovery_yaml" "$classifications_yaml" \
...
echo "$model_json" \
  | bash .apexyard/skills/dfd/generate-dragon.sh \
