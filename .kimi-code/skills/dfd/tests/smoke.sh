...
SB="$TMPROOT/sandbox"
mkdir -p "$SB/.apexyard/hooks" "$SB/projects" "$SB/workspace/billing-api/src" "$SB/workspace/notifications-svc/src"
...
cp "$HOOKS_DIR/_lib-multi-repo-trace.sh" "$SB/.apexyard/hooks/"
cp "$HOOKS_DIR/_lib-portfolio-paths.sh"  "$SB/.apexyard/hooks/"
cp "$HOOKS_DIR/_lib-read-config.sh"      "$SB/.apexyard/hooks/"
DEFAULTS_SRC="$(cd "$DFD_DIR/../.." && pwd)/project-config.defaults.json"
cp "$DEFAULTS_SRC" "$SB/.apexyard/project-config.defaults.json" 2>/dev/null || true
...
RESULT=$(cd "$SB" && bash -c '
  ...
  source ./.apexyard/hooks/_lib-multi-repo-trace.sh
