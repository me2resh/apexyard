...
```bash
bash .apexyard/skills/dfd/tests/smoke.sh
```
...
bash .apexyard/skills/dfd/discover.sh /path/to/project [scope-hint]

# Classification heuristics (axis 6)
bash .apexyard/skills/dfd/classify.sh /path/to/project

# Mermaid markdown generator
bash .apexyard/skills/dfd/generate-mermaid.sh <project-name> [discovery.yaml] [classifications.yaml]

# Threat Dragon JSON serialiser
echo "$MODEL_JSON" | bash .apexyard/skills/dfd/generate-dragon.sh
```
