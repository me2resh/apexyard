...
# `.apexyard/project-config.json` → `.git.protected_branches[]`.
#
...
All changes must go through a PR (.apexyard/rules/git-conventions.md
...
Customise (rare): .apexyard/project-config.json → .git.protected_branches[]
