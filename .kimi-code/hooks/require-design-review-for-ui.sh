...
# .apexyard/session/reviews/<pr>-design.approved (with a matching HEAD SHA) before
...
#
# Enforces .apexyard/rules/pr-quality.md § "Design Review" and
...
if [ -n "$REPO_ROOT" ] && [ -f "${REPO_ROOT}/.apexyard/project-config.json" ]; then
  CUSTOM=$(jq -r '.ui_paths // [] | join("|")' "${REPO_ROOT}/.apexyard/project-config.json" 2>/dev/null)
...
if [ -n "$REPO_ROOT" ] && [ -f "${REPO_ROOT}/.apexyard/project-config.json" ]; then
  EXCLUDE=$(jq -r '.ui_paths_exclude // [] | join("|")' "${REPO_ROOT}/.apexyard/project-config.json" 2>/dev/null)
...
see .apexyard/rules/pr-quality.md § "Design Review (UI Changes)" and
...
To unblock:
  ...
  2. When the designer approves, record it with the current HEAD SHA:
       mkdir -p .apexyard/session/reviews
       git rev-parse HEAD > .apexyard/session/reviews/${PR_NUMBER}-design.approved
  3. Retry the merge
...
Both keys live in .apexyard/project-config.json.
