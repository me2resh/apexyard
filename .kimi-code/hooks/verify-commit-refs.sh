...
#
# Backstop for the ticket-vocabulary rule (.apexyard/rules/ticket-vocabulary.md).
...
#   1. .apexyard/project-config.json `.tracker_repo`
...
#   - CONFIG_ROOT: the ops fork holding .apexyard/project-config.json. When
...
TRACKER_REPO=""
if [ -n "$CONFIG_ROOT" ] && [ -f "${CONFIG_ROOT}/.apexyard/project-config.json" ]; then
...
real issue behind them. See .apexyard/rules/ticket-vocabulary.md.
