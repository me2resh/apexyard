...
# .apexyard/agents/*.md frontmatter in-place.
#
...
#       1. Locate .apexyard/agents/<name>.md — silently skip if missing
#          (adopter may have a stale entry; harmless, not an error)
#       2. Snapshot the framework default model: line (from HEAD of dev
#          for the file) into .apexyard/agents/.framework-defaults.json
...
#       4. If endpoint: set AND reachable, write to .apexyard/session/
...
#   - Write session-wide .apexyard/session/agent-env/__session__.env with
...
elif [ -f "$ROOT/.apexyard/hooks/_lib-portfolio-paths.sh" ] && [ -f "$ROOT/.apexyard/hooks/_lib-read-config.sh" ]; then
  LIB_SRC_DIR="$ROOT/.apexyard/hooks"
else
  exit 0
fi
# shellcheck source=/dev/null
...
ENV_DIR="$ROOT/.apexyard/session/agent-env"
mkdir -p "$ENV_DIR"
...
# allowed-tools list lives in .apexyard/agents/<name>.md frontmatter.)
...
snapshot_framework_default() {
  ...
  if git -C "$ROOT" rev-parse --verify dev >/dev/null 2>&1; then
    default_model=$(git -C "$ROOT" show "dev:.apexyard/agents/${agent_name}.md" 2>/dev/null \
