...
# from `.apexyard/project-config.defaults.json` → `ticket.create_command_patterns`
# (adopters extend via `.apexyard/project-config.json` shallow-merge).
#
# Resolution:
#   1. If the command does not match any configured pattern → exit 0 (no-op).
#   2. If the bootstrap-skill marker (.apexyard/session/active-bootstrap) is
...
#   3. If the ticket-skill marker (.apexyard/session/active-issue-skill) is
...
SKILL_MARKER="$MARKER_HOME/.apexyard/session/active-issue-skill"
...
custom), add patterns under .apexyard/project-config.json →
