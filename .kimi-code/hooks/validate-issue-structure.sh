...
#
# Schema source:
#   .apexyard/project-config.defaults.json → .ticket.required_sections (map)
#   .apexyard/project-config.json          → user overrides (shallow-merged)
...
# shipped .apexyard/project-config.defaults.json so behaviour is identical.
...
Accepted prefixes (from .apexyard/project-config.*.json → .ticket.prefix_whitelist):
  ${ACCEPTED}

Either fix the bracketed prefix in the title, or extend prefix_whitelist in
.apexyard/project-config.json if this team actually uses a new category.
...
Schema source: .apexyard/project-config.*.json → .ticket.required_sections.
