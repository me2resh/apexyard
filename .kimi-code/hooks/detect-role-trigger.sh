...
# .apexyard/rules/role-triggers.md and emits a system-reminder-style banner
...
#
# 1:1 by default — `roles/<dept>/<slug>.md` → `.apexyard/agents/<slug>.md`.
...
emit_banner() {
  ...
  if [ "$class" = "isolated-work-class" ]; then
    printf 'ROLE TRIGGER: %s activates per .apexyard/rules/role-triggers.md (%s). Isolated-work-class — SPAWN the sub-agent via the Agent tool with subagent_type: %s. Canonical role at %s, agent wrapper at .apexyard/agents/%s.md. Per AgDR-0050 § Axis 6.\n' \
      "$role" "$reason" "$agent_slug" "$file" "$agent_slug" >&2
  else
    printf 'ROLE TRIGGER: %s activates per .apexyard/rules/role-triggers.md (%s). In-flow-class — adopt the persona IN-THREAD. Read %s before continuing. Per AgDR-0050 § Axis 6.\n' \
      "$role" "$reason" "$file" >&2
  fi
}
...
# Activation Table in .apexyard/rules/role-triggers.md.
# ---------------------------------------------------------------------------
ROLES_TABLE='
