...
# via the public fork's .apexyard/skills/ discovery path.
#
# Implementation: gitignored symlinks at .apexyard/skills/<name>/ pointing to
# <private>/custom-skills/<name>/. Claude Code's skill discovery walks
# .apexyard/skills/*/SKILL.md, so the symlink makes the private skill
...
#     contains SKILL.md, ensure a symlink exists at .apexyard/skills/<name>.
#       * If a framework skill of the same name exists at .apexyard/skills/<name>
...
LIB_PORTFOLIO="$ops_root/.apexyard/hooks/_lib-portfolio-paths.sh"
LIB_CONFIG="$ops_root/.apexyard/hooks/_lib-read-config.sh"
...
if [ "$IS_WINDOWS" -eq 1 ]; then
  ...
  echo "  Manual workaround: copy each subdir of custom-skills/ into .apexyard/skills/ on this machine." >&2
  echo "  See docs/multi-project.md § 'Private custom skills + handbooks' for the full pointer." >&2
  exit 0
fi

skills_target_dir="$ops_root/.apexyard/skills"
...
if [ "$linked_count" -gt 0 ]; then
  echo "ApexYard: linked $linked_count custom skill(s) from $custom_skills_dir into .apexyard/skills/." >&2
fi
