...
#
# Many framework hooks call `jq` to read `.apexyard/project-config.json`
# and `.apexyard/project-config.defaults.json` for adopter overrides
...
if [ ! -f "$ops_root/.apexyard/project-config.json" ] && \
   [ ! -f "$ops_root/.apexyard/project-config.defaults.json" ]; then
  exit 0
fi
