...
# resolved `.apexyard/project-config.json` via `git rev-parse --show-toplevel`.
...
YAML
    mkdir -p .apexyard/hooks/tests
    cp "$TRACKER_LIB"        .apexyard/hooks/_lib-tracker.sh
    cp "$CONFIG_LIB"         .apexyard/hooks/_lib-read-config.sh
    cp "$OPS_ROOT_LIB"       .apexyard/hooks/_lib-ops-root.sh
    cp "$PR_CREATE_HOOK"     .apexyard/hooks/validate-pr-create.sh
    cp "$COMMIT_REFS_HOOK"   .apexyard/hooks/verify-commit-refs.sh
    cp "$SKILL_GATE_HOOK"    .apexyard/hooks/require-skill-for-issue-create.sh
    [ -f "$EXTRACT_PUSH_REF" ]   && cp "$EXTRACT_PUSH_REF"   .apexyard/hooks/
    [ -f "$DETECT_BASH_WRITE" ] && cp "$DETECT_BASH_WRITE" .apexyard/hooks/
    chmod +x .apexyard/hooks/*.sh
...
case_1() {
  ...
  out=$(
    ...
    . "$sb/.apexyard/hooks/_lib-read-config.sh"
...
case_2() {
  ...
  out=$(
    cd "$sb/workspace/test-project" || exit 99
    . "$sb/.apexyard/hooks/_lib-read-config.sh"
    . "$sb/.apexyard/hooks/_lib-tracker.sh"
    tracker_clear_cache
    tracker_kind
  )
  if [ "$out" = "jira" ]; then
...
case_3() {
  ...
  rc=$(
    cd "$sb/workspace/test-project" || exit 99
    PATH="$sb/bin:$PATH" "$sb/.apexyard/hooks/verify-commit-refs.sh" <<<"$input" >/dev/null 2>&1
    echo $?
  )
...
" --head feature/PROJ-42-jira-shape'
  ...
  rc=$(
    cd "$sb/workspace/test-project" || exit 99
    PATH="$sb/bin:$PATH" "$sb/.apexyard/hooks/validate-pr-create.sh" <<<"$input" >/dev/null 2>&1
    echo $?
  )
...
case_5() {
  ...
  rc=$(
    cd "$sb/workspace/test-project" || exit 99
    PATH="$sb/bin:$PATH" "$sb/.apexyard/hooks/require-skill-for-issue-create.sh" <<<"$input" >/dev/null 2>&1
    echo $?
  )
  if [ "$rc" = "2" ]; then
...
case_7() {
  ...
  # .apexyard/project-config.json at its root — that's what we expect the
