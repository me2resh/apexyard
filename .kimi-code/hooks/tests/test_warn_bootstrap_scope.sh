#!/bin/bash
# Smoke tests for .apexyard/hooks/warn-bootstrap-scope.sh — verifies the
...
HOOK="$SRC_ROOT/.apexyard/hooks/warn-bootstrap-scope.sh"
...
# Path the hook will look for: .apexyard/session/active-bootstrap under
...
mkdir -p "$FAKE_ROOT/.git" "$FAKE_ROOT/.apexyard/session"
...
MARKER="$FAKE_ROOT/.apexyard/session/active-bootstrap"
