...
HOOK_SRC="$SRC_ROOT/.apexyard/hooks/require-skill-for-issue-create.sh"
...
make_sandbox() {
  ...
  cp "$HOOK_SRC" "$sb/.apexyard/hooks/require-skill-for-issue-create.sh"
  [ "$HAVE_LIB_OPS" = "1" ] && cp "$LIB_OPS" "$sb/.apexyard/hooks/_lib-ops-root.sh"
  cp "$LIB_CFG" "$sb/.apexyard/hooks/_lib-read-config.sh"
  cp "$DEFAULTS" "$sb/.apexyard/project-config.defaults.json"
  chmod +x "$sb/.apexyard/hooks/require-skill-for-issue-create.sh"
  echo "$sb"
}
...
run_case() {
  ...
  if [ -n "$env_var" ]; then
    got_stderr=$(cd "$sb" && echo "$input" | env "$env_var" bash .apexyard/hooks/require-skill-for-issue-create.sh 2>&1 >/dev/null)
  else
    got_stderr=$(cd "$sb" && echo "$input" | bash .apexyard/hooks/require-skill-for-issue-create.sh 2>&1 >/dev/null)
  fi
  got_rc=$?
  rm -rf "$sb"
...
sb=$(make_sandbox); echo "feature" > "$sb/.apexyard/session/active-issue-skill"
...
sb=$(make_sandbox); echo "task" > "$sb/.apexyard/session/active-issue-skill"
...
sb=$(make_sandbox); echo "bug" > "$sb/.apexyard/session/active-issue-skill"
...
sb=$(make_sandbox); echo "handover" > "$sb/.apexyard/session/active-bootstrap"
in=$(jq -nc --arg c "gh issue create --repo foo/bar --title x" '{tool_name:"Bash", tool_input:{command:$c}}')
run_case "gh issue create allowed during handover bootstrap" 0 "" "$in" "$sb"

sb=$(make_sandbox); echo "setup" > "$sb/.apexyard/session/active-bootstrap"
in=$(jq -nc --arg c "gh issue create --repo foo/bar --title x" '{tool_name:"Bash", tool_input:{command:$c}}')
...
sb=$(make_sandbox); echo "some-random-skill" > "$sb/.apexyard/session/active-bootstrap"
in=$(jq -nc --arg c "gh issue create --repo foo/bar --title x" '{tool_name:"Bash", tool_input:{command:$c}}')
...
sb=$(make_sandbox); : > "$sb/.apexyard/session/active-issue-skill"
in=$(jq -nc --arg c "gh issue create --repo foo/bar --title x" '{tool_name:"Bash", tool_input:{command:$c}}')
