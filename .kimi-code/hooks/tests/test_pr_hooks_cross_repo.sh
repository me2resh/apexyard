...
#     framework paths (.apexyard/hooks/, topologies/, handbooks/domain/) that are
...
#
# To run:  ./.apexyard/hooks/tests/test_pr_hooks_cross_repo.sh
...
AGDR_HOOK="$REPO_ROOT/.apexyard/hooks/require-agdr-for-arch-pr.sh"
VALIDATE_HOOK="$REPO_ROOT/.apexyard/hooks/validate-pr-create.sh"
...
make_validate_sandbox() {
  ...
  mkdir -p "$sb/.apexyard/hooks"
  cp "$VALIDATE_HOOK" "$sb/.apexyard/hooks/validate-pr-create.sh"
  chmod +x "$sb/.apexyard/hooks/validate-pr-create.sh"
  for lib in \
    _lib-read-config.sh \
    _lib-tracker.sh \
    _lib-ops-root.sh \
    _lib-pr-repo.sh; do
    [ -f "$REPO_ROOT/.apexyard/hooks/$lib" ] && cp "$REPO_ROOT/.apexyard/hooks/$lib" "$sb/.apexyard/hooks/$lib"
  done
  [ -f "$REPO_ROOT/.apexyard/project-config.defaults.json" ] && \
    cp "$REPO_ROOT/.apexyard/project-config.defaults.json" "$sb/.apexyard/project-config.defaults.json"
  echo "$sb"
}
...
STDERR=$(cd "$SB" && echo "$INPUT" | bash .apexyard/hooks/validate-pr-create.sh 2>&1 >/dev/null)
RC=$?
rm -rf "$SB"
...
# THE SANDBOX, never touching $REPO_ROOT/.apexyard/hooks/ (the live hooks dir).
...
SB_C1=$(mktemp -d)
mkdir -p "$SB_C1/.apexyard/hooks"
cp "$AGDR_HOOK" "$SB_C1/.apexyard/hooks/require-agdr-for-arch-pr.sh"
chmod +x "$SB_C1/.apexyard/hooks/require-agdr-for-arch-pr.sh"
for lib in \
  _lib-read-config.sh \
  _lib-ops-root.sh \
  _lib-pr-repo.sh; do
  [ -f "$REPO_ROOT/.apexyard/hooks/$lib" ] && cp "$REPO_ROOT/.apexyard/hooks/$lib" "$SB_C1/.apexyard/hooks/$lib"
done
[ -f "$REPO_ROOT/.apexyard/project-config.defaults.json" ] && \
  cp "$REPO_ROOT/.apexyard/project-config.defaults.json" "$SB_C1/.apexyard/project-config.defaults.json"
# Remove the lib from the SANDBOX COPY (not the live hooks dir).
rm -f "$SB_C1/.apexyard/hooks/_lib-pr-repo.sh"
AGDR_HOOK_C1="$SB_C1/.apexyard/hooks/require-agdr-for-arch-pr.sh"
STDERR_C1=$( cd "$DIR_C1" && \
...
rm -f "$SB_C2/.apexyard/hooks/_lib-pr-repo.sh"
STDERR_C2=$(cd "$SB_C2" && echo "$INPUT_C2" | bash .apexyard/hooks/validate-pr-create.sh 2>&1 >/dev/null)
RC_C2=$?
rm -rf "$SB_C2"
