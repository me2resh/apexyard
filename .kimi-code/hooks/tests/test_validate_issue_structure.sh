...
#
# Run:
#   ./.apexyard/hooks/tests/test_validate_issue_structure.sh
#
...
HOOK="$REPO_ROOT/.apexyard/hooks/validate-issue-structure.sh"
...
mkdir -p "$TMPDIR/fork/.apexyard/hooks" "$TMPDIR/fork/subdir"
...
cp "$REPO_ROOT/.apexyard/project-config.defaults.json" "$TMPDIR/fork/.apexyard/project-config.defaults.json"
cp "$REPO_ROOT/.apexyard/hooks/_lib-read-config.sh" "$TMPDIR/fork/.apexyard/hooks/_lib-read-config.sh"
