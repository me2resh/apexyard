...
BRANCH_HOOK="$SRC_ROOT/.apexyard/hooks/validate-branch-name.sh"
COMMIT_HOOK="$SRC_ROOT/.apexyard/hooks/validate-commit-format.sh"
PR_HOOK="$SRC_ROOT/.apexyard/hooks/validate-pr-create.sh"
LIB_CFG="$SRC_ROOT/.apexyard/hooks/_lib-read-config.sh"
LIB_PUSHREF="$SRC_ROOT/.apexyard/hooks/_lib-extract-push-ref.sh"
...
make_sandbox() {
  ...
  cp "$BRANCH_HOOK" "$sb/.apexyard/hooks/validate-branch-name.sh"
  cp "$COMMIT_HOOK" "$sb/.apexyard/hooks/validate-commit-format.sh"
  [ -f "$LIB_CFG" ]     && cp "$LIB_CFG"     "$sb/.apexyard/hooks/_lib-read-config.sh"
  [ -f "$LIB_PUSHREF" ] && cp "$LIB_PUSHREF" "$sb/.apexyard/hooks/_lib-extract-push-ref.sh"
  [ -f "$DEFAULTS" ]    && cp "$DEFAULTS"    "$sb/.apexyard/project-config.defaults.json"
  chmod +x "$sb/.apexyard/hooks/"*.sh
  echo "$sb"
}
...
run_hook_case() {
  ...
  got_stderr=$(cd "$sb" && echo "$input" | bash ".apexyard/hooks/$hook" 2>&1 >/dev/null)
  got_rc=$?
  rm -rf "$sb"
