...
#
# To run:  ./.apexyard/hooks/tests/test_require_agdr_for_arch_pr.sh
...
HOOK="$REPO_ROOT/.apexyard/hooks/require-agdr-for-arch-pr.sh"
...
projects: []" > apexyard.projects.yaml
    mkdir -p .apexyard/session
    cat > .apexyard/session/current-ticket <<'EOF'
repo=test/repo
number=180
