...
make_sandbox() {
  ...
  if [ -f "$src_root/.apexyard/hooks/_lib-tracker.sh" ]; then
    cp "$src_root/.apexyard/hooks/_lib-tracker.sh" "$sb/.apexyard/hooks/_lib-tracker.sh"
  fi
...
run_case() {
  ...
  if [ -n "$extra_config" ]; then
    echo "$extra_config" > "$sb/.apexyard/project-config.json"
  fi
