...
HOOK_SRC="$SRC_ROOT/.apexyard/hooks/validate-commit-format.sh"
...
make_sandbox() {
  ...
  cp "$HOOK_SRC" "$sb/.apexyard/hooks/validate-commit-format.sh"
  ...
  chmod +x "$sb/.apexyard/hooks/validate-commit-format.sh"
  echo "$sb"
}
...
run_case() {
  ...
  got_stderr=$(cd "$sb" && echo "$input" | bash .apexyard/hooks/validate-commit-format.sh 2>&1 >/dev/null)
  got_rc=$?
  rm -rf "$sb"
