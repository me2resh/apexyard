...
#
# Usage: bash .apexyard/hooks/tests/test_token_efficiency_wave1.sh
...
SKILLS_DIR="$ROOT/.apexyard/skills"
HOOKS_DIR="$ROOT/.apexyard/hooks"
...
# -----------------------------------------------------------------------------
echo "== Invariant 3: every skill in .apexyard/skills/ is catalogued in CLAUDE.md"
missing=0
for d in "$SKILLS_DIR"/*/; do
  name=$(basename "$d")
  case "$name" in _lib*) continue ;; esac
  if ! grep -qE "^\| \`/$name\` \|" "$CLAUDE_MD"; then
    red "  FAIL: /$name is in .apexyard/skills/ but not in CLAUDE.md skill table"
    missing=$((missing + 1))
    FAIL=$((FAIL + 1))
  fi
done
