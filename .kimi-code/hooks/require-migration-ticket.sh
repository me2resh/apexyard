...
#
# Enforces `.apexyard/rules/workflow-gates.md` gate 4a (migration) —
...
#   - FILE_PATH is under .apexyard/, docs/, projects/*/docs/, any *.md
...
#   }
#
# The config is read from `<ops_root>/.apexyard/project-config.json` if
...
case "$FILE_PATH" in
  */.apexyard/*|*/.claude|*/docs/*|*/docs) exit 0 ;;
  *.md|*.example) exit 0 ;;
esac
...
CUSTOM_PATHS=""
PCONFIG="$MARKER_HOME/.apexyard/project-config.json"
...
if [ -n "$PROJECT" ]; then
  ...
  if [ -n "$WT_BRANCH" ]; then
    ...
    if [ -f "$MARKER_HOME/.apexyard/session/tickets/$PROJECT/$SAFE_BRANCH" ]; then
      MARKER="$MARKER_HOME/.apexyard/session/tickets/$PROJECT/$SAFE_BRANCH"
    fi
  fi
fi
if [ -z "$MARKER" ] && [ -n "$PROJECT" ] && [ -f "$MARKER_HOME/.apexyard/session/tickets/$PROJECT" ]; then
  MARKER="$MARKER_HOME/.apexyard/session/tickets/$PROJECT"
elif [ -z "$MARKER" ] && [ -f "$MARKER_HOME/.apexyard/session/current-ticket" ]; then
  MARKER="$MARKER_HOME/.apexyard/session/current-ticket"
fi
