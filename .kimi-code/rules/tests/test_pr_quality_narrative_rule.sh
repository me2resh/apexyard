...
#
#   - .apexyard/rules/pr-quality.md       — the rule + bad/good example pair
#   - .apexyard/agents/code-reviewer.md   — Rex's advisory check + heuristic
...
#
# Style matches the hook tests (.apexyard/hooks/tests/*.sh) — plain bash + grep,
...
RULE_FILE="$SRC_ROOT/.apexyard/rules/pr-quality.md"
AGENT_FILE="$SRC_ROOT/.apexyard/agents/code-reviewer.md"
...
assert "workflow:cross-link-rule-path" \
  grep -qF '.apexyard/rules/pr-quality.md' "$WORKFLOW_FILE"
