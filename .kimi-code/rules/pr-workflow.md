...
- Write any file under `.apexyard/session/reviews/`, including `*-rex.approved` or `*-ceo.approved`
...
1. `warn-review-marker-write.sh` — PreToolUse advisory (exit 0, never blocks) that fires when a Write or Bash call targets `*-rex.approved` or `*-ceo.approved` under `.apexyard/session/reviews/`, reminding that markers must come from the real reviewer or `/approve-merge`.
...
The `block-unreviewed-merge.sh` hook enforces this rule at the shell level. It requires **two** approval markers in `.apexyard/session/reviews/` before letting any merge command through:
...
Historically only the first shape was matched. In April 2026 (incident: `me2resh/curios-dog#190` was merged via `gh api` while CI was still running), the second shape was discovered as a silent bypass and closed in [#47](https://github.com/me2resh/apexyard/issues/47). Both the matcher entries in `.apexyard/settings.json` and the PR-number extraction in each hook (`.apexyard/hooks/_lib-extract-pr.sh`) now recognise both shapes. Invoking either triggers the gate — there is no supported merge path that skips the two-reviews rule.
