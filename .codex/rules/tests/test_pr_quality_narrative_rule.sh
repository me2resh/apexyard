...
#
#   - .codex/rules/pr-quality.md       — the rule + bad/good example pair
#   - .codex/agents/code-reviewer.toml   — Rex's advisory check + heuristic
...
#
# Style matches the hook tests (.codex/hooks/tests/*.sh) — plain bash + grep,
...
RULE_FILE="$SRC_ROOT/.codex/rules/pr-quality.md"
AGENT_FILE="$SRC_ROOT/.codex/agents/code-reviewer.toml"
...
assert "workflow:cross-link-rule-path" \
  grep -qF '.codex/rules/pr-quality.md' "$WORKFLOW_FILE"
