...
Rex's advisory check (see `.codex/agents/code-reviewer.toml`) flags label-only bullets as a `nit:` / `suggestion:` finding, not a blocking verdict, so legitimate short bullets don't churn the merge gate.
...
If the PR touches user-facing UI → design review is required before merge. Mechanically enforced by `require-design-review-for-ui.sh` (blocks merge without a design marker) and the `/approve-design <pr>` skill (writes the marker on explicit designer approval). See `.agents/skills/approve-design/SKILL.md` for the full invocation rules.

**Customising what counts as "UI"** (`.codex/project-config.json`):
