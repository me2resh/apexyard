...
Codex ships **plan mode** — a harness state where the agent thinks through a multi-step approach and presents it for user approval *before* executing any tool calls that mutate state. This rule is the **trigger heuristic** — it defines when an agent should proactively enter plan mode instead of jumping straight into tool calls.
...
Anthropic's `opusplan` model alias is a related prior-art pattern: Opus runs during plan mode (deeper reasoning while the cost matters), Sonnet runs during execution (cheaper while the choices are mechanical). See [Codex model configuration](https://code.claude.com/docs/en/model-config) for the alias and tier-routing semantics. The `opusplan` shape implies plan mode is meant as a *deliberate, more-expensive thinking phase*, not a default-on ceremony — which lines up with the "when NOT to enter" list above.
