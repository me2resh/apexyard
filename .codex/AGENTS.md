...
This file is generated by bin/sync.ts. Codex reads project instructions
from `AGENTS.md` at the project root.

## Defaults

- Default model: `gpt-5.5` (Codex only ships OpenAI providers; the
  OpenCode free-tier model `opencode/minimax-m3-free` is not available
  in Codex. See INSTRUCTIONS.md for the tiered model strategy.)

## Source of truth

The canonical instructions live in `INSTRUCTIONS.md` at the repo root.
That file is harness-agnostic; this `AGENTS.md` is a Codex-specific
projection. If you change `INSTRUCTIONS.md`, run `bun run sync` to
