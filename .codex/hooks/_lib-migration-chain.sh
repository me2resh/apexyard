...
#
# Version anchor lives at <ops_fork_root>/.codex/framework-version.
...
migration_anchor_path() {
  ...
  [ -n "$root" ] || return 1
  echo "$root/.codex/framework-version"
}
...
# attached. Source of truth is the filenames in .codex/migrations/.
...
migration_known_versions() {
  ...
  root=$(_migration_ops_root)
  [ -n "$root" ] || return 1
  local dir="$root/.codex/migrations"
  [ -d "$dir" ] || return 0
...
migration_script_path() {
  ...
  root=$(_migration_ops_root)
  [ -n "$root" ] || return 1
  echo "$root/.codex/migrations/${pair}.sh"
}
