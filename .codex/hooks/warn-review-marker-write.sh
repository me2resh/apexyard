...
# be locked out).
#
# Wired in .codex/settings.json PreToolUse for:
#   matcher: Write    (catches direct file writes)
#   matcher: Bash     (catches shell redirections, printf, tee, etc.)
#
# See AgDR-0062 and .codex/rules/pr-workflow.md § "Build agents cannot
...
_is_marker_target() {
  ...
  # under a .codex/session/reviews/ directory.
  echo "$text" | grep -qE '\.codex/session/reviews/[^[:space:]"'"'"']+-(rex|ceo)\.approved'
}
...
if [ "$MATCHED" = "1" ]; then
  cat >&2 <<'BANNER'
[apexyard] ADVISORY: You are about to write a review marker under .codex/session/reviews/.
