...
#
# The accepted type list is project-configurable via .codex/project-config.json
# (.pr.title_type_whitelist). Defaults ship at .codex/project-config.defaults.json.
# See apexyard#109.
#
# Resolve the directory holding the config:
#   - HOOK_DIR points at the lib files (always next to this script).
#   - CONFIG_ROOT is the ops fork (where .codex/project-config.json lives).
...
if [ -n "$TITLE" ]; then
  if ! echo "$TITLE" | grep -qE "^(${PR_TYPES})\(([A-Z]{2,10}-[0-9]+|#[0-9]+)\)!?:"; then
    ...
    ERRORS="${ERRORS}Accepted types (from .codex/project-config.*.json → .pr.title_type_whitelist): ${PR_TYPES//|/, }\n"
  else
...
if [ -n "$TICKET_REF" ]; then
  ...
  elif [ -n "$CONFIG_ROOT" ] && [ -f "${CONFIG_ROOT}/.codex/project-config.json" ]; then
    TRACKER_REPO=$(jq -r '.tracker_repo // empty' "${CONFIG_ROOT}/.codex/project-config.json" 2>/dev/null)
  fi
...
See .codex/rules/ticket-vocabulary.md § "The rule".
...
See .codex/rules/ticket-vocabulary.md and the "every PR needs its own open
...
# .codex/project-config.*.json (`.pr.required_sections`). Shipped default
  ...
  if [ "$ALLOW_MULTI_CLOSES" != "true" ]; then
    ...
    if [ "$CLOSE_COUNT" -gt 1 ]; then
      ...
      if printf '%s\n' "$STRIPPED_BODY" | grep -qF -- "$MULTI_CLOSE_SKIP"; then
        ...
        ERRORS="${ERRORS}PR body has $CLOSE_COUNT distinct closing references (${NUMS_LIST}) — one ticket per PR (see AGENTS.md). If this really is an umbrella PR, add the skip marker: $MULTI_CLOSE_SKIP\n"
      fi
    fi
  fi
fi
...
if [ -n "$ERRORS" ]; then
  ...
  echo "See .codex/rules/git-conventions.md and .codex/rules/pr-quality.md." >&2
  exit 2
fi
