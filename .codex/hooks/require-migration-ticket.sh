...
#
# Enforces `.codex/rules/workflow-gates.md` gate 4a (migration) —
...
#   - FILE_PATH is under .codex/, docs/, projects/*/docs/, any *.md
...
#   }
#
# The config is read from `<ops_root>/.codex/project-config.json` if
...
CUSTOM_PATHS=""
PCONFIG="$MARKER_HOME/.codex/project-config.json"
...
if [ -n "$PROJECT" ]; then
  ...
  if [ -n "$WT_BRANCH" ]; then
    ...
    if [ -f "$MARKER_HOME/.codex/session/tickets/$PROJECT/$SAFE_BRANCH" ]; then
      MARKER="$MARKER_HOME/.codex/session/tickets/$PROJECT/$SAFE_BRANCH"
    fi
  fi
fi
if [ -z "$MARKER" ] && [ -n "$PROJECT" ] && [ -f "$MARKER_HOME/.codex/session/tickets/$PROJECT" ]; then
  MARKER="$MARKER_HOME/.codex/session/tickets/$PROJECT"
elif [ -z "$MARKER" ] && [ -f "$MARKER_HOME/.codex/session/current-ticket" ]; then
  MARKER="$MARKER_HOME/.codex/session/current-ticket"
fi
