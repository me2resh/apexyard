...
# .codex/rules/role-triggers.md and emits a system-reminder-style banner
...
#   - docs/agdr/**                                    → Tech Lead
#
# The banner is written to stderr; Codex surfaces hook stderr to
...
#
# 1:1 by default — `roles/<dept>/<slug>.md` → `.codex/agents/<slug>.md`.
...
emit_banner() {
  ...
  if [ "$class" = "isolated-work-class" ]; then
    printf 'ROLE TRIGGER: %s activates per .codex/rules/role-triggers.md (%s). Isolated-work-class — SPAWN the sub-agent via the Agent tool with subagent_type: %s. Canonical role at %s, agent wrapper at .codex/agents/%s.md. Per AgDR-0050 § Axis 6.\n' \
      "$role" "$reason" "$agent_slug" "$file" "$agent_slug" >&2
  else
    printf 'ROLE TRIGGER: %s activates per .codex/rules/role-triggers.md (%s). In-flow-class — adopt the persona IN-THREAD. Read %s before continuing. Per AgDR-0050 § Axis 6.\n' \
      "$role" "$reason" "$file" >&2
  fi
}
...
# Activation Table in .codex/rules/role-triggers.md.
# ---------------------------------------------------------------------------
ROLES_TABLE='
