...
# .codex/agents/*.md frontmatter in-place.
#
...
#       4. If endpoint: set AND reachable, write to .codex/session/
#          agent-env/<name>.env (per-agent endpoint env file — informational
#          in v1 until Codex exposes per-agent env scoping)
#       5. If env: block set, append KEY=VALUE lines to that env file
#          (resolving $VAR_NAME refs against the parent env)
#   - Write session-wide .codex/session/agent-env/__session__.env with
...
elif [ -f "$ROOT/.codex/hooks/_lib-portfolio-paths.sh" ] && [ -f "$ROOT/.codex/hooks/_lib-read-config.sh" ]; then
  LIB_SRC_DIR="$ROOT/.codex/hooks"
else
  exit 0
fi
# shellcheck source=/dev/null
...
ENV_DIR="$ROOT/.codex/session/agent-env"
mkdir -p "$ENV_DIR"
...
extract_model_from_git() {
  ...
  model=$(git -C "$ROOT" show "${ref}:.codex/agents/${agent_name}.md" 2>/dev/null \
    | extract_model_from_stream md)
  if [ -n "$model" ]; then
    echo "$model"
    return 0
  fi
  model=$(git -C "$ROOT" show "${ref}:.codex/agents/${agent_name}.toml" 2>/dev/null \
...
# allowed-tools list lives in .codex/agents/<name>.md frontmatter.)
...
# routing mechanism that works in v1 — per AgDR-0050 § Axis 5, Codex
...
"
    ...
    timeout_seconds)
      # Recorded but not actively consumed in v1 — Codex doesn't
...
# § Axis 5, v1 supports one endpoint per session because Codex doesn't
...
"
  ...
  if [ "$REACHABLE_COUNT" -ge 1 ]; then
    ...
    # __session__.env is written above, but Codex's process env was set
    ...
    # in their shell profile BEFORE launching Codex, the routing won't
    ...
    if [ -z "$current_endpoint_env" ] || [ "$current_endpoint_env" != "$FIRST_EP" ]; then
      cat >&2 <<EOF
⚠ agent-routing: $SESSION_ENDPOINT_ENV_KEY=$FIRST_EP was written to $SESSION_ENV but is NOT set in this Claude session's process env. Routing is INACTIVE — every agent call still hits the default provider. To activate, add this line to your shell profile (~/.zshrc / ~/.bashrc) and relaunch Codex from a fresh terminal:
