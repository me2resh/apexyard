...
# v1 supports one endpoint per session because Codex doesn't consume
...
if [ "$REACHABLE_COUNT" -ge 1 ]; then
  ...
  if [ -z "${ANTHROPIC_BASE_URL:-}" ] || [ "${ANTHROPIC_BASE_URL:-}" != "$FIRST_EP" ]; then
    cat >&2 <<EOF
⚠ agent-routing: ANTHROPIC_BASE_URL=$FIRST_EP was written to $SESSION_ENV but is NOT set in this Claude session's process env. Routing is INACTIVE — every agent call still hits the Anthropic API. To activate, add this line to your shell profile (~/.zshrc / ~/.bashrc) and relaunch Codex from a fresh terminal:
