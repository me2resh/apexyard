...
# .codex/session/reviews/<pr>-architecture.approved (with a matching HEAD SHA)
...
#
# Enforces .codex/rules/workflow-gates.md § "Architecture Review Gate" and
...
if [ -n "$REPO_ROOT" ] && [ -f "${REPO_ROOT}/.codex/project-config.json" ]; then
  CUSTOM=$(jq -r '.design_paths // [] | join("|")' "${REPO_ROOT}/.codex/project-config.json" 2>/dev/null)
...
if [ -n "$REPO_ROOT" ] && [ -f "${REPO_ROOT}/.codex/project-config.json" ]; then
  EXCLUDE=$(jq -r '.design_paths_exclude // [] | join("|")' "${REPO_ROOT}/.codex/project-config.json" 2>/dev/null)
...
before the team builds against it. See .codex/rules/workflow-gates.md
