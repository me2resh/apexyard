...
#
# Enforces .codex/rules/agdr-decisions.md § "Pre-commit hook warns if
...
# .codex/project-config.json `.architecture_paths` (a JSON array of globs).
#
...
if [ -f "${REPO_ROOT}/.codex/project-config.json" ]; then
  CUSTOM=$(jq -r '.architecture_paths // [] | join("|")' "${REPO_ROOT}/.codex/project-config.json" 2>/dev/null)
...
#
# See .codex/rules/workflow-gates.md § Spike work.
# ---------------------------------------------------------------------------
spike_commit_exempt() {
...
if spike_commit_exempt; then
  echo "WARN: spike commit detected — require-agdr-for-arch-changes bypassed. AgDRs not required for hypothesis-driven throw-away work; ship a memo on /spike-close instead. See .codex/rules/workflow-gates.md § Spike work." >&2
  exit 0
fi
...
changes — see .codex/rules/agdr-decisions.md § Enforcement. Every
...
.codex/project-config.json \`.architecture_paths\` (JSON array of regex patterns).
MSG_END
exit 2
