...
#
# Closes the asymmetry noted in `.codex/rules/agdr-decisions.md`:
...
#   - Config via .codex/project-config.defaults.json (shallow-merged with
...
if echo "$HAYSTACK" | grep -qF -- "$SKIP_MARKER"; then
  echo "WARN: agdr: not-applicable marker present — require-agdr-for-arch-pr bypassed. See .codex/rules/agdr-decisions.md." >&2
  exit 0
fi
...
# surgical, not blanket. See .codex/rules/workflow-gates.md § Spike work.
# ---------------------------------------------------------------------------
spike_pr_exempt() {
  # (a) spike(...) PR title
  ...
  if [ -f "$marker_home/.codex/session/current-ticket" ]; then
    if grep -qE '^title=\[Spike\]' "$marker_home/.codex/session/current-ticket" 2>/dev/null; then
      return 0
    fi
  fi
  if [ -d "$marker_home/.codex/session/tickets" ]; then
    for marker in "$marker_home/.codex/session/tickets"/*; do
      [ -f "$marker" ] || continue
...
if spike_pr_exempt; then
  echo "WARN: spike PR detected — require-agdr-for-arch-pr bypassed. AgDRs not required for hypothesis-driven throw-away work; ship a memo on /spike-close instead. See .codex/rules/workflow-gates.md § Spike work." >&2
  exit 0
fi
...
if [ -f "$REPO_ROOT/.codex/hooks/_lib-read-config.sh" ]; then
...
# Fallback defaults — kept in sync with .codex/project-config.defaults.json.
...
Why this is blocked:
  .codex/rules/agdr-decisions.md calls /decide a HARD STOP before any
...
.codex/project-config.json via:
  - .agdr_trigger_paths[]     (shell globs)
  - .agdr_trigger_dep_files[] (literal filenames)
See .codex/project-config.defaults.json for the shipped defaults.
MSG
} >&2
