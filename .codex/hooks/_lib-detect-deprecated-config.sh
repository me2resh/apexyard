...
# top-level keys present in `.codex/project-config.json` (the adopter override)
# that are NOT present in `.codex/project-config.defaults.json` (the upstream-
...
detect_deprecated_config_keys() {
  ...
  if [ -z "$defaults" ] || [ -z "$overrides" ]; then
    local root
    root=$(_dep_repo_root)
    [ -z "$defaults" ] && defaults="$root/.codex/project-config.defaults.json"
    [ -z "$overrides" ] && overrides="$root/.codex/project-config.json"
  fi
