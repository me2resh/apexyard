...
# NOT merge approval" rule in .codex/rules/pr-workflow.md. An umbrella
...
ApexYard requires two reviews before merge (workflow-gates rule #5):
  1. Code Reviewer agent (Rex) — automated, recorded in .codex/session/reviews/
...
Never skip this check — even for typo fixes. See .codex/rules/pr-workflow.md.
MSG
  exit 2
fi
...
PR. See .codex/rules/pr-workflow.md § "Plan-level 'go' is NOT merge
