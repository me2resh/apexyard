#!/bin/bash
# _lib-read-config.sh — shared reader for .codex/project-config.*.json
#
# Source this library from any hook or skill that needs to read project config.
# Defaults ship at .codex/project-config.defaults.json (committed, upstream-
# maintained). User overrides live at .codex/project-config.json (optional;
...
# _config_repo_root: resolve the directory that holds .codex/project-config.*.
#
...
# .codex/project-config.json (or a different one), so config_get falls back
...
_config_repo_root() {
  ...
  # file in .codex/hooks/, so locate it via BASH_SOURCE — not via
...
_config_defaults_file() {
  ...
  [ -n "$root" ] && echo "$root/.codex/project-config.defaults.json"
}

_config_overrides_file() {
  local root
  root=$(_config_repo_root)
  [ -n "$root" ] && echo "$root/.codex/project-config.json"
}
