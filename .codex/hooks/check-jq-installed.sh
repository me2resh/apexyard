...
#
# Many framework hooks call `jq` to read `.codex/project-config.json`
# and `.codex/project-config.defaults.json` for adopter overrides
...
if [ ! -f "$ops_root/.codex/project-config.json" ] && \
   [ ! -f "$ops_root/.codex/project-config.defaults.json" ]; then
  exit 0
fi
