...
#
# Schema source:
#   .codex/project-config.defaults.json → .ticket.required_sections (map)
#   .codex/project-config.json          → user overrides (shallow-merged)
...
# shipped .codex/project-config.defaults.json so behaviour is identical.
...
Accepted prefixes (from .codex/project-config.*.json → .ticket.prefix_whitelist):
  ${ACCEPTED}

Either fix the bracketed prefix in the title, or extend prefix_whitelist in
.codex/project-config.json if this team actually uses a new category.
...
Schema source: .codex/project-config.*.json → .ticket.required_sections.
