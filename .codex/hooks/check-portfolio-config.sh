...
# adopter who mistyped a path in `.codex/project-config.json` (or moved
...
CONFIG_LIB="$ROOT/.codex/hooks/_lib-read-config.sh"
PORTFOLIO_LIB="$ROOT/.codex/hooks/_lib-portfolio-paths.sh"
...
ApexYard: portfolio config is broken — $result
  Fix .codex/project-config.json (key: portfolio.{registry, projects_dir, ideas_backlog}),
