...
# reads a list of shell commands from `.codex/project-config.*.json`
...
#
# This implements the HARD STOP documented in `.codex/rules/pr-workflow.md`
...
#   - No `.codex/project-config.defaults.json` AND no `package.json` in the
...
# Shipped defaults ship at .codex/project-config.defaults.json.
...
auditable. See .codex/rules/pr-workflow.md "Before git push (HARD STOP)".
MSG
  exit 2
fi
