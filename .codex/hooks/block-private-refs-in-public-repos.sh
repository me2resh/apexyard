...
#
# Configuration (future): a `.codex/project-config.json` may override
...
#    Sources (in order):
#      1. `.codex/project-config.*.json` → `leak_protection.public_framework_repos[]`
...
if [ -n "$REPO_ROOT_FOR_CONFIG" ] && [ -f "$REPO_ROOT_FOR_CONFIG/.codex/hooks/_lib-read-config.sh" ]; then
  # shellcheck disable=SC1090,SC1091
  . "$REPO_ROOT_FOR_CONFIG/.codex/hooks/_lib-read-config.sh"
...
if echo "$HAYSTACK" | grep -qF -- "$SKIP_MARKER"; then
  echo "WARN: private-refs: allow marker present — leak-protection hook bypassed for this ${TARGET_REPO} call. See .codex/rules/leak-protection.md." >&2
  exit 0
fi
...
#    by /start-ticket (see .agents/skills/start-ticket/SKILL.md) — we do
...
See .codex/rules/leak-protection.md for the full rationale.
MSG
