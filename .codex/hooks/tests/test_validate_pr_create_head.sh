...
HOOK_SRC="$SRC_ROOT/.codex/hooks/validate-pr-create.sh"
LIB_CFG="$SRC_ROOT/.codex/hooks/_lib-read-config.sh"
LIB_TRACKER="$SRC_ROOT/.codex/hooks/_lib-tracker.sh"
...
make_sandbox() {
  ...
  if [ -f "$LIB_TRACKER" ]; then cp "$LIB_TRACKER" "$sb/.codex/hooks/_lib-tracker.sh"; fi
