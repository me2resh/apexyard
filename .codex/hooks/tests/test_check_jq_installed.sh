#!/bin/bash
# Smoke tests for .codex/hooks/check-jq-installed.sh — SessionStart
...
#   $1/.codex/hooks/check-jq-installed.sh   (copy of the source hook)
#   $1/.codex/hooks/_lib-ops-root.sh        (copy of the shared lib)
build_fork() {
  local fk="$1"
  mkdir -p "$fk/.codex/hooks"
  : > "$fk/onboarding.yaml"
  : > "$fk/apexyard.projects.yaml"
  cp "$HOOK_SRC" "$fk/.codex/hooks/check-jq-installed.sh"
  chmod +x "$fk/.codex/hooks/check-jq-installed.sh"
  if [ -f "$LIB_SRC" ]; then
    cp "$LIB_SRC" "$fk/.codex/hooks/_lib-ops-root.sh"
  fi
}
...
run_hook_from() {
  ...
  case "$mode" in
    present)
      ( cd "$fk" || exit 1; bash .codex/hooks/check-jq-installed.sh 2>&1 )
      ;;
    missing)
      ...
      ( cd "$fk" || exit 1
        ...
        if ! command -v jq >/dev/null 2>&1; then
          bash .codex/hooks/check-jq-installed.sh 2>&1
        else
          bash -c '
            command() {
              if [ "$1" = "-v" ] && [ "$2" = "jq" ]; then
            ...
            cd "$1" || exit 1
            . .codex/hooks/check-jq-installed.sh
          ' _ "$fk" 2>&1
        fi
      )
      ;;
...
case_jq_missing_with_defaults() {
  local fk; fk=$(mktemp -d)/fork
  build_fork "$fk"
  : > "$fk/.codex/project-config.defaults.json"
