...
#     framework paths (.codex/hooks/, topologies/, handbooks/domain/) that are
...
#
# To run:  ./.codex/hooks/tests/test_pr_hooks_cross_repo.sh
...
AGDR_HOOK="$REPO_ROOT/.codex/hooks/require-agdr-for-arch-pr.sh"
VALIDATE_HOOK="$REPO_ROOT/.codex/hooks/validate-pr-create.sh"
...
make_validate_sandbox() {
  ...
  cp "$VALIDATE_HOOK" "$sb/.codex/hooks/validate-pr-create.sh"
  chmod +x "$sb/.codex/hooks/validate-pr-create.sh"
  for lib in \
    _lib-read-config.sh \
    _lib-tracker.sh \
    _lib-ops-root.sh \
    _lib-pr-repo.sh; do
    [ -f "$REPO_ROOT/.codex/hooks/$lib" ] && cp "$REPO_ROOT/.codex/hooks/$lib" "$sb/.codex/hooks/$lib"
  done
  [ -f "$REPO_ROOT/.codex/project-config.defaults.json" ] && \
    cp "$REPO_ROOT/.codex/project-config.defaults.json" "$sb/.codex/project-config.defaults.json"
  echo "$sb"
}
...
STDERR=$(cd "$SB" && echo "$INPUT" | bash .codex/hooks/validate-pr-create.sh 2>&1 >/dev/null)
RC=$?
rm -rf "$SB"
...
# THE SANDBOX, never touching $REPO_ROOT/.codex/hooks/ (the live hooks dir).
...
SB_C1=$(mktemp -d)
mkdir -p "$SB_C1/.codex/hooks"
cp "$AGDR_HOOK" "$SB_C1/.codex/hooks/require-agdr-for-arch-pr.sh"
chmod +x "$SB_C1/.codex/hooks/require-agdr-for-arch-pr.sh"
for lib in \
  _lib-read-config.sh \
  _lib-ops-root.sh \
  _lib-pr-repo.sh; do
  [ -f "$REPO_ROOT/.codex/hooks/$lib" ] && cp "$REPO_ROOT/.codex/hooks/$lib" "$SB_C1/.codex/hooks/$lib"
done
[ -f "$REPO_ROOT/.codex/project-config.defaults.json" ] && \
  cp "$REPO_ROOT/.codex/project-config.defaults.json" "$SB_C1/.codex/project-config.defaults.json"
# Remove the lib from the SANDBOX COPY (not the live hooks dir).
rm -f "$SB_C1/.codex/hooks/_lib-pr-repo.sh"
AGDR_HOOK_C1="$SB_C1/.codex/hooks/require-agdr-for-arch-pr.sh"
STDERR_C1=$( cd "$DIR_C1" && \
...
rm -f "$SB_C2/.codex/hooks/_lib-pr-repo.sh"
STDERR_C2=$(cd "$SB_C2" && echo "$INPUT_C2" | bash .codex/hooks/validate-pr-create.sh 2>&1 >/dev/null)
RC_C2=$?
rm -rf "$SB_C2"
