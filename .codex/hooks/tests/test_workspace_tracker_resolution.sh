...
# resolved `.codex/project-config.json` via `git rev-parse --show-toplevel`.
...
YAML
    mkdir -p .codex/hooks/tests
    cp "$TRACKER_LIB"        .codex/hooks/_lib-tracker.sh
    cp "$CONFIG_LIB"         .codex/hooks/_lib-read-config.sh
    cp "$OPS_ROOT_LIB"       .codex/hooks/_lib-ops-root.sh
    cp "$PR_CREATE_HOOK"     .codex/hooks/validate-pr-create.sh
    cp "$COMMIT_REFS_HOOK"   .codex/hooks/verify-commit-refs.sh
    cp "$SKILL_GATE_HOOK"    .codex/hooks/require-skill-for-issue-create.sh
    [ -f "$EXTRACT_PUSH_REF" ]   && cp "$EXTRACT_PUSH_REF"   .codex/hooks/
    [ -f "$DETECT_BASH_WRITE" ] && cp "$DETECT_BASH_WRITE" .codex/hooks/
...
case_1() {
  ...
  out=$(
    ...
    . "$sb/.codex/hooks/_lib-read-config.sh"
...
case_2() {
  ...
  out=$(
    cd "$sb/workspace/test-project" || exit 99
    . "$sb/.codex/hooks/_lib-read-config.sh"
    . "$sb/.codex/hooks/_lib-tracker.sh"
    tracker_clear_cache
    tracker_kind
  )
  if [ "$out" = "jira" ]; then
...
case_3() {
  ...
  rc=$(
    cd "$sb/workspace/test-project" || exit 99
    PATH="$sb/bin:$PATH" "$sb/.codex/hooks/verify-commit-refs.sh" <<<"$input" >/dev/null 2>&1
    echo $?
  )
...
" --head feature/PROJ-42-jira-shape'
  ...
  rc=$(
    cd "$sb/workspace/test-project" || exit 99
    PATH="$sb/bin:$PATH" "$sb/.codex/hooks/validate-pr-create.sh" <<<"$input" >/dev/null 2>&1
    echo $?
  )
...
case_5() {
  ...
  rc=$(
    cd "$sb/workspace/test-project" || exit 99
    PATH="$sb/bin:$PATH" "$sb/.codex/hooks/require-skill-for-issue-create.sh" <<<"$input" >/dev/null 2>&1
    echo $?
  )
  if [ "$rc" = "2" ]; then
...
case_7() {
  ...
  # .codex/project-config.json at its root — that's what we expect the
