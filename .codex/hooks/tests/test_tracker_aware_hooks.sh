...
YAML
    mkdir -p projects .codex/hooks/tests
    ...
    cp "$BRANCH_NAME_HOOK" .codex/hooks/validate-branch-name.sh
    ...
    if [ -f "$HOOK_DIR/_lib-extract-push-ref.sh" ]; then
      cp "$HOOK_DIR/_lib-extract-push-ref.sh" .codex/hooks/_lib-extract-push-ref.sh
    fi
...
run_pr_hook() {
  ...
  (
    cd "$sb" || exit 99
    PATH="$sb/bin:$PATH" .codex/hooks/validate-pr-create.sh <<<"$input" >/dev/null 2>&1
  )
  rc=$?
  [ "$rc" = "$expected_rc" ]
}
...
run_commit_hook() {
  ...
  (
    cd "$sb" || exit 99
    PATH="$sb/bin:$PATH" .codex/hooks/verify-commit-refs.sh <<<"$input" >/dev/null 2>&1
  )
  rc=$?
  [ "$rc" = "$expected_rc" ]
}
...
run_branch_hook() {
  ...
  (
    cd "$sb" || exit 99
    PATH="$sb/bin:$PATH" .codex/hooks/validate-branch-name.sh <<<"$input" >/dev/null 2>&1
  )
  rc=$?
  [ "$rc" = "$expected_rc" ]
}
