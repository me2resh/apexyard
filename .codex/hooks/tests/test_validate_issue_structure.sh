...
#
# Run:
#   ./.codex/hooks/tests/test_validate_issue_structure.sh
#
...
HOOK="$REPO_ROOT/.codex/hooks/validate-issue-structure.sh"
...
mkdir -p "$TMPDIR/fork/.codex/hooks" "$TMPDIR/fork/subdir"
...
cp "$REPO_ROOT/.codex/project-config.defaults.json" "$TMPDIR/fork/.codex/project-config.defaults.json"
cp "$REPO_ROOT/.codex/hooks/_lib-read-config.sh" "$TMPDIR/fork/.codex/hooks/_lib-read-config.sh"
