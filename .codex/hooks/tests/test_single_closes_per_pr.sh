...
make_sandbox() {
  ...
  cp "$HOOK_SRC" "$sb/.codex/hooks/validate-pr-create.sh"
  ...
  if [ -f "$src_root/.codex/hooks/_lib-tracker.sh" ]; then
    cp "$src_root/.codex/hooks/_lib-tracker.sh" "$sb/.codex/hooks/_lib-tracker.sh"
  fi
...
run_case() {
  ...
  if [ -n "$extra_config" ]; then
    echo "$extra_config" > "$sb/.codex/project-config.json"
  fi
  local cmd="gh pr create --repo me2resh/apexyard --title 'chore(#114): test' --body-file $body_file"
  local input
  input=$(jq -nc --arg c "$cmd" '{tool_input:{command:$c}}')
  local got_stderr got_rc
  got_stderr=$(cd "$sb" && echo "$input" | bash .codex/hooks/validate-pr-create.sh 2>&1 >/dev/null)
  got_rc=$?
  rm -rf "$sb"
