#!/bin/bash
# Smoke tests for .codex/hooks/pre-push-gate.sh
#
# Each case:
...
make_sandbox() {
  ...
  cp "$HOOK_SRC" "$sb/.codex/hooks/pre-push-gate.sh"
  chmod +x "$sb/.codex/hooks/pre-push-gate.sh"
  ...
  if [ -f "$src_root/.codex/hooks/_lib-read-config.sh" ]; then
    cp "$src_root/.codex/hooks/_lib-read-config.sh" "$sb/.codex/hooks/_lib-read-config.sh"
  fi
  if [ -f "$src_root/.codex/project-config.defaults.json" ]; then
    cp "$src_root/.codex/project-config.defaults.json" "$sb/.codex/project-config.defaults.json"
  fi
  echo "$sb"
}
...
run_hook() {
  ...
  (
    cd "$sb" || exit 1
    echo "$stdin_payload" | bash .codex/hooks/pre-push-gate.sh 2>/tmp/pre-push-gate-stderr.$$
  )
  local got_rc=$?
  local got_stderr
...
case1() {
  ...
  echo '{"tool_input":{"command":"ls -la"}}' | (cd "$sb" && bash .codex/hooks/pre-push-gate.sh 2>/dev/null)
  local rc=$?
  if [ "$rc" = "0" ]; then
...
case2() {
  local sb; sb=$(make_sandbox)
  cat > "$sb/.codex/project-config.json" <<'EOF'
{"pre_push": {"commands": []}}
EOF
