...
HOOK_SRC="$SRC_ROOT/.codex/hooks/require-architecture-review.sh"
...
make_sandbox() {
  ...
  mkdir -p "$sb/.codex/hooks" "$sb/.codex/session/reviews"
  # Copy libs needed by the hook.
  cp "$SRC_ROOT/.codex/hooks/_lib-extract-pr.sh" "$sb/.codex/hooks/_lib-extract-pr.sh"
  cp "$SRC_ROOT/.codex/hooks/_lib-review-markers.sh" "$sb/.codex/hooks/_lib-review-markers.sh"
  if [ -f "$SRC_ROOT/.codex/hooks/_lib-ops-root.sh" ]; then
