...
#        exist under .codex/agents/; silently skipped, no error.
...
#     5. Drift FIRES — pre-commit on a staged .codex/agents/<name>.md
...
SYNC_HOOK="$SRC_ROOT/.codex/hooks/apply-agent-routing.sh"
DRIFT_HOOK="$SRC_ROOT/.codex/hooks/block-agent-routing-drift.sh"
LIB_OPS="$SRC_ROOT/.codex/hooks/_lib-ops-root.sh"
...
YAML
    ...
    # Drop libs + defaults into the sandbox's .codex/hooks tree.
    mkdir -p .codex/hooks .codex/agents .codex/session
    cp "$LIB_OPS"  .codex/hooks/_lib-ops-root.sh
    cp "$LIB_PORT" .codex/hooks/_lib-portfolio-paths.sh
    cp "$LIB_CFG"  .codex/hooks/_lib-read-config.sh
    cp "$DEFAULTS" .codex/project-config.defaults.json
    cp "$SYNC_HOOK"  .codex/hooks/apply-agent-routing.sh
    cp "$DRIFT_HOOK" .codex/hooks/block-agent-routing-drift.sh
    chmod +x .codex/hooks/apply-agent-routing.sh .codex/hooks/block-agent-routing-drift.sh
...
TOML
    ...
    cat > .codex/agents/ticket-manager.toml <<'TOML'
...
.codex/agents/.framework-defaults.json
.codex/session/agent-env/
agent-routing.yaml
IGNORE
...
before_qa=$(read_model_line "$SB/.codex/agents/qa-engineer.toml")
before_lead=$(read_model_line "$SB/.codex/agents/tech-lead.toml")
before_be=$(read_model_line "$SB/.codex/agents/backend-engineer.toml")
...
after_be=$(read_model_line "$SB/.codex/agents/backend-engineer.toml")
...
(cd "$SB" && ANTHROPIC_BASE_URL=http://localhost:11434 PATH="$MOCK_BIN:$PATH" bash .codex/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true) >/dev/null

# Capture state after first run.
first_qa=$(read_model_line "$SB/.codex/agents/qa-engineer.toml")
first_env=""
[ -f "$SB/.codex/session/agent-env/qa-engineer.env" ] && first_env=$(cat "$SB/.codex/session/agent-env/qa-engineer.env")

# Run again.
(cd "$SB" && ANTHROPIC_BASE_URL=http://localhost:11434 PATH="$MOCK_BIN:$PATH" bash .codex/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true) >/dev/null

second_qa=$(read_model_line "$SB/.codex/agents/qa-engineer.toml")
second_env=""
...
# `git add`-ing it).
(cd "$SB" && git add .codex/agents/qa-engineer.toml 2>/dev/null)

# Invoke the drift guard with a `git commit -m "..."` command.
hook_output=$(hook_stdin "git commit -m 'feat: tweak QA agent'" | (cd "$SB" && bash .codex/hooks/block-agent-routing-drift.sh 2>&1) || true)
hook_exit=$(hook_stdin "git commit -m 'feat: tweak QA agent'" | (cd "$SB" && bash .codex/hooks/block-agent-routing-drift.sh > /dev/null 2>&1); echo $?)
...
cat > "$SB/.codex/agents/qa-engineer.toml" <<'MD'
---
name: qa-engineer
description: QA Engineer wrapper.
model: gpt-5.5
...
hook_exit=$(hook_stdin "git commit -m 'feat: bump QA default'" | (cd "$SB" && bash .codex/hooks/block-agent-routing-drift.sh > /dev/null 2>&1); echo $?)
...
hook_exit=$(hook_stdin "git commit -m 'docs: tidy QA agent description'" | (cd "$SB" && bash .codex/hooks/block-agent-routing-drift.sh > /dev/null 2>&1); echo $?)
...
after_tm=$(read_model_line "$SB/.codex/agents/ticket-manager.toml")
after_qa=$(read_model_line "$SB/.codex/agents/qa-engineer.toml")
defaults_snapshot_exists=0
[ -f "$SB/.codex/agents/.framework-defaults.json" ] && defaults_snapshot_exists=1
...
# does NOT fire.
banner_output=$(cd "$SB" && ANTHROPIC_BASE_URL=http://localhost:4000 PATH="$MOCK_BIN:$PATH" bash .codex/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true)
after_tm=$(read_model_line "$SB/.codex/agents/ticket-manager.toml")
session_env="$SB/.codex/session/agent-env/__session__.env"
agent_env="$SB/.codex/session/agent-env/ticket-manager.env"
session_set=0
agent_set=0
...
banner_output=$(cd "$SB" && PATH="$MOCK_BIN:$PATH" bash .codex/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true)
after_tm=$(read_model_line "$SB/.codex/agents/ticket-manager.toml")
session_env="$SB/.codex/session/agent-env/__session__.env"
agent_env="$SB/.codex/session/agent-env/ticket-manager.env"
session_absent=1
...
# is INACTIVE because Codex's process env doesn't have the var.
...
banner_output=$(cd "$SB" && unset ANTHROPIC_BASE_URL; PATH="$MOCK_BIN:$PATH" bash .codex/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true)
session_env="$SB/.codex/session/agent-env/__session__.env"
session_set=0
[ -f "$session_env" ] && grep -q '^ANTHROPIC_BASE_URL=http://localhost:4000$' "$session_env" && session_set=1
