...
LIB_CHAIN="$SRC_ROOT/.codex/hooks/_lib-migration-chain.sh"
REAL_V1_V2="$SRC_ROOT/.codex/migrations/v1.2.0-to-v1.3.0.sh"
REAL_V2_V3="$SRC_ROOT/.codex/migrations/v1.3.0-to-v1.4.0.sh"
...
#     - .codex/ with stub migrations for each pair implied by <versions_csv>
# E.g. build_fork /tmp/x "v1.0.0,v1.1.0,v1.2.0" creates two stubs:
#   .codex/migrations/v1.0.0-to-v1.1.0.sh
#   .codex/migrations/v1.1.0-to-v1.2.0.sh
# Each stub increments a counter file in $root/.counters/<pair>.
build_fork() {
  local root="$1"
  local versions_csv="$2"
  mkdir -p "$root/.codex/hooks" "$root/.codex/migrations" "$root/.counters"
  ...
  for cur in "${vs[@]}"; do
    if [ -n "$prev" ]; then
      ...
      local script="$root/.codex/migrations/${pair}.sh"
      cat > "$script" <<STUB
#!/bin/bash
...
rm -f "$SB/.codex/framework-version"
...
ANCHOR=$(head -n 1 "$SB/.codex/framework-version" 2>/dev/null)
[ "$ANCHOR" = "v1.4.0" ] \
...
cp "$REAL_V1_V2" "$SB/.codex/migrations/v1.2.0-to-v1.3.0.sh"
chmod +x "$SB/.codex/migrations/v1.2.0-to-v1.3.0.sh"

# No project-config.json → single-fork adopter
(
  cd "$SB" || exit 99
  APEXYARD_MIGRATION_QUIET=1 bash .codex/migrations/v1.2.0-to-v1.3.0.sh
)
rc1=$?
(
  cd "$SB" || exit 99
...
rm -f "$SB/.codex/migrations/v1.1.0-to-v1.2.0.sh"
...
cat > "$SB/.codex/migrations/v1.2.0-to-v1.3.0.sh" <<'SCR'
#!/bin/bash
exit 1
SCR
chmod +x "$SB/.codex/migrations/v1.2.0-to-v1.3.0.sh"
...
rm -f "$SB/.codex/migrations/v1.2.0-to-v1.3.0.sh"
(
  cd "$SB" || exit 99
  # shellcheck source=/dev/null
  . "$LIB_CHAIN"
