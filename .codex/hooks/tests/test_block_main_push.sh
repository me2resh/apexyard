...
HOOK_SRC="$SRC_ROOT/.codex/hooks/block-main-push.sh"
LIB_SRC="$SRC_ROOT/.codex/hooks/_lib-extract-push-ref.sh"
LIB_CONFIG_SRC="$SRC_ROOT/.codex/hooks/_lib-read-config.sh"
LIB_OPS_ROOT_SRC="$SRC_ROOT/.codex/hooks/_lib-ops-root.sh"
...
make_sandbox() {
  ...
  cp "$HOOK_SRC"        "$sb/.codex/hooks/block-main-push.sh"
  cp "$LIB_SRC"         "$sb/.codex/hooks/_lib-extract-push-ref.sh"
  if [ -f "$LIB_CONFIG_SRC" ]; then
    cp "$LIB_CONFIG_SRC"  "$sb/.codex/hooks/_lib-read-config.sh"
  fi
  if [ -f "$LIB_OPS_ROOT_SRC" ]; then
    cp "$LIB_OPS_ROOT_SRC" "$sb/.codex/hooks/_lib-ops-root.sh"
  fi
  if [ -f "$SRC_ROOT/.codex/project-config.defaults.json" ]; then
    cp "$SRC_ROOT/.codex/project-config.defaults.json" \
       "$sb/.codex/project-config.defaults.json"
  fi
  chmod +x "$sb/.codex/hooks/block-main-push.sh"
...
run_case() {
  ...
  got_stderr=$(cd "$hook_cwd" && echo "$input" | bash .codex/hooks/block-main-push.sh 2>&1 >/dev/null)
  got_rc=$?
