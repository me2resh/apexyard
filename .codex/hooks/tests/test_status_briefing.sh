#!/bin/bash
# Tests for .agents/skills/status/briefing.sh — the helper that backs
...
HELPER_SRC="$SRC_ROOT/.agents/skills/status/briefing.sh"
...
YAML
  ...
  # the shim's "find helper at \$ops_root/.agents/skills/status/briefing.sh"
  # resolution works.
  mkdir -p "$sb/.agents/skills/status" "$sb/.codex/session/tickets"
  cp "$HELPER_SRC" "$sb/.agents/skills/status/briefing.sh"
  chmod +x "$sb/.agents/skills/status/briefing.sh"
...
#   $1 — sandbox root
#   $2 — marker filename relative to .codex/session/ (e.g.
...
write_marker() {
  ...
  local path="$sb/.codex/session/$rel"
...
run_case() {
  ...
  out=$(APEXYARD_OPS_ROOT="$sb" APEXYARD_CWD="$cwd" APEXYARD_GH="$gh_bin" \
        bash "$sb/.agents/skills/status/briefing.sh" 2>&1)
  local rc=$?
...
# ---------------------------------------------------------------------------
# 4. ops-fallback ticket marker — read from .codex/session/current-ticket
# ---------------------------------------------------------------------------
sb=$(make_fork)
