#!/bin/bash
# Smoke tests for .codex/hooks/link-custom-skills.sh — the SessionStart
# hook that surfaces private custom skills (split-portfolio v2 + #243) to
# Codex by symlinking them into the public fork's .agents/skills/.
#
# Cases:
...
# _lib-ops-root.sh and WRITES symlinks into <ops-root>/.agents/skills/. Inside a
# live Codex session the session pin would resolve to the REAL fork, so a
# sandbox run would symlink into the real .agents/skills/. Disable the pin so
...
MD
    ...
    mkdir -p .codex/hooks .agents/skills
    cp "$HOOK_SRC"          .codex/hooks/link-custom-skills.sh
    cp "$LIB_PORTFOLIO_SRC" .codex/hooks/_lib-portfolio-paths.sh
    cp "$LIB_CONFIG_SRC"    .codex/hooks/_lib-read-config.sh
    if [ "$copy_ops_root" -eq 1 ]; then
      cp "$LIB_OPS_ROOT_SRC" .codex/hooks/_lib-ops-root.sh
    fi
    cp "$DEFAULTS_SRC"      .codex/project-config.defaults.json
    chmod +x .codex/hooks/link-custom-skills.sh
...
run_hook() {
  ...
  local prefix="${2:-}"
  ( cd "$sb" || exit 99
    eval "$prefix bash .codex/hooks/link-custom-skills.sh 2>&1" )
}
...
[ "$rc" -eq 0 ] || ok=0
[ -z "$out" ] || ok=0
[ -z "$(ls -A "$SB/.agents/skills" 2>/dev/null)" ] || ok=0
[ "$ok" -eq 1 ] && rc2=0 || rc2=1
...
[ -L "$SB/.agents/skills/file-internal-bug" ] || ok=0
[ -L "$SB/.agents/skills/check-policy" ] || ok=0
[ -f "$SB/.agents/skills/file-internal-bug/SKILL.md" ] || ok=0
[ -f "$SB/.agents/skills/check-policy/SKILL.md" ] || ok=0
[ "$ok" -eq 1 ] && rc2=0 || rc2=1
...
mkdir -p "$SB/.agents/skills/feature"
cat > "$SB/.agents/skills/feature/SKILL.md" <<'MD'
---
name: feature
---
# Framework /feature
...
[ -d "$SB/.agents/skills/feature.framework.bak" ] || ok=0
# Custom symlink in place.
[ -L "$SB/.agents/skills/feature" ] || ok=0
# Symlink resolves to the custom SKILL.md (the body says "Custom").
grep -q 'Custom /feature' "$SB/.agents/skills/feature/SKILL.md" || ok=0
...
[ -L "$SB/.agents/skills/real-skill" ] || ok=0
[ ! -e "$SB/.agents/skills/no-skill-md" ] || ok=0
# Summary line says 1.
...
[ ! -f "$SB/.codex/hooks/_lib-ops-root.sh" ] || {
...
out=$(run_hook "$SB")
ok=1
[ -L "$SB/.agents/skills/fallback-skill" ] || ok=0
echo "$out" | grep -q "linked 1 custom skill" || ok=0
[ "$ok" -eq 1 ] && rc2=0 || rc2=1
