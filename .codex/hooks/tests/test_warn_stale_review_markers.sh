#!/bin/bash
# Smoke tests for .codex/hooks/warn-stale-review-markers.sh
#
# Each case:
#   - sets up an isolated sandbox repo under $TMPDIR
#   - stages marker files (or not) under .codex/session/reviews/
...
make_sandbox() {
  ...
  cp "$HOOK_SRC"    "$sb/.codex/hooks/warn-stale-review-markers.sh"
  cp "$LIB_MARKERS" "$sb/.codex/hooks/_lib-review-markers.sh"
  chmod +x "$sb/.codex/hooks/warn-stale-review-markers.sh"
...
run_hook() {
  ...
  (
    ...
    printf '%s' "$json" \
      | "$sb/.codex/hooks/warn-stale-review-markers.sh" 2>"$stderr_file"
  )
  local rc=$?
