#!/bin/bash
# Smoke tests for .codex/hooks/warn-bootstrap-scope.sh — verifies the
...
HOOK="$SRC_ROOT/.codex/hooks/warn-bootstrap-scope.sh"
...
# Path the hook will look for: .codex/session/active-bootstrap under
...
mkdir -p "$FAKE_ROOT/.git" "$FAKE_ROOT/.codex/session"
...
MARKER="$FAKE_ROOT/.codex/session/active-bootstrap"
