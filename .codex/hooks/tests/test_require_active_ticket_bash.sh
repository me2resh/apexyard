...
HOOK_SRC="$SRC_ROOT/.codex/hooks/require-active-ticket.sh"
LIB_BASH="$SRC_ROOT/.codex/hooks/_lib-detect-bash-write.sh"
...
make_sandbox() {
  ...
  mkdir -p "$sb/.codex/hooks" "$sb/.codex/session"
  cp "$HOOK_SRC" "$sb/.codex/hooks/require-active-ticket.sh"
  cp "$LIB_BASH" "$sb/.codex/hooks/_lib-detect-bash-write.sh"
  cp "$LIB_CFG"  "$sb/.codex/hooks/_lib-read-config.sh"
  cp "$DEFAULTS" "$sb/.codex/project-config.defaults.json"
  chmod +x "$sb/.codex/hooks/require-active-ticket.sh"
  echo "$sb"
}
...
run_case() {
  ...
  got_stderr=$(cd "$sb" && echo "$input" | bash .codex/hooks/require-active-ticket.sh 2>&1 >/dev/null)
  got_rc=$?
  rm -rf "$sb"
...
# 4. echo > .codex/foo.json → allowed (path exemption catches it)
sb=$(make_sandbox)
in=$(jq -nc --arg c "echo x > .codex/foo.json" '{tool_name:"Bash", tool_input:{command:$c}}')
run_case "bash write to .codex/ exempt" 0 "" "$in" "$sb"
...
sb=$(make_sandbox)
echo "setup" > "$sb/.codex/session/active-bootstrap"
...
sb=$(make_sandbox)
echo "handover" > "$sb/.codex/session/active-bootstrap"
in=$(jq -nc --arg p "$sb/src/foo.ts" '{tool_name:"Edit", tool_input:{file_path:$p}}')
...
sb=$(make_sandbox)
echo "some-random-skill" > "$sb/.codex/session/active-bootstrap"
in=$(jq -nc --arg p "$sb/src/foo.ts" '{tool_name:"Edit", tool_input:{file_path:$p}}')
...
sb=$(make_sandbox)
: > "$sb/.codex/session/active-bootstrap"
in=$(jq -nc --arg p "$sb/src/foo.ts" '{tool_name:"Edit", tool_input:{file_path:$p}}')
...
sb=$(make_sandbox)
cat > "$sb/.codex/session/current-ticket" <<EOF
...
sb=$(make_sandbox)
rsb=$(cd "$sb" && pwd -P)
mkdir -p "$sb/.codex/session/tickets/myproj"
cat > "$sb/.codex/session/tickets/myproj/feature__x" <<EOF
repo=me2resh/apexyard
number=513
title=worktree A
EOF
...
rsb=$(cd "$sb" && pwd -P)
mkdir -p "$sb/.codex/session/tickets/myproj"
cat > "$sb/.codex/session/tickets/myproj/feature__a" <<EOF
repo=me2resh/apexyard
number=513
title=worktree A
EOF
...
sb=$(make_sandbox)
rsb=$(cd "$sb" && pwd -P)
mkdir -p "$sb/.codex/session/tickets"
cat > "$sb/.codex/session/tickets/myproj" <<EOF
repo=me2resh/apexyard
number=513
title=single agent
EOF
...
mkdir -p "$sb/.codex/session/tickets/myproj"
cat > "$sb/.codex/session/tickets/myproj/wt-x" <<EOF
repo=me2resh/apexyard
number=513
...
# 19. echo > .codex/session/foo with no ticket → allowed (exempt .codex/ path)
sb=$(make_sandbox)
in=$(jq -nc --arg c "echo x > .codex/session/foo" '{tool_name:"Bash", tool_input:{command:$c}}')
run_case "bash redirect to .codex/ exempt" 0 "" "$in" "$sb"

# 20. cp src dst where dst is a .codex/ path → allowed (exempt destination)
sb=$(make_sandbox)
in=$(jq -nc --arg c "cp .codex/session/tickets/myproj .codex/session/current-ticket" \
      '{tool_name:"Bash", tool_input:{command:$c}}')
run_case "bash cp to .codex/ destination exempt" 0 "" "$in" "$sb"
