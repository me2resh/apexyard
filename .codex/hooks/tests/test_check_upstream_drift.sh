#!/bin/bash
# Smoke tests for .codex/hooks/check-upstream-drift.sh — focused on the
...
make_fork() {
  ...
  (
    ...
    mkdir -p .codex/hooks .codex/session
    cp "$HOOK_SRC" .codex/hooks/check-upstream-drift.sh
    chmod +x .codex/hooks/check-upstream-drift.sh
  )
  echo "$fk"
}
...
run_hook_from() {
  local fk="$1"
  ( cd "$fk" || exit 1; bash .codex/hooks/check-upstream-drift.sh 2>&1 )
}
