...
BRANCH_HOOK="$SRC_ROOT/.codex/hooks/validate-branch-name.sh"
COMMIT_HOOK="$SRC_ROOT/.codex/hooks/validate-commit-format.sh"
PR_HOOK="$SRC_ROOT/.codex/hooks/validate-pr-create.sh"
LIB_CFG="$SRC_ROOT/.codex/hooks/_lib-read-config.sh"
LIB_PUSHREF="$SRC_ROOT/.codex/hooks/_lib-extract-push-ref.sh"
...
make_sandbox() {
  ...
  cp "$BRANCH_HOOK" "$sb/.codex/hooks/validate-branch-name.sh"
  cp "$COMMIT_HOOK" "$sb/.codex/hooks/validate-commit-format.sh"
  [ -f "$LIB_CFG" ]     && cp "$LIB_CFG"     "$sb/.codex/hooks/_lib-read-config.sh"
  [ -f "$LIB_PUSHREF" ] && cp "$LIB_PUSHREF" "$sb/.codex/hooks/_lib-extract-push-ref.sh"
  [ -f "$DEFAULTS" ]    && cp "$DEFAULTS"    "$sb/.codex/project-config.defaults.json"
  chmod +x "$sb/.codex/hooks/"*.sh
  echo "$sb"
}
...
run_hook_case() {
  ...
  got_stderr=$(cd "$sb" && echo "$input" | bash ".codex/hooks/$hook" 2>&1 >/dev/null)
  got_rc=$?
  rm -rf "$sb"
