...
# private sibling repo at <private>/custom-skills/<name>/) to Codex
# via the public fork's .agents/skills/ discovery path.
#
# Implementation: gitignored symlinks at .agents/skills/<name>/ pointing to
# <private>/custom-skills/<name>/. Codex's skill discovery walks
# .agents/skills/*/SKILL.md, so the symlink makes the private skill
...
#     contains SKILL.md, ensure a symlink exists at .agents/skills/<name>.
#       * If a framework skill of the same name exists at .agents/skills/<name>
...
LIB_PORTFOLIO="$ops_root/.codex/hooks/_lib-portfolio-paths.sh"
LIB_CONFIG="$ops_root/.codex/hooks/_lib-read-config.sh"
...
if [ "$IS_WINDOWS" -eq 1 ]; then
  ...
  echo "  Manual workaround: copy each subdir of custom-skills/ into .agents/skills/ on this machine." >&2
  echo "  See docs/multi-project.md § 'Private custom skills + handbooks' for the full pointer." >&2
  exit 0
fi

skills_target_dir="$ops_root/.agents/skills"
...
if [ "$linked_count" -gt 0 ]; then
  echo "ApexYard: linked $linked_count custom skill(s) from $custom_skills_dir into .agents/skills/." >&2
fi
