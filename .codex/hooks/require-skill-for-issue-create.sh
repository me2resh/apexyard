...
#
# Wired as PreToolUse:Bash. Reads stdin as the Codex hook JSON payload.
...
# from `.codex/project-config.defaults.json` → `ticket.create_command_patterns`
# (adopters extend via `.codex/project-config.json` shallow-merge).
#
# Resolution:
#   1. If the command does not match any configured pattern → exit 0 (no-op).
#   2. If the bootstrap-skill marker (.codex/session/active-bootstrap) is
...
#   3. If the ticket-skill marker (.codex/session/active-issue-skill) is
...
SKILL_MARKER="$MARKER_HOME/.codex/session/active-issue-skill"
...
custom), add patterns under .codex/project-config.json →
