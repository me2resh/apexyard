...
ApexYard is **built for Claude Code** — that's where the whole experience is native. But its mechanical enforcement layer is **portable bash**, not Claude-Code-specific, so other agent harnesses reach the *same* gates through thin adapters that shell out to the unmodified `.claude/hooks/*.sh` scripts. This is the honest, per-harness "what works where, today" view — no over-claiming an enforcement path that hasn't been live-proven. As of 2026-07-09 **three adapters (opencode, pi, Codex) are live-proven** — a real model turn under each was actually blocked by a delegated bash gate.

Status legend: **Native** · ✅ **live-proven** (a real model-turn actually blocked by a delegated gate) · ⏳ **pending live verification** (transport tested in-repo, credentialed run not yet recorded) · 🟡 **failClosed-only** (blocks, but by failing closed on a hook-runner error, not verified delegated execution).

The pattern that emerged: **every adapter enforces live in headless with the right trust/location precondition** — opencode with `--auto`, pi with `-a`/`--approve` (or user-level install), Codex with hook-trust granted (`/hooks`, `--dangerously-bypass-hook-trust`, or user-level `~/.codex/hooks.json`). Three are live-proven on that basis. The **one outlier is Cursor**: its hook-runner can't cleanly exec external commands in agent mode (a `MainThreadShellExec` limitation), so it only *fails closed* rather than running the delegated gate, and the `cursor-agent` CLI ignores hooks entirely.

| Harness | Advisory governance | Mechanical gates | Install | Live-enforcement status | Known preconditions & limits |
|---------|---------------------|------------------|---------|-------------------------|------------------------------|
| **Claude Code** | Native — `CLAUDE.md` auto-loads; all rules, skills, agents are first-class | **Enforced live** — bash hooks fire on every tool call | Native — `/setup`, `.claude/` auto-picked-up | **Native** | Windows needs Git Bash / WSL for the bash hooks |
| **opencode** | `AGENTS.md` bridge | **Delegated** — plugin derives its gate table from `settings.json` and execs the unmodified bash hooks | `bash bin/install-opencode-adapter.sh` → `.opencode/plugins/apexyard/` | ✅ **Live-proven** (2026-07-09) — a real `opencode run --auto` turn's `git add -A` was blocked by the delegated `block-git-add-all.sh` | Run with `--auto` so the tool call reaches the gate; discovery dir is plural `.opencode/plugins/` (#844/#845). [AgDR-0092](docs/agdr/AgDR-0092-opencode-gate-adapter.md) |
| **pi** (pi.dev) | `AGENTS.md` + `SYSTEM.md` advisory bridge | **Delegated** — one dispatcher extension over the unmodified bash hooks | `bash bin/install-pi-adapter.sh` → `.pi/extensions/apexyard/` | ✅ **Live-proven** (2026-07-09) — a real `pi -p -a` turn was blocked the same way | Headless needs `-a` / `--approve` (project trust) so the tool call reaches the gate (#844/#845). [AgDR-0082](docs/agdr/AgDR-0082-pi-gate-dispatcher-adapter.md) |
| **Codex** | Generated from `.claude/` into `.agents/` + `.codex/` | **Delegated** — generated `.codex/hooks.json` execs the unmodified bash hooks | `bin/sync-codex-adapter.sh` → `.codex/hooks.json` | ✅ **Live-proven** (2026-07-09) — a real `codex exec --dangerously-bypass-hook-trust -m gpt-5.5` turn's `git add -A` fired the delegated `block-git-add-all.sh` (clean exit 2, nothing staged) | Project `.codex/hooks.json` requires **trust** — grant via `/hooks` (interactive), `--dangerously-bypass-hook-trust` (headless one-off), or install to user-level `~/.codex/hooks.json`. Model mapping `opus`→`gpt-5.5` is the ChatGPT-account default. [AgDR-0088](docs/agdr/AgDR-0088-codex-adapter-generation.md) |
| **Cursor** | Generated `.cursor/rules/` bridge | **Delegated** — generated `.cursor/hooks.json`; native `exit 2`, `failClosed` on security-critical gates | `bin/sync-cursor-adapter.sh` → user-level `~/.cursor/hooks.json` (see [`cursor.md`](docs/harnesses/cursor.md)) | 🟡 **failClosed-only** — Cursor IDE 3.x enforces via `failClosed`, not verified delegated execution; not live-proven | `cursor-agent` CLI does **not** honor `hooks.json` (uses `cli-config` permissions); IDE 3.x loads hooks only from **user-level** `~/.cursor/hooks.json`, not project. [AgDR-0091](docs/agdr/AgDR-0091-cursor-adapter-generation.md) |
