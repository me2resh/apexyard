...
The split is architectural: the **imperative-plugin** adapters (opencode, pi) fire their gate inside the harness's own tool-call event, so they enforce live in headless/CLI mode — both are proven. The **declarative-hooks** adapters (Codex, Cursor) rely on the harness loading a `hooks.json` file, and their headless/CLI modes (`codex exec`, `cursor-agent`) don't load project hooks — so they need the interactive/IDE path and likely a user-level config. That's *why* opencode and pi cleared the bar first.
...
| **Codex** | Generated from `.claude/` into `.agents/` + `.codex/` | **Delegated** — generated `.codex/hooks.json` execs the unmodified bash hooks | `bin/sync-codex-adapter.sh` → `.codex/hooks.json` | ⏳ **Not enforcing in headless** — `codex exec -m gpt-5.5` runs a shell command *without* firing the project `.codex/hooks.json` hook; interactive `codex` TUI + user-level `~/.codex/hooks.json` path untested (#840) | Schema is correct and model mapping (`opus`→`gpt-5.5`) is right; the headless CLI just doesn't honor project hooks. Like `cursor-agent`, a declarative-hooks adapter that needs the interactive/user-level path. [AgDR-0088](docs/agdr/AgDR-0088-codex-adapter-generation.md) |
