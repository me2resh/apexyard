...
**Signalling activation**: when activated, print the marker convention from `.apexyard/rules/role-triggers.md` § "How to signal activation". Example: `▸ Activating Khalil (Head of Data) for #<ticket> (trigger: <reason>)`.
...
**Sub-agent file**: `.apexyard/agents/head-of-data.md` (ships in #347 PR 3; will use model `sonnet` + restricted tools per AgDR-0050 Axis 2)

**On trigger**: once PR 3 lands, the `detect-role-trigger.sh` hook spawns the sub-agent at `.apexyard/agents/head-of-data.md`; the main thread continues with the spawned agent's verdict folded back via standard sub-agent return. Until then, in-thread role-adoption is the active mechanism.
