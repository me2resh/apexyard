...
| Rules | `.claude/rules/` | 17 modular rule files (AgDR triggers, agent role selection, code standards, git conventions, isolated builds, leak protection, loop mode, parallel work, plan mode, PR quality, PR workflow, reconcile before build, reporting style, role triggers, skill first, ticket vocabulary, workflow gates) |
