...
function renderCodexConfigToml(roles: Role[], config: Config): string {
  // Codex CLI only ships with OpenAI providers. We use gpt-5.4 as the default
  ...
  //
  // Per Codex v0.136 docs:
  //   model_provider = "openai"      # built-in, defined below for clarity
  //   [model_providers.openai]
  //   env_key = "OPENAI_API_KEY"     # where the bearer token lives
  ...
  const codexDefaultModel = "gpt-5.4"
...
# Built-in OpenAI provider. env_key tells Codex which env var holds the
# bearer token. wire_api = "responses" uses the Responses API (recommended
# for gpt-5.x); "chat" uses the older Chat Completions endpoint.
[model_providers.openai]
name = "OpenAI"
...
async function buildCodexAgentsMd(roles: Role[], config: Config): Promise<void> {
  ...
  content += `## Defaults\n\n`
  content += `- Default model: \`gpt-5.4\` (Codex only ships OpenAI providers; the\n`
...
//   - critical (head-of-*, code-reviewer, security-reviewer) → gpt-5.4
//   - default → gpt-5.4 (matches the top-level model)
...
// we read and embed as `developer_instructions`. If body_file is missing,
...
function tomlMultiline(s: string): string {
  // Use TOML triple-quoted basic string for the developer_instructions body.
  // Indentation is preserved literally inside """...""" (no escape needed
  // for newlines or quotes), but the closing """ must be on its own line.
  // To be safe, replace any literal `"""` in the input with `\"\"\"` even
  // though it's not strictly necessary for TOML parsers.
  return s.replace(/"""/g, '\\"\\"\\"')
}
...
function resolveCodexModel(role: Role, config: Config): string {
  ...
  //   anything else     → gpt-5.4
  const resolved = resolveModelForRole(role, config)
  if (resolved === "openai/gpt-5" || resolved === "gpt-5") return "gpt-5.4"
...
function renderCodexAgentToml(role: Role, body: string, config: Config): string {
  ...
  const instructions = tomlMultiline(body.trim())
  // Per Codex v0.137 schema, agent files use a top-level [instructions] table
  // with a `text` field (NOT a `developer_instructions` top-level string).
  // See https://developers.openai.com/codex/subagents
  let toml = `# GENERATED by bin/sync.ts — do not edit by hand.
