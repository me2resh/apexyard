...
- **Verify means build + tests + Rex — not just build.** A loop's eval stage must run the project's test suite (and lint/typecheck) in addition to the build, then a Rex pass. Skipping the test run is how a green build still ships broken behaviour — exactly what happened on the v3 view-redesign loop, where unverified agents left failing tests for cleanup afterward.
