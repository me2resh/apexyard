...
Before doing anything else, write this skill's name to the active-issue-skill marker so the `require-skill-for-issue-create.sh` hook lets any tracker-CLI call you make (if you escalate from the ideas backlog to a real ticket) through. Run from any bash block at skill entry:
