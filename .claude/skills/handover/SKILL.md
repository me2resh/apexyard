...
Also replace the bottom-of-file guidance section with a brief **"Handover-generated — refine me"** note so the user knows this was machine-drafted. Use this literal block (markdown fenced with `~~~` to avoid nested backtick parsing — render it to the target file as normal Markdown):

~~~markdown
> **Note**: this diagram was auto-generated by `/handover` on YYYY-MM-DD from repo signals (`package.json`, `docker-compose.yml`, `Dockerfile`, Prisma schema, `.env.example`). It is a **starting point** — review and refine. Specifically look at:
>
