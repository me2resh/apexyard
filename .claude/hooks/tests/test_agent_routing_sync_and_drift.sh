...
if is_codex_adapter; then
  if codex_agent_has_endpoint "$SB/.claude/agents/qa-engineer.md" "qa-engineer" "http://localhost:11434"; then
    codex_provider_ok=1
  fi
  codex_provider_count=$(codex_provider_count_for_agent "$SB/.claude/agents/qa-engineer.md" "qa-engineer")
fi
...
elif [ "$first_qa" = "sonnet" ] && [ "$second_qa" = "sonnet" ] && [ "$first_env" = "$second_env" ] \
...
codex_provider_ok=0
if is_codex_adapter && codex_agent_has_endpoint "$SB/.claude/agents/ticket-manager.md" "ticket-manager" "http://localhost:4000"; then
  codex_provider_ok=1
fi
...
qa_provider_ok=0
if is_codex_adapter; then
  if codex_agent_has_endpoint "$SB/.claude/agents/ticket-manager.md" "ticket-manager" "http://localhost:4000"; then
    tm_provider_ok=1
  fi
  if codex_agent_has_endpoint "$SB/.claude/agents/qa-engineer.md" "qa-engineer" "http://localhost:4000"; then
    qa_provider_ok=1
  fi
fi
...
if is_codex_adapter; then
  ...
  if codex_agent_has_endpoint "$SB/.claude/agents/qa-engineer.md" "qa-engineer" "http://localhost:4001"; then
    qa_provider_ok=1
  fi
fi
