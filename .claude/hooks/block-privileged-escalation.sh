...
if printf '%s\n' "$COMMAND" | grep -Eq '(^|[;&|[:space:]])sudo([[:space:]]|$)|(^|[[:space:]])gh[[:space:]]+pr[[:space:]]+merge([[:space:]]|$)[^|;&]*([[:space:]])--admin([[:space:]]|$)|(^|[[:space:]])git[[:space:]]+push[^|;&]*([[:space:]]--force|[[:space:]]-f)([[:space:]]|$)|(^|[[:space:]])git[[:space:]]+[^|;&]*--no-verify([[:space:]]|$)|(^|[[:space:]])(aws[[:space:]]+sts[[:space:]]+assume-role|gcloud[[:space:]]+auth[[:space:]]+activate-service-account)([[:space:]]|$)'; then
