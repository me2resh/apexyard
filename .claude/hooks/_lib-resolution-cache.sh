...
# ---------------------------------------------------------------------------
# Every cached value here is a pure function of exactly three inputs:
#   1. the resolved ops root
#   2. .claude/project-config.defaults.json's (mtime, size)
#   3. .claude/project-config.json's (mtime, size), or the literal ABSENT
#      token when the file does not exist
#
# _resolution_cache_current_fingerprint() combines all three into one
# opaque string. Every cache file stores its own fingerprint ALONGSIDE its
# value (fingerprint on line 1, value on the remaining line(s)) so each
# file is independently self-describing — a reader never has to trust that
# some OTHER file's fingerprint update implies this file is current too.
# On every read, the CURRENT fingerprint is recomputed fresh (two `stat`
...
# next read, never silently serving the pre-edit answer.
#
...
# ------------------------------------------------------------------------------
# _resolution_cache_config_fingerprint <root> <defaults_file> <overrides_file>
...
#   functions of these same three inputs. One shared signature means every
#   dependent cache entry invalidates together on any of the three
#   changing, rather than each consumer growing its own slightly different
#   staleness rule that could disagree with the others.
#
...
_resolution_cache_config_fingerprint() {
  local root="$1" defaults="$2" overrides="$3"
  local d_sig o_sig
  ...
  printf '%s|%s|%s' "$root" "$d_sig" "$o_sig"
}

# ------------------------------------------------------------------------------
# _resolution_cache_current_fingerprint
#   Convenience wrapper: resolves defaults/overrides paths + ops root via
...
#   assembling the three inputs themselves, so there is exactly one place
...
_resolution_cache_current_fingerprint() {
  ...
  local defaults overrides root
  defaults=$(_config_defaults_file)
  overrides=$(_config_overrides_file)
  root="${_CONFIG_ROOT_CACHE:-}"
  _resolution_cache_config_fingerprint "$root" "$defaults" "$overrides"
}
...
#   must never be trusted as a cache key by a future reader.
#
#   Returns 1 (no-op, nothing written) on: caching disabled, unwritable
#   cache dir, or any I/O failure. Never surfaces an error to stdout —
#   callers' stdout is the hook's/lib's own return value and must stay
#   clean regardless of cache-write success.
# ------------------------------------------------------------------------------
...
#   Same contract as _resolution_cache_write, specialised for the one
