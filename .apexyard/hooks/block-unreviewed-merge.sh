...
# NOT merge approval" rule in .apexyard/rules/pr-workflow.md. An umbrella
...
ApexYard requires two reviews before merge (workflow-gates rule #5):
  1. Code Reviewer agent (Rex) — automated, recorded in .apexyard/session/reviews/
...
Never skip this check — even for typo fixes. See .apexyard/rules/pr-workflow.md.
MSG
  exit 2
fi
...
PR. See .apexyard/rules/pr-workflow.md § "Plan-level 'go' is NOT merge
