...
# top-level keys present in `.apexyard/project-config.json` (the adopter override)
# that are NOT present in `.apexyard/project-config.defaults.json` (the upstream-
...
detect_deprecated_config_keys() {
  ...
  if [ -z "$defaults" ] || [ -z "$overrides" ]; then
    local root
    root=$(_dep_repo_root)
    [ -z "$defaults" ] && defaults="$root/.apexyard/project-config.defaults.json"
    [ -z "$overrides" ] && overrides="$root/.apexyard/project-config.json"
  fi
