...
# .apexyard/session/reviews/<pr>-architecture.approved (with a matching HEAD SHA)
...
#
# Enforces .apexyard/rules/workflow-gates.md § "Architecture Review Gate" and
...
if [ -n "$REPO_ROOT" ] && [ -f "${REPO_ROOT}/.apexyard/project-config.json" ]; then
  CUSTOM=$(jq -r '.design_paths // [] | join("|")' "${REPO_ROOT}/.apexyard/project-config.json" 2>/dev/null)
...
if [ -n "$REPO_ROOT" ] && [ -f "${REPO_ROOT}/.apexyard/project-config.json" ]; then
  EXCLUDE=$(jq -r '.design_paths_exclude // [] | join("|")' "${REPO_ROOT}/.apexyard/project-config.json" 2>/dev/null)
...
before the team builds against it. See .apexyard/rules/workflow-gates.md
