...
# in CLAUDE.md, workflows/sdlc.md, or .apexyard/rules/workflow-gates.md.
#
# Active tickets are declared by the /start-ticket skill. The marker
# layout is three-tier (apexyard#41 + #513):
#
#   ops_root/.apexyard/session/tickets/<project>/<branch>  ← per-worktree (#513)
#   ops_root/.apexyard/session/tickets/<project>           ← per-project (#41)
#   ops_root/.apexyard/session/current-ticket              ← ops-repo / fallback
#
...
if [ "$TOOL_NAME" = "Bash" ]; then
  ...
  # exemptions (.apexyard/, docs/, *.md). If extraction fails, FILE_PATH
  ...
  # (`$CEO`, `${marker}` — unresolvable, in practice a .apexyard/ scratch path). A
...
if [ -n "$REL_PATH" ]; then
  case "$REL_PATH" in
    .apexyard/*|.claude|*/.apexyard/*|*/.claude) exit 0 ;;
...
# marker at .apexyard/session/active-bootstrap with the skill name on
...
BOOTSTRAP_MARKER="$MARKER_HOME/.apexyard/session/active-bootstrap"
if [ -f "$BOOTSTRAP_MARKER" ]; then
  ...
  if [ -n "$active_bootstrap" ]; then
    ...
    if [ -f "$MARKER_HOME/.apexyard/hooks/_lib-read-config.sh" ]; then
      # shellcheck source=/dev/null
      . "$MARKER_HOME/.apexyard/hooks/_lib-read-config.sh"
...
# .apexyard/session/tickets/<project>. This keeps per-project session state
# keyed by the managed-project name and localised in the ops fork
# (gitignored), instead of the pre-#41 scheme that relied on a
# .apexyard/session/ inside each managed-project clone.
#
...
if [ -n "$PROJECT" ]; then
  ...
  if [ -n "$WT_BRANCH" ]; then
    ...
    PER_WORKTREE_MARKER="$MARKER_HOME/.apexyard/session/tickets/$PROJECT/$SAFE_BRANCH"
...
PER_PROJECT_MARKER=""
if [ -n "$PROJECT" ]; then
  PER_PROJECT_MARKER="$MARKER_HOME/.apexyard/session/tickets/$PROJECT"
...
FALLBACK_MARKER="$MARKER_HOME/.apexyard/session/current-ticket"
...
Exempt paths (no ticket required): .apexyard/, docs/, projects/*/docs/, *.md
MSG
exit 2
