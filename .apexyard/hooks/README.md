...
Hooks are shell scripts the Claude Code harness runs **before or after tool calls**. They are the only reliable way to make process rules stick — anything written only in `CLAUDE.md`, `.apexyard/rules/*.md`, or `workflows/*.md` is advice the model may drop under pressure. Anything in this directory is mechanically enforced.
...
These four hooks make the SDLC mechanical instead of advisory. Each enforces a rule that was previously only prose in `workflows/sdlc.md` or `.apexyard/rules/*.md`.

### 1. Ticket-first — `require-active-ticket.sh`

**Event:** `PreToolUse` on `Edit | Write | MultiEdit`.

**What it does:** blocks edits to any code path unless a session marker exists. Resolution is three-tier (#41 + #513): (0) per-worktree marker at `<ops_root>/.apexyard/session/tickets/<project>/<safe-branch>` when the edited file's repo is on a git-worktree branch — lets parallel agents on the *same* project hold independent tickets; (1) per-project marker at `<ops_root>/.apexyard/session/tickets/<project>` (a FILE) when `FILE_PATH` is under `<ops_root>/workspace/<project>/`; (2) fallback `<ops_root>/.apexyard/session/current-ticket`. Exempts `.apexyard/`, `docs/`, `projects/*/docs/`, and any `*.md` file so framework / doc / meta work is still fluid.

**Enforces:** the Pre-Build Gate in `.apexyard/rules/workflow-gates.md` — "do not start coding until the ticket exists, has acceptance criteria, and is broken into tasks."
...
2. The referenced tracker issue is OPEN and carries the `migration` label (default; overridable per project via `.apexyard/project-config.json` → `migration_label`)
...
**Enforces:** gate 3a in `.apexyard/rules/workflow-gates.md` — "any edit to migration paths requires a labelled migration ticket + linked AgDR".
...
**What it does:** parses the PR URL from the `gh` output, writes a pending-review marker at `.apexyard/session/pending-reviews/<pr>`, and emits a loud reminder telling Claude to invoke the `code-reviewer` agent (Rex) immediately. Not a tool error — the PR is created fine. The hook just pushes the next step into the conversation so it can't be forgotten.

**Enforces:** the "After `gh pr create` → Invoke Code Reviewer agent" section of `.apexyard/rules/pr-workflow.md` and the Code Review phase of `workflows/sdlc.md`.
...
|--------|----------------------------------|------------|-----------|
| Rex | `.apexyard/session/reviews/<owner>__<repo>__<pr>-rex.approved` | `code-reviewer` agent after a successful review | "Code reviewed, no blocking issues" |
| CEO | `.apexyard/session/reviews/<owner>__<repo>__<pr>-ceo.approved` | `/approve-merge <pr>` skill, **only** on explicit user invocation | "The human approver has looked at this specific PR and said ship it" |
...
**Why two markers:** The Rex marker alone isn't enough because it would only enforce the "code review happened" half of the 2-reviews rule. The CEO marker is the mechanical enforcement of **"plan-level 'go' is NOT merge approval"** from `.apexyard/rules/pr-workflow.md`. A plan-level authorization does not produce the CEO marker — only the `/approve-merge` skill does, and the skill is defined to run only on explicit per-PR user invocation. This closes the failure mode where Claude infers merge approval from an umbrella "go" on a broader plan.
...
**Companion skill:** `/approve-merge <pr>` (in `.apexyard/skills/approve-merge/`) is the only supported way to write the CEO marker. The skill definition includes explicit anti-patterns describing the wrong invocation triggers; read it before using.

### 4. Onboarding — `onboarding-check.sh`

**Event:** `SessionStart`.

**What it does:** on every new session, if `.apexyard/session/onboarded` is missing, injects a reminder telling Claude to run `/onboard` with the user before doing work. The `/onboard` skill asks the day-one discovery questions (project identity, tracker, required checks, reviewers, UI, deploy targets, sensitive topics) and writes the marker plus `.apexyard/project-config.json`.

### 4a. Upstream drift — `check-upstream-drift.sh`

**Event:** `SessionStart`.

**What it does:** if the repo has an `upstream` remote, runs `git fetch upstream --quiet` (cached to once per 10 minutes via `.apexyard/session/last-upstream-fetch`) and prints a one-line banner when the local default branch is behind `upstream/<default-branch>`:
...
These two hooks are the mechanical backstop for the rule in `.apexyard/rules/ticket-vocabulary.md` — "`Ticket`, `#N`, and dependency notation refer ONLY to real GitHub issues". The rule itself is self-discipline; these hooks catch the downstream symptom (a fabricated `#N` that slipped into a durable artifact).
...
1. First tries `.tracker_repo` in `.apexyard/project-config.json` if present
...
**Customize:** set `.architecture_paths` in `.apexyard/project-config.json` to a JSON array of regex patterns. The default list is deliberately narrow — see AgDR-0001 for why dependency manifests (`package.json`, `go.mod`) and API schemas are explicitly excluded.
...
**Enforces:** `.apexyard/rules/agdr-decisions.md § Enforcement` — specifically the line "Pre-commit hook warns if architecture files changed without an AgDR reference", which was prose-only until this hook shipped.

### 8. Design-review-for-UI merge gate — `require-design-review-for-ui.sh`

**Event:** `PreToolUse` on `Bash(gh pr merge *)`.

**What it does:** if the PR's diff touches any UI file, requires a design-approval marker at `.apexyard/session/reviews/<owner>__<repo>__<pr>-design.approved` (repo-qualified, see AgDR-0060) with a SHA matching HEAD. Non-UI PRs bypass silently.
...
**Customize:** `.ui_paths` in `.apexyard/project-config.json`.

**Companion skill:** `/approve-design <pr>` (in `.apexyard/skills/approve-design/`) writes the marker. It follows the same pattern as `/approve-merge`: verify PR state → verify Rex marker at HEAD → write the design marker at the repo root → confirm → stop. The skill definition includes explicit valid/invalid triggers and an anti-pattern section distinguishing mockup approval (design phase) from implementation-review approval (PR phase).

**Manual fallback:** for projects that deliberately skip design review (admin tools, internal dashboards), create `touch .apexyard/session/reviews/<owner>__<repo>__<pr>-design.approved` (using the repo-qualified name) as a visible, auditable "we decided to skip" artifact.

**Enforces:** `.apexyard/rules/pr-quality.md § "Design Review (UI Changes)"` and `workflows/code-review.md § "UI Designer (conditional)"` — both prose-only until this hook shipped.
...
**Enforces:** `.apexyard/rules/pr-quality.md § "No Red CI Before Merge"` — "Never merge with red CI, even if the failure is pre-existing or unrelated." Was prose-only.
...
**Customize:** set `.commit_types` in `.apexyard/project-config.json` to a JSON array of strings. When set, ONLY those types are accepted — the default list is **not** merged; the override replaces it entirely. This lets teams with strict conventions whitelist exactly the types they use:
...
**Enforces:** `.apexyard/rules/git-conventions.md § "Commit Message Format"` — was prose-only.
...
`.apexyard/session/` is gitignored. It holds per-machine, per-clone state:
...
printf '%s' '{"tool_name":"Edit","tool_input":{"file_path":"/tmp/foo.ts"}}' \
  | .apexyard/hooks/require-active-ticket.sh
echo "exit=$?"
...
printf '%s' '{"tool_name":"Bash","tool_input":{"command":"gh pr create --title foo"},"tool_response":{"stdout":"https://github.com/acme/repo/pull/42"}}' \
  | .apexyard/hooks/auto-code-review.sh
echo "exit=$?"
```
