...
#
# Version anchor lives at <ops_fork_root>/.apexyard/framework-version.
...
migration_anchor_path() {
  ...
  [ -n "$root" ] || return 1
  echo "$root/.apexyard/framework-version"
}
...
# attached. Source of truth is the filenames in .apexyard/migrations/.
...
migration_known_versions() {
  ...
  root=$(_migration_ops_root)
  [ -n "$root" ] || return 1
  local dir="$root/.apexyard/migrations"
  [ -d "$dir" ] || return 0
...
migration_script_path() {
  ...
  root=$(_migration_ops_root)
  [ -n "$root" ] || return 1
  echo "$root/.apexyard/migrations/${pair}.sh"
}
