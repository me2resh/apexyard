...
# reads a list of shell commands from `.apexyard/project-config.*.json`
...
#
# This implements the HARD STOP documented in `.apexyard/rules/pr-workflow.md`
...
#   - No `.apexyard/project-config.defaults.json` AND no `package.json` in the
...
# Shipped defaults ship at .apexyard/project-config.defaults.json.
...
auditable. See .apexyard/rules/pr-workflow.md "Before git push (HARD STOP)".
MSG
  exit 2
fi
