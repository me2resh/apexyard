...
# .apexyard/rules/git-conventions.md:
#
#   type: subject
#
...
#
# Default types per .apexyard/rules/git-conventions.md ship at
# .apexyard/project-config.defaults.json (.commit.type_whitelist). Projects
# override per-fork via .apexyard/project-config.json — see apexyard#109.
#
# Backward-compat: the legacy flat `commit_types` top-level key in
# .apexyard/project-config.json is still honoured when present, so forks that
...
if [ -n "$REPO_ROOT" ] && [ -f "$REPO_ROOT/.apexyard/hooks/_lib-read-config.sh" ]; then
...
if [ -z "$TYPES" ] && [ -n "$REPO_ROOT" ] && [ -f "${REPO_ROOT}/.apexyard/project-config.json" ]; then
  LEGACY=$(jq -r '.commit_types // [] | join("|")' "${REPO_ROOT}/.apexyard/project-config.json" 2>/dev/null)
...
Expected format (from .apexyard/rules/git-conventions.md):
  type: subject
  type(scope): subject
...
list in .apexyard/hooks/validate-commit-format.sh or file a ticket to add
