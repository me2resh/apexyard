...
# exemption marker is active (.apexyard/session/active-bootstrap) but the
...
#
# Resolution: .apexyard/rules/workflow-gates.md § Bootstrap-skill exemption
# and .apexyard/skills/handover/SKILL.md § "Bootstrap scope".
#
...
MARKER="$REPO_ROOT/.apexyard/session/active-bootstrap"
if [ ! -f "$MARKER" ]; then
  exit 0
fi
