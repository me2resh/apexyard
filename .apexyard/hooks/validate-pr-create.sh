...
#
# The accepted type list is project-configurable via .apexyard/project-config.json
# (.pr.title_type_whitelist). Defaults ship at .apexyard/project-config.defaults.json.
# See apexyard#109.
#
# Resolve the directory holding the config:
#   - HOOK_DIR points at the lib files (always next to this script).
#   - CONFIG_ROOT is the ops fork (where .apexyard/project-config.json lives).
...
if [ -n "$TITLE" ]; then
  if ! echo "$TITLE" | grep -qE "^(${PR_TYPES})\(([A-Z]{2,10}-[0-9]+|#[0-9]+)\)!?:"; then
    ...
    ERRORS="${ERRORS}Accepted types (from .apexyard/project-config.*.json → .pr.title_type_whitelist): ${PR_TYPES//|/, }\n"
  else
...
if [ -n "$TICKET_REF" ]; then
  ...
  elif [ -n "$CONFIG_ROOT" ] && [ -f "${CONFIG_ROOT}/.apexyard/project-config.json" ]; then
    TRACKER_REPO=$(jq -r '.tracker_repo // empty' "${CONFIG_ROOT}/.apexyard/project-config.json" 2>/dev/null)
  fi
...
See .apexyard/rules/ticket-vocabulary.md § "The rule".
...
See .apexyard/rules/ticket-vocabulary.md and the "every PR needs its own open
...
# .apexyard/project-config.*.json (`.pr.required_sections`). Shipped default
...
if [ -n "$ERRORS" ]; then
  ...
  echo "See .apexyard/rules/git-conventions.md and .apexyard/rules/pr-quality.md." >&2
  exit 2
fi
