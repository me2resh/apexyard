...
# adopter who mistyped a path in `.apexyard/project-config.json` (or moved
...
CONFIG_LIB="$ROOT/.apexyard/hooks/_lib-read-config.sh"
PORTFOLIO_LIB="$ROOT/.apexyard/hooks/_lib-portfolio-paths.sh"
...
ApexYard: portfolio config is broken — $result
  Fix .apexyard/project-config.json (key: portfolio.{registry, projects_dir, ideas_backlog}),
