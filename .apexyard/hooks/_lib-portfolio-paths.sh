...
# from .apexyard/project-config.{defaults,}.json (via _lib-read-config.sh's
# `config_get_or`).
#
# Usage:
...
_portfolio_get() {
  ...
  if ! command -v config_get_or >/dev/null 2>&1; then
    local root
    root=$(_portfolio_root)
    if [ -n "$root" ] && [ -f "$root/.apexyard/hooks/_lib-read-config.sh" ]; then
      # shellcheck source=/dev/null
      . "$root/.apexyard/hooks/_lib-read-config.sh"
    fi
  fi
...
#   can drop a custom skill straight into .apexyard/skills/<name>/ if they
...
#   gitignored symlinks at .apexyard/skills/<name>/ for each subdirectory it
#   finds.
#
...
#   .apexyard/project-config.json (e.g.
...
portfolio_validate() {
  ...
  if _outside_fork "$registry" || _outside_fork "$projects_dir"; then
    if ! _outside_fork "$wd_real"; then
      echo "broken: partial split-portfolio v2 config — registry/projects_dir point at a sibling repo but workspace_dir falls back to the in-fork default; set .portfolio.workspace_dir in .apexyard/project-config.json to the sibling path (e.g. \"../<fork>-portfolio/workspace\"). See me2resh/apexyard#373."
      return 1
    fi
  fi
