#!/bin/bash
# Smoke tests for .apexyard/hooks/warn-stale-review-markers.sh
#
# Each case:
#   - sets up an isolated sandbox repo under $TMPDIR
#   - stages marker files (or not) under .apexyard/session/reviews/
...
make_sandbox() {
  ...
  cp "$HOOK_SRC"    "$sb/.apexyard/hooks/warn-stale-review-markers.sh"
  cp "$LIB_MARKERS" "$sb/.apexyard/hooks/_lib-review-markers.sh"
  chmod +x "$sb/.apexyard/hooks/warn-stale-review-markers.sh"
...
run_hook() {
  ...
  (
    ...
    printf '%s' "$json" \
      | "$sb/.apexyard/hooks/warn-stale-review-markers.sh" 2>"$stderr_file"
  )
  local rc=$?
