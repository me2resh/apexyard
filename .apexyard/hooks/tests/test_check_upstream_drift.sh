#!/bin/bash
# Smoke tests for .apexyard/hooks/check-upstream-drift.sh — focused on the
...
make_fork() {
  ...
  (
    ...
    mkdir -p .apexyard/hooks .apexyard/session
    cp "$HOOK_SRC" .apexyard/hooks/check-upstream-drift.sh
    chmod +x .apexyard/hooks/check-upstream-drift.sh
  )
  echo "$fk"
}
...
run_hook_from() {
  local fk="$1"
  ( cd "$fk" || exit 1; bash .apexyard/hooks/check-upstream-drift.sh 2>&1 )
}
