...
HOOK_SRC="$SRC_ROOT/.apexyard/hooks/validate-branch-name.sh"
...
make_sandbox_with_wrong_local_branch() {
  ...
  cp "$HOOK_SRC" "$sb/.apexyard/hooks/validate-branch-name.sh"
  ...
  chmod +x "$sb/.apexyard/hooks/validate-branch-name.sh"
  echo "$sb"
}
...
run_case() {
  ...
  got_stderr=$(cd "$sb" && echo "$input" | bash .apexyard/hooks/validate-branch-name.sh 2>&1 >/dev/null)
  got_rc=$?
  rm -rf "$sb"
