...
#        exist under .apexyard/agents/; silently skipped, no error.
...
#     5. Drift FIRES — pre-commit on a staged .apexyard/agents/<name>.md
...
# the real .apexyard/agents/*.md and write a stray snapshot there, while every
...
SYNC_HOOK="$SRC_ROOT/.apexyard/hooks/apply-agent-routing.sh"
DRIFT_HOOK="$SRC_ROOT/.apexyard/hooks/block-agent-routing-drift.sh"
LIB_OPS="$SRC_ROOT/.apexyard/hooks/_lib-ops-root.sh"
LIB_PORT="$SRC_ROOT/.apexyard/hooks/_lib-portfolio-paths.sh"
LIB_CFG="$SRC_ROOT/.apexyard/hooks/_lib-read-config.sh"
DEFAULTS="$SRC_ROOT/.apexyard/project-config.defaults.json"
...
YAML
    ...
    # Drop libs + defaults into the sandbox's .apexyard/hooks tree.
    mkdir -p .apexyard/hooks .apexyard/agents .apexyard/session
    cp "$LIB_OPS"  .apexyard/hooks/_lib-ops-root.sh
    cp "$LIB_PORT" .apexyard/hooks/_lib-portfolio-paths.sh
    cp "$LIB_CFG"  .apexyard/hooks/_lib-read-config.sh
    cp "$DEFAULTS" .apexyard/project-config.defaults.json
    cp "$SYNC_HOOK"  .apexyard/hooks/apply-agent-routing.sh
    cp "$DRIFT_HOOK" .apexyard/hooks/block-agent-routing-drift.sh
    chmod +x .apexyard/hooks/apply-agent-routing.sh .apexyard/hooks/block-agent-routing-drift.sh

    # Framework-default agent files (matrix per AgDR-0050 § Axis 2).
    cat > .apexyard/agents/qa-engineer.md <<'MD'
---
name: qa-engineer
...
# Salim — QA Engineer
MD
    cat > .apexyard/agents/tech-lead.md <<'MD'
---
name: tech-lead
...
# Hisham — Tech Lead
MD
    cat > .apexyard/agents/backend-engineer.md <<'MD'
---
...
MD
    ...
    cat > .apexyard/agents/ticket-manager.md <<'MD'
---
...
.apexyard/agents/.framework-defaults.json
.apexyard/session/agent-env/
agent-routing.yaml
IGNORE
...
before_qa=$(read_model_line "$SB/.apexyard/agents/qa-engineer.md")
before_lead=$(read_model_line "$SB/.apexyard/agents/tech-lead.md")
before_be=$(read_model_line "$SB/.apexyard/agents/backend-engineer.md")

# Run the hook (chdir into the fork so the walk-up finds the anchors).
banner_output=$(cd "$SB" && bash .apexyard/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true)

after_qa=$(read_model_line "$SB/.apexyard/agents/qa-engineer.md")
after_lead=$(read_model_line "$SB/.apexyard/agents/tech-lead.md")
after_be=$(read_model_line "$SB/.apexyard/agents/backend-engineer.md")
...
[ -f "$SB/.apexyard/agents/.framework-defaults.json" ] && defaults_snapshot_exists=1
...
if [ "$after_qa" = "opus" ] && [ "$after_lead" = "opus" ] && [ "$after_be" = "sonnet" ] \
  ...
  if grep -q '"qa-engineer":"haiku"' "$SB/.apexyard/agents/.framework-defaults.json"; then
    mark_pass "case 2: qa-engineer override haiku→opus (other agents untouched, snapshot recorded)"
  else
    mark_fail "case 2" "snapshot did not record qa-engineer=haiku: $(cat "$SB/.apexyard/agents/.framework-defaults.json" 2>/dev/null)"
  fi
else
...
(cd "$SB" && ANTHROPIC_BASE_URL=http://localhost:11434 PATH="$MOCK_BIN:$PATH" bash .apexyard/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true) >/dev/null

# Capture state after first run.
first_qa=$(read_model_line "$SB/.apexyard/agents/qa-engineer.md")
first_env=""
[ -f "$SB/.apexyard/session/agent-env/qa-engineer.env" ] && first_env=$(cat "$SB/.apexyard/session/agent-env/qa-engineer.env")

# Run again.
(cd "$SB" && ANTHROPIC_BASE_URL=http://localhost:11434 PATH="$MOCK_BIN:$PATH" bash .apexyard/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true) >/dev/null

second_qa=$(read_model_line "$SB/.apexyard/agents/qa-engineer.md")
second_env=""
[ -f "$SB/.apexyard/session/agent-env/qa-engineer.env" ] && second_env=$(cat "$SB/.apexyard/session/agent-env/qa-engineer.env")
...
agents:
  qa-engineer:
    model: opus
YAML
(cd "$SB" && bash .apexyard/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true) >/dev/null

# Stage the rewritten file (simulating an adopter accidentally
# `git add`-ing it).
(cd "$SB" && git add .apexyard/agents/qa-engineer.md 2>/dev/null)

# Invoke the drift guard with a `git commit -m "..."` command.
hook_output=$(hook_stdin "git commit -m 'feat: tweak QA agent'" | (cd "$SB" && bash .apexyard/hooks/block-agent-routing-drift.sh 2>&1) || true)
hook_exit=$(hook_stdin "git commit -m 'feat: tweak QA agent'" | (cd "$SB" && bash .apexyard/hooks/block-agent-routing-drift.sh > /dev/null 2>&1); echo $?)
...
cat > "$SB/.apexyard/agents/qa-engineer.md" <<'MD'
---
name: qa-engineer
description: QA Engineer wrapper.
model: opus
...
hook_exit=$(hook_stdin "git commit -m 'feat: bump QA default'" | (cd "$SB" && bash .apexyard/hooks/block-agent-routing-drift.sh > /dev/null 2>&1); echo $?)
...
hook_exit=$(hook_stdin "git commit -m 'docs: tidy QA agent description'" | (cd "$SB" && bash .apexyard/hooks/block-agent-routing-drift.sh > /dev/null 2>&1); echo $?)
...
after_tm=$(read_model_line "$SB/.apexyard/agents/ticket-manager.md")
after_qa=$(read_model_line "$SB/.apexyard/agents/qa-engineer.md")
defaults_snapshot_exists=0
[ -f "$SB/.apexyard/agents/.framework-defaults.json" ] && defaults_snapshot_exists=1
...
if [ "$after_tm" = "opus" ] && [ "$after_qa" = "haiku" ] \
  ...
  if grep -q '"ticket-manager":"sonnet"' "$SB/.apexyard/agents/.framework-defaults.json"; then
    mark_pass "case 8: ticket-manager utility override sonnet→opus (qa-engineer untouched, snapshot records sonnet)"
  else
    mark_fail "case 8" "snapshot did not record ticket-manager=sonnet: $(cat "$SB/.apexyard/agents/.framework-defaults.json" 2>/dev/null)"
  fi
else
...
# does NOT fire.
banner_output=$(cd "$SB" && ANTHROPIC_BASE_URL=http://localhost:4000 PATH="$MOCK_BIN:$PATH" bash .apexyard/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true)
after_tm=$(read_model_line "$SB/.apexyard/agents/ticket-manager.md")
session_env="$SB/.apexyard/session/agent-env/__session__.env"
agent_env="$SB/.apexyard/session/agent-env/ticket-manager.env"
session_set=0
agent_set=0
...
banner_output=$(cd "$SB" && PATH="$MOCK_BIN:$PATH" bash .apexyard/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true)
after_tm=$(read_model_line "$SB/.apexyard/agents/ticket-manager.md")
session_env="$SB/.apexyard/session/agent-env/__session__.env"
agent_env="$SB/.apexyard/session/agent-env/ticket-manager.env"
session_absent=1
...
banner_output=$(cd "$SB" && unset ANTHROPIC_BASE_URL; PATH="$MOCK_BIN:$PATH" bash .apexyard/hooks/apply-agent-routing.sh 2>&1 < /dev/null || true)
session_env="$SB/.apexyard/session/agent-env/__session__.env"
session_set=0
[ -f "$session_env" ] && grep -q '^ANTHROPIC_BASE_URL=http://localhost:4000$' "$session_env" && session_set=1
