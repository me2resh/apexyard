#!/bin/bash
# Tests for .apexyard/skills/status/briefing.sh — the helper that backs
...
HELPER_SRC="$SRC_ROOT/.apexyard/skills/status/briefing.sh"
...
YAML
  ...
  # the shim's "find helper at \$ops_root/.apexyard/skills/status/briefing.sh"
  # resolution works.
  mkdir -p "$sb/.apexyard/skills/status" "$sb/.apexyard/session/tickets"
  cp "$HELPER_SRC" "$sb/.apexyard/skills/status/briefing.sh"
  chmod +x "$sb/.apexyard/skills/status/briefing.sh"
...
#   $1 — sandbox root
#   $2 — marker filename relative to .apexyard/session/ (e.g.
...
write_marker() {
  ...
  local path="$sb/.apexyard/session/$rel"
...
run_case() {
  ...
  out=$(APEXYARD_OPS_ROOT="$sb" APEXYARD_CWD="$cwd" APEXYARD_GH="$gh_bin" \
        bash "$sb/.apexyard/skills/status/briefing.sh" 2>&1)
  local rc=$?
...
# ---------------------------------------------------------------------------
# 4. ops-fallback ticket marker — read from .apexyard/session/current-ticket
# ---------------------------------------------------------------------------
sb=$(make_fork)
