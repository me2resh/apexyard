...
HOOK_SRC="$SRC_ROOT/.apexyard/hooks/block-main-push.sh"
LIB_SRC="$SRC_ROOT/.apexyard/hooks/_lib-extract-push-ref.sh"
LIB_CONFIG_SRC="$SRC_ROOT/.apexyard/hooks/_lib-read-config.sh"
LIB_OPS_ROOT_SRC="$SRC_ROOT/.apexyard/hooks/_lib-ops-root.sh"
...
#   $sb/.apexyard/hooks/  — the hook + libs
#
...
make_sandbox() {
  ...
  cp "$HOOK_SRC"        "$sb/.apexyard/hooks/block-main-push.sh"
  cp "$LIB_SRC"         "$sb/.apexyard/hooks/_lib-extract-push-ref.sh"
  if [ -f "$LIB_CONFIG_SRC" ]; then
    cp "$LIB_CONFIG_SRC"  "$sb/.apexyard/hooks/_lib-read-config.sh"
  fi
  if [ -f "$LIB_OPS_ROOT_SRC" ]; then
    cp "$LIB_OPS_ROOT_SRC" "$sb/.apexyard/hooks/_lib-ops-root.sh"
  fi
  if [ -f "$SRC_ROOT/.apexyard/project-config.defaults.json" ]; then
    cp "$SRC_ROOT/.apexyard/project-config.defaults.json" \
       "$sb/.apexyard/project-config.defaults.json"
  fi
  chmod +x "$sb/.apexyard/hooks/block-main-push.sh"
...
run_case() {
  ...
  got_stderr=$(cd "$hook_cwd" && echo "$input" | bash .apexyard/hooks/block-main-push.sh 2>&1 >/dev/null)
  got_rc=$?
