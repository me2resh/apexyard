...
# also run this locally before pushing: `bash .apexyard/hooks/tests/test_site_counts.sh`.
#
# Rationale: docs/agdr/AgDR-0046-site-counts-drift-prevention.md.

set -u

# Resolve the framework root — the test sits at .apexyard/hooks/tests/,
...
actual_skills=$(find .apexyard/skills -name SKILL.md 2>/dev/null | wc -l | tr -d ' ')
actual_hooks=$(find .apexyard/hooks -maxdepth 1 -name '*.sh' ! -name '_lib*' 2>/dev/null | wc -l | tr -d ' ')
...
2. **Capability layer** — the runnable spec: `.apexyard/skills/` (52
   slash commands), `.apexyard/agents/` (23 sub-agents).
FIXTURE
...
# (step 3.5 of .apexyard/skills/release/SKILL.md). This guard asserts the site's
...
if [ "$SITE_VERSION" != "$CHANGELOG_VERSION" ]; then
  ...
  echo "      per .apexyard/skills/release/SKILL.md step 3.5) to match CHANGELOG.md."
  exit 1
fi
