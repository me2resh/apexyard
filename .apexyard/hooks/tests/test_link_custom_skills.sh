#!/bin/bash
# Smoke tests for .apexyard/hooks/link-custom-skills.sh — the SessionStart
# hook that surfaces private custom skills (split-portfolio v2 + #243) to
# Claude Code by symlinking them into the public fork's .apexyard/skills/.
#
# Cases:
...
# _lib-ops-root.sh and WRITES symlinks into <ops-root>/.apexyard/skills/. Inside a
# live Claude Code session the session pin would resolve to the REAL fork, so a
# sandbox run would symlink into the real .apexyard/skills/. Disable the pin so
...
MD
    ...
    mkdir -p .apexyard/hooks .apexyard/skills
    cp "$HOOK_SRC"          .apexyard/hooks/link-custom-skills.sh
    cp "$LIB_PORTFOLIO_SRC" .apexyard/hooks/_lib-portfolio-paths.sh
    cp "$LIB_CONFIG_SRC"    .apexyard/hooks/_lib-read-config.sh
    if [ "$copy_ops_root" -eq 1 ]; then
      cp "$LIB_OPS_ROOT_SRC" .apexyard/hooks/_lib-ops-root.sh
    fi
    cp "$DEFAULTS_SRC"      .apexyard/project-config.defaults.json
    chmod +x .apexyard/hooks/link-custom-skills.sh
...
run_hook() {
  ...
  local prefix="${2:-}"
  ( cd "$sb" || exit 99
    eval "$prefix bash .apexyard/hooks/link-custom-skills.sh 2>&1" )
}
...
[ "$rc" -eq 0 ] || ok=0
[ -z "$out" ] || ok=0
[ -z "$(ls -A "$SB/.apexyard/skills" 2>/dev/null)" ] || ok=0
[ "$ok" -eq 1 ] && rc2=0 || rc2=1
...
# /check-policy
MD
cat > "$SB/.apexyard/project-config.json" <<JSON
...
[ -L "$SB/.apexyard/skills/file-internal-bug" ] || ok=0
[ -L "$SB/.apexyard/skills/check-policy" ] || ok=0
[ -f "$SB/.apexyard/skills/file-internal-bug/SKILL.md" ] || ok=0
[ -f "$SB/.apexyard/skills/check-policy/SKILL.md" ] || ok=0
[ "$ok" -eq 1 ] && rc2=0 || rc2=1
...
mkdir -p "$SB/.apexyard/skills/feature"
cat > "$SB/.apexyard/skills/feature/SKILL.md" <<'MD'
---
name: feature
---
# Framework /feature
...
[ -d "$SB/.apexyard/skills/feature.framework.bak" ] || ok=0
# Custom symlink in place.
[ -L "$SB/.apexyard/skills/feature" ] || ok=0
# Symlink resolves to the custom SKILL.md (the body says "Custom").
grep -q 'Custom /feature' "$SB/.apexyard/skills/feature/SKILL.md" || ok=0
...
[ -L "$SB/.apexyard/skills/real-skill" ] || ok=0
[ ! -e "$SB/.apexyard/skills/no-skill-md" ] || ok=0
# Summary line says 1.
...
[ ! -f "$SB/.apexyard/hooks/_lib-ops-root.sh" ] || {
...
out=$(run_hook "$SB")
ok=1
[ -L "$SB/.apexyard/skills/fallback-skill" ] || ok=0
echo "$out" | grep -q "linked 1 custom skill" || ok=0
[ "$ok" -eq 1 ] && rc2=0 || rc2=1
