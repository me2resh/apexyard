#!/bin/bash
# Smoke tests for .apexyard/hooks/pre-push-gate.sh
#
# Each case:
...
make_sandbox() {
  ...
  cp "$HOOK_SRC" "$sb/.apexyard/hooks/pre-push-gate.sh"
  chmod +x "$sb/.apexyard/hooks/pre-push-gate.sh"
  ...
  if [ -f "$src_root/.apexyard/hooks/_lib-read-config.sh" ]; then
    cp "$src_root/.apexyard/hooks/_lib-read-config.sh" "$sb/.apexyard/hooks/_lib-read-config.sh"
  fi
  if [ -f "$src_root/.apexyard/project-config.defaults.json" ]; then
    cp "$src_root/.apexyard/project-config.defaults.json" "$sb/.apexyard/project-config.defaults.json"
  fi
  echo "$sb"
}
...
run_hook() {
  ...
  (
    cd "$sb" || exit 1
    echo "$stdin_payload" | bash .apexyard/hooks/pre-push-gate.sh 2>/tmp/pre-push-gate-stderr.$$
  )
  local got_rc=$?
  local got_stderr
...
case1() {
  ...
  echo '{"tool_input":{"command":"ls -la"}}' | (cd "$sb" && bash .apexyard/hooks/pre-push-gate.sh 2>/dev/null)
  local rc=$?
  if [ "$rc" = "0" ]; then
...
case2() {
  local sb; sb=$(make_sandbox)
  cat > "$sb/.apexyard/project-config.json" <<'EOF'
{"pre_push": {"commands": []}}
EOF
