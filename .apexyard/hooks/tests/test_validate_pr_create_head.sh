...
HOOK_SRC="$SRC_ROOT/.apexyard/hooks/validate-pr-create.sh"
LIB_CFG="$SRC_ROOT/.apexyard/hooks/_lib-read-config.sh"
LIB_TRACKER="$SRC_ROOT/.apexyard/hooks/_lib-tracker.sh"
...
make_sandbox() {
  ...
  cp "$HOOK_SRC" "$sb/.apexyard/hooks/validate-pr-create.sh"
  if [ -f "$LIB_CFG" ]; then cp "$LIB_CFG" "$sb/.apexyard/hooks/_lib-read-config.sh"; fi
  if [ -f "$LIB_TRACKER" ]; then cp "$LIB_TRACKER" "$sb/.apexyard/hooks/_lib-tracker.sh"; fi
  if [ -f "$DEFAULTS" ]; then cp "$DEFAULTS" "$sb/.apexyard/project-config.defaults.json"; fi
...
run_case() {
  ...
  got_stderr=$(cd "$sb" && echo "$input" | bash .apexyard/hooks/validate-pr-create.sh 2>&1 >/dev/null)
  got_rc=$?
  rm -rf "$sb"
