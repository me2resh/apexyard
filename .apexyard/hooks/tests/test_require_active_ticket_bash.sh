...
HOOK_SRC="$SRC_ROOT/.apexyard/hooks/require-active-ticket.sh"
LIB_BASH="$SRC_ROOT/.apexyard/hooks/_lib-detect-bash-write.sh"
...
make_sandbox() {
  ...
  mkdir -p "$sb/.apexyard/hooks" "$sb/.apexyard/session"
  cp "$HOOK_SRC" "$sb/.apexyard/hooks/require-active-ticket.sh"
  cp "$LIB_BASH" "$sb/.apexyard/hooks/_lib-detect-bash-write.sh"
  cp "$LIB_CFG"  "$sb/.apexyard/hooks/_lib-read-config.sh"
  cp "$DEFAULTS" "$sb/.apexyard/project-config.defaults.json"
  chmod +x "$sb/.apexyard/hooks/require-active-ticket.sh"
  echo "$sb"
}
...
run_case() {
  ...
  got_stderr=$(cd "$sb" && echo "$input" | bash .apexyard/hooks/require-active-ticket.sh 2>&1 >/dev/null)
  got_rc=$?
  rm -rf "$sb"
...
# 4. echo > .apexyard/foo.json → allowed (path exemption catches it)
sb=$(make_sandbox)
in=$(jq -nc --arg c "echo x > .apexyard/foo.json" '{tool_name:"Bash", tool_input:{command:$c}}')
run_case "bash write to .apexyard/ exempt" 0 "" "$in" "$sb"
...
sb=$(make_sandbox)
echo "setup" > "$sb/.apexyard/session/active-bootstrap"
...
sb=$(make_sandbox)
echo "handover" > "$sb/.apexyard/session/active-bootstrap"
in=$(jq -nc --arg p "$sb/src/foo.ts" '{tool_name:"Edit", tool_input:{file_path:$p}}')
...
sb=$(make_sandbox)
echo "some-random-skill" > "$sb/.apexyard/session/active-bootstrap"
in=$(jq -nc --arg p "$sb/src/foo.ts" '{tool_name:"Edit", tool_input:{file_path:$p}}')
...
sb=$(make_sandbox)
: > "$sb/.apexyard/session/active-bootstrap"
in=$(jq -nc --arg p "$sb/src/foo.ts" '{tool_name:"Edit", tool_input:{file_path:$p}}')
...
sb=$(make_sandbox)
cat > "$sb/.apexyard/session/current-ticket" <<EOF
...
sb=$(make_sandbox)
rsb=$(cd "$sb" && pwd -P)
mkdir -p "$sb/.apexyard/session/tickets/myproj"
cat > "$sb/.apexyard/session/tickets/myproj/feature__x" <<EOF
repo=me2resh/apexyard
number=513
title=worktree A
EOF
...
rsb=$(cd "$sb" && pwd -P)
mkdir -p "$sb/.apexyard/session/tickets/myproj"
cat > "$sb/.apexyard/session/tickets/myproj/feature__a" <<EOF
repo=me2resh/apexyard
number=513
title=worktree A
EOF
...
sb=$(make_sandbox)
rsb=$(cd "$sb" && pwd -P)
mkdir -p "$sb/.apexyard/session/tickets"
cat > "$sb/.apexyard/session/tickets/myproj" <<EOF
repo=me2resh/apexyard
number=513
title=single agent
EOF
...
mkdir -p "$sb/.apexyard/session/tickets/myproj"
cat > "$sb/.apexyard/session/tickets/myproj/wt-x" <<EOF
repo=me2resh/apexyard
number=513
...
# 19. echo > .apexyard/session/foo with no ticket → allowed (exempt .apexyard/ path)
sb=$(make_sandbox)
in=$(jq -nc --arg c "echo x > .apexyard/session/foo" '{tool_name:"Bash", tool_input:{command:$c}}')
run_case "bash redirect to .apexyard/ exempt" 0 "" "$in" "$sb"

# 20. cp src dst where dst is a .apexyard/ path → allowed (exempt destination)
sb=$(make_sandbox)
in=$(jq -nc --arg c "cp .apexyard/session/tickets/myproj .apexyard/session/current-ticket" \
      '{tool_name:"Bash", tool_input:{command:$c}}')
run_case "bash cp to .apexyard/ destination exempt" 0 "" "$in" "$sb"
