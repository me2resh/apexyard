...
#   - writes the relevant marker files to .apexyard/session/reviews/
...
HOOK_SRC="$SRC_ROOT/.apexyard/hooks/block-unreviewed-merge.sh"
LIB_PR="$SRC_ROOT/.apexyard/hooks/_lib-extract-pr.sh"
LIB_MARKERS="$SRC_ROOT/.apexyard/hooks/_lib-review-markers.sh"
...
make_sandbox() {
  ...
  mkdir -p "$sb/.apexyard/hooks" "$sb/.apexyard/session/reviews" "$sb/bin"
  cp "$HOOK_SRC"    "$sb/.apexyard/hooks/block-unreviewed-merge.sh"
  cp "$LIB_PR"      "$sb/.apexyard/hooks/_lib-extract-pr.sh"
  cp "$LIB_MARKERS" "$sb/.apexyard/hooks/_lib-review-markers.sh"
  chmod +x "$sb/.apexyard/hooks/block-unreviewed-merge.sh"
...
run_case() {
  ...
  got_stderr=$(cd "$sb" && APEXYARD_OPS_DISABLE_PIN=1 PATH="$sb/bin:$PATH" bash -c "echo '$input' | bash .apexyard/hooks/block-unreviewed-merge.sh" 2>&1 >/dev/null)
  got_rc=$?
  rm -rf "$sb"
...
cp "$SRC_ROOT/.apexyard/hooks/_lib-ops-root.sh"      "$sb/.apexyard/hooks/_lib-ops-root.sh"
cp "$SRC_ROOT/.apexyard/hooks/_lib-review-markers.sh" "$sb/.apexyard/hooks/_lib-review-markers.sh"
...
got_stderr=$(cd "$sb/workspace/demo" && APEXYARD_OPS_DISABLE_PIN=1 PATH="$sb/bin:$PATH" bash -c "echo '$input' | bash $sb/.apexyard/hooks/block-unreviewed-merge.sh" 2>&1 >/dev/null)
got_rc=$?
rm -rf "$sb"
if [ "$got_rc" = "0" ] && [ -z "$got_stderr" ]; then
...
cmd="cat > $sb/.apexyard/session/reviews/42-ceo.approved <<EOF
sha=${FIXED_SHA}
approved_by=user
...
sb=$(make_sandbox)
write_rex_marker "$sb" 42
cmd="printf 'sha=${FIXED_SHA}\nskill_version=2\n' > $sb/.apexyard/session/reviews/42-ceo.approved && gh pr merge 42 --repo me2resh/apexyard --squash"
...
WARN_HOOK_SRC="$SRC_ROOT/.apexyard/hooks/warn-review-marker-write.sh"
if [ ! -f "$WARN_HOOK_SRC" ]; then
  ...
  input=$(jq -nc --arg fp ".apexyard/session/reviews/me2resh__apexyard__42-rex.approved" \
  ...
  input=$(jq -nc --arg c "echo 'sha=abc' > .apexyard/session/reviews/me2resh__apexyard__42-ceo.approved" \
    '{tool_name:"Bash", tool_input:{command:$c}}')
  got_stderr=$(echo "$input" | bash "$WARN_HOOK_SRC" 2>&1 >/dev/null)
  got_rc=$?
  if [ "$got_rc" = "0" ] && echo "$got_stderr" | grep -q "ADVISORY"; then
