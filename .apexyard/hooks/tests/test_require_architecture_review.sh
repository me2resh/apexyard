...
HOOK_SRC="$SRC_ROOT/.apexyard/hooks/require-architecture-review.sh"
...
make_sandbox() {
  ...
  mkdir -p "$sb/.apexyard/hooks" "$sb/.apexyard/session/reviews"
  # Copy libs needed by the hook.
  cp "$SRC_ROOT/.apexyard/hooks/_lib-extract-pr.sh" "$sb/.apexyard/hooks/_lib-extract-pr.sh"
  cp "$SRC_ROOT/.apexyard/hooks/_lib-review-markers.sh" "$sb/.apexyard/hooks/_lib-review-markers.sh"
  if [ -f "$SRC_ROOT/.apexyard/hooks/_lib-ops-root.sh" ]; then
