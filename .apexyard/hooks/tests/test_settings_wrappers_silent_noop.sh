...
# "$r/.apexyard/hooks/<name>.sh"`. When `$r` walks all the way to `/`, the
# exec lands on `/.apexyard/hooks/<name>.sh` — which doesn't exist. Result:
...
WRAPPER='r="";if [ -n "${CLAUDE_CODE_SESSION_ID:-}" ];then p="${APEXYARD_OPS_PIN_DIR:-$HOME/.apexyard/pins}/ops-root-${CLAUDE_CODE_SESSION_ID}";[ -f "$p" ] && IFS= read -r r < "$p" && [ -d "$r/.apexyard/hooks" ] || r="";fi;if [ -z "$r" ];then r=$PWD;while [ -n "$r" ] && [ "$r" != / ];do { [ -f "$r/.apexyard-fork" ] || [ -f "$r/onboarding.yaml" ]; } && [ -d "$r/.apexyard/hooks" ] && break;r=${r%/*};done;fi;[ -d "$r/.apexyard/hooks" ] || exit 0;exec "$r/.apexyard/hooks/detect-role-trigger.sh"'
