...
Rex's advisory check (see `.apexyard/agents/code-reviewer.md`) flags label-only bullets as a `nit:` / `suggestion:` finding, not a blocking verdict, so legitimate short bullets don't churn the merge gate.
...
If the PR touches user-facing UI → design review is required before merge. Mechanically enforced by `require-design-review-for-ui.sh` (blocks merge without a design marker) and the `/approve-design <pr>` skill (writes the marker on explicit designer approval). See `.apexyard/skills/approve-design/SKILL.md` for the full invocation rules.

**Customising what counts as "UI"** (`.apexyard/project-config.json`):
