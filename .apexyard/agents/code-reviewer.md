...
- **Framework rules** at `.apexyard/rules/*.md` — the generic ApexYard standards (code quality, PR workflow, AgDR requirements, etc.). Always loaded.
...
Per `.apexyard/rules/pr-quality.md` § "Summary bullets — narrative quality", every bullet in the PR's `## Summary` section should answer two questions: **what changed** AND **why it matters to the person reading this**. Label-only bullets force reviewers into diff archaeology before they can pick a review focus.
...
**Verdict effect: NONE.** This is an **advisory** finding only. Surface it as a `nit:` or `suggestion:` comment in the review body, cite `.apexyard/rules/pr-quality.md` § "Summary bullets — narrative quality" so the author can self-correct on the next PR, and do NOT downgrade the overall verdict from APPROVED to CHANGES REQUESTED on this finding alone. The rationale: the false-positive rate on a heuristic this simple is too high to justify blocking (legitimate one-line bug fixes and dependency bumps would churn the merge gate); the goal is to surface the rule, not to mechanically enforce it.
...
| **Private custom handbooks** | `<private_repo>/custom-handbooks/**/*.md`, resolved via `portfolio_custom_handbooks_dir` from `.apexyard/hooks/_lib-portfolio-paths.sh` | Company-confidential standards that name internal systems, refer to proprietary policy, or otherwise should not appear on a public repo (split-portfolio adopters only — single-fork adopters typically don't have this dir) |
...
PRIV=""
if [ -f "$OPS_ROOT/.apexyard/hooks/_lib-portfolio-paths.sh" ]; then
  source "$OPS_ROOT/.apexyard/hooks/_lib-read-config.sh"
  source "$OPS_ROOT/.apexyard/hooks/_lib-portfolio-paths.sh"
...
. "$MARKER_HOME/.apexyard/hooks/_lib-review-markers.sh"
mkdir -p "$MARKER_HOME/.apexyard/session/reviews"
...
41 bytes: 40 hex + `\n`. No labels, no keys, no timestamp, no trailing text. Confirm with `od -c .apexyard/session/reviews/{number}-rex.approved | head -2` — the first two bytes of the second line should be `\n` then `*` (the asterisk is `od`'s repeat marker for EOF).
...
All of these fail the hook's whitespace-strip-then-compare check. The merge gate blocks the PR; the only way forward is hand-editing the marker, which is itself a rule violation per `.apexyard/rules/pr-workflow.md` § "Mechanical enforcement". Don't create that situation.

### Where to write

The marker lands at `$REX_MARKER` — the repo-qualified path returned by `review_marker_path` above. Its form is `<ops_fork_root>/.apexyard/session/reviews/<owner>__<repo>__<pr>-rex.approved` (AgDR-0060). The merge-gate hook resolves the same path via the same helper. Inside a workspace clone (`workspace/<project>/`), this is NOT the project clone's `.apexyard/session/reviews/` — it's the ops fork above. If running in a nested worktree of the ops fork, the worktree shares the ops fork's session state (worktrees see the parent's tree below `.apexyard/`).
