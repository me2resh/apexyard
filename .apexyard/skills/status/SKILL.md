...
When `--briefing` (or `-b`) is passed, skip every other section and shell out to `.apexyard/skills/status/briefing.sh`. The full default `/status` output is unchanged for invocations without the flag.
...
| Active ticket | `<ops_root>/.apexyard/session/tickets/<workspace>` first (only when workspace is a real name), then `<ops_root>/.apexyard/session/current-ticket`. Format: `#<number> — <title>`. No marker → `(none)` |
...
Implementation: the logic lives in `.apexyard/skills/status/briefing.sh` so it's testable in isolation and runnable from a plain shell (no Claude Code dependency). Smoke tests at `.apexyard/hooks/tests/test_status_briefing.sh`.

When invoked from inside Claude Code as `/status --briefing`, run the helper:

```bash
bash .apexyard/skills/status/briefing.sh
```
