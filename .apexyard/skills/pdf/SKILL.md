...
Read `workspace_dir` and `projects_dir` from `.apexyard/hooks/_lib-portfolio-paths.sh` so split-portfolio adopters resolve to the sibling private repo transparently:
...
`.apexyard/project-config.defaults.json` ships a `pdf` block:
...
Adopters override in `.apexyard/project-config.json`:
...
- `.apexyard/skills/process/lint.sh` — graceful-degrade-on-missing-dep pattern that this skill mirrors for converter detection
- `.apexyard/skills/_lib-mermaid-lint.sh` — same pattern, npx-fallback case
