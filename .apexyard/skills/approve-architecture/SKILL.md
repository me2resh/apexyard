...
Writes `.apexyard/session/reviews/<owner>__<repo>__<pr>-architecture.approved` (repo-qualified path, see AgDR-0060) with the current HEAD SHA so the `require-architecture-review.sh` merge-gate hook will let a design-artifact PR through. Without this marker, the hook blocks merges on any PR that touches a technical design, a migration AgDR, or a feature spec / PRD.
...
Normally Tariq writes the marker himself on an APPROVED verdict (see `.apexyard/agents/solution-architect.md`). This skill is the **operator path** to record the same marker — for when a human architect reviewed the design, or when you need to re-record after a rebase.
...
- The marker is gitignored (`.apexyard/session/` is in `.gitignore`). Session state, not code.
