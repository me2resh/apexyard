...
Pair this DFD with [\`/threat-model\`](../../../.apexyard/skills/threat-model/SKILL.md) so the boundary table feeds directly into a per-arrow STRIDE enumeration. Pair with [\`/compliance-check\`](../../../.apexyard/skills/compliance-check/SKILL.md) so the classifications table feeds directly into cross-border-transfer and DPA-coverage analysis.
