...
**G3 detail.** The skill iterates the AI-crawler list at `.apexyard/registries/ai-crawlers.json` (12 entries spanning training and retrieval scopes). For each crawler it reports:
...
5. **AI-crawler list lives in the registry.** `.apexyard/registries/ai-crawlers.json` is the single source of truth. The G3 check iterates the list. Adopters who want to add an internal crawler edit the JSON.
...
| File | Purpose |
|------|---------|
| `.apexyard/registries/ai-crawlers.json` | v1 AI-crawler list (12 entries) consumed by the G3 check |
| `templates/audits/geo-audit.md` | Per-run body template (audit family standard, AgDR-0019) |
| `.apexyard/hooks/_lib-audit-history.sh` | Shared persistence + trend rendering (audit family standard, AgDR-0019) |
| `.apexyard/skills/seo-audit/SKILL.md` | Google-shaped SEO sibling — independent, complementary |
| `.apexyard/skills/launch-check/SKILL.md` | Fans out to both this skill and `/seo-audit` at milestone boundaries |
