...
`/handover` may run before any tracker tickets exist for the project being adopted, so the `require-active-ticket.sh` PreToolUse hook would block the registry / `projects/<name>/` writes the skill needs. Write a marker so the hook exempts this skill (it's on the default `bootstrap_skills` list in `.apexyard/project-config.defaults.json`):

```bash
mkdir -p .apexyard/session && echo "handover" > .apexyard/session/active-bootstrap
```
...
- `.apexyard/session/active-bootstrap` — the marker itself (step 0)
...
- [ ] PR workflow (`.apexyard/rules/pr-workflow.md`) — every change goes through a PR
...
`/update` reads these two files to know which topology to diff against (see [`.apexyard/skills/update/SKILL.md`](../update/SKILL.md) § "Topology drift detection").
...
Mirrors `/tickets-batch`'s failure-handling shape (see [`.apexyard/skills/tickets-batch/SKILL.md`](../tickets-batch/SKILL.md) § "Failure handling").
