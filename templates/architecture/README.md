...
| **One feature's slice** — the routes / models / jobs / screens that participate in **one** feature | Generated by [`/feature-diagram`](../../.apexyard/skills/feature-diagram/SKILL.md) — no hand-template; consumes the inventory at `projects/<name>/feature-inventory.md` produced by `/extract-features` |
...
The two C4 templates describe **what is**. `vision.md` describes **what will be**. `dfd.md` and `sequence.md` are **lenses** on the same components — DFD asks "where are the security-relevant data crossings?", sequence asks "what happens, in what order, when this flow runs?". Per-feature diagrams (`features/<slug>.md`, generated by [`/feature-diagram`](../../.apexyard/skills/feature-diagram/SKILL.md)) are a **lens scoped to one feature** — the routes / models / jobs / screens that participate in just that one feature, with the rest of the system filtered out.
