...
- `.apexyard/` — canonical framework content (agent/tool-neutral source of truth)
  - `.apexyard/hooks/` — 38 shell scripts (PreToolUse / PostToolUse / SessionStart)
  - `.apexyard/skills/` — 53 slash commands (one dir per skill, each with `SKILL.md`)
  - `.apexyard/agents/` — 23 sub-agents: 5 utility (Rex code-reviewer, Hakim security-reviewer/auditor, Munir dep-auditor, Tariq PR-manager, Idris ticket-manager) + 18 dept-aligned agents across engineering / product / design / security / data
  - `.apexyard/rules/` — 11 modular rule files
  - `.apexyard/project-config.defaults.json` — framework defaults
  - `.apexyard/project-config.json` — adopter overrides (gitignored)
- `.claude/` — Claude Code registration layer (generated from `.apexyard/` by `bin/apexyard-sync-tool-dirs`)
  - `.claude/settings.json` — Claude Code hook wiring
- `.kimi-code/` — Kimi Code registration layer (generated from `.apexyard/` by `bin/apexyard-sync-tool-dirs`)
- `bin/apexyard-sync-tool-dirs` — keeps `.claude/` and `.kimi-code/` in sync with `.apexyard/`
...
- `KIMI.md` — framework-level instructions for Kimi Code CLI adopters
...
- `~/.kimi-code/config.toml` — Kimi Code CLI hook wiring (registered by the adopter; see `KIMI.md`)
- `.apexyard/project-config.defaults.json` — framework defaults (immutable from the framework's side; adopters override via `.apexyard/project-config.json`)
- `bin/apexyard-sync-tool-dirs` — syncs generated `.claude/` and `.kimi-code/` directories with `.apexyard/`
- `README.md` — public-facing project description + Quick Start
- `LICENSE` — MIT

## Sandbox & test environments

- `.apexyard/hooks/tests/` — hook test suite (~30+ bash test files; run via `bash .apexyard/hooks/tests/test_<name>.sh`)
- `.apexyard/skills/<name>/tests/` — per-skill smoke tests where applicable
...
- **Ticket-first hook** — code edits are blocked without an active ticket marker at `.apexyard/session/current-ticket`. Bootstrap-class skills (`/setup`, `/handover`, `/update`, `/split-portfolio`) are exempt.
...
- **Workflow gates** — documented in `.apexyard/rules/workflow-gates.md`. Six gates from PRD → Done; each gate has a mechanical check or an advisory reminder.
...
- **Plan mode for ≥ 4 dependent steps** — see `.apexyard/rules/plan-mode.md`.
- **Fan-out for ≥ 2 independent items** — see `.apexyard/rules/parallel-work.md`.
...
3. Browse `.apexyard/skills/` for the 53 slash commands (each `SKILL.md` is one capability)
4. Browse `roles/` to understand the role-activation model
5. Browse `templates/` for the standard document shapes
6. Check `.apexyard/rules/` for the mechanical rules (ticket vocabulary, PR workflow, plan mode, parallel work, leak protection, etc.)

The framework is plain markdown + shell — no build step, no SaaS, no lock-in. MIT licensed.

## Related entry-point conventions

- **`site/skill.md`** — capability manifest at site root (lowercase, distinct from `.apexyard/skills/<name>/SKILL.md` — see the naming-clash callout inside that file)
